...
const UNDERLINE_ICON = `<svg><path fill="#000000" d="M6,12 C8.76,12 11,9.76 11,7 L11,0 L9,0 L9,7 C9,8.75029916 7.49912807,10 6,10 C4.50087193,10 3,8.75837486 3,7 L3,0 L1,0 L1,7 C1,9.76 3.24,12 6,12 Z M0,13 L0,15 L12,15 L12,13 L0,13 Z" transform="translate(3 3)"/></svg>`;
...
const FILTER_ICON = `<svg><path fill="#000000" fill-rule="evenodd" d="M0,0 L0,1 L6,7 L6,12 L8,11 L8,7 L14,1 L14,0 L0,0 Z M4,3 L10,3 L7,6 L4,3 Z" transform="translate(2 3)"/></svg>`;
const SIGMA_ICON = `<svg><polygon fill="#000000" fill-rule="evenodd" points="10 0 0 0 0 1.8 5.5 7 0 12.2 0 14 10 14 10 12 3.1 12 8 7 3.1 2 10 2" transform="translate(4 2)"/></svg>`;
const TRIANGLE_DOWN_ICON = `<svg><polygon fill="#000000" fill-rule="evenodd" points="0 0 4 4 8 0" transform="translate(5 7)"/></svg>`;
const TRIANGLE_RIGHT_ICON = `<svg><polygon fill="#000000" fill-rule="evenodd" points="-2 2 2 6 6 2" transform="rotate(-90 8 3)"/></svg>`;
// const REDO_ICON = `<svg></svg>`;
// const REDO_ICON = `<svg></svg>`;

// -----------------------------------------------------------------------------
// Tools
// -----------------------------------------------------------------------------
class UndoTool extends Component {
  static template = xml`<div class="o-tool">${UNDO_ICON}</div>`;
}

class RedoTool extends Component {
  static template = xml`<div class="o-tool">${REDO_ICON}</div>`;
}
  ...
  static template = xml/* xml */ `
    <div class="o-spreadsheet-toolbar">
      ...
      <div class="o-tools">
        <UndoTool />
        <RedoTool />
        <span class="o-tool">${PRINT_ICON}</span>
        <span class="o-tool">${PAINT_FORMAT_ICON}</span>
        <span class="o-tool">${CLEAR_FORMAT_ICON}</span>
        <span class="o-tool">${BOLD_ICON}</span>
        <span class="o-tool">${ITALIC_ICON}</span>
        <span class="o-tool">${UNDERLINE_ICON}</span>
        <span class="o-tool">${STRIKE_ICON}</span>
        <span class="o-tool">${TEXT_COLOR_ICON}</span>
        <span class="o-tool">${FILL_COLOR_ICON}</span>
        <span class="o-tool">${MERGE_CELL_ICON}</span>
        <span class="o-tool">${ALIGN_LEFT_ICON}</span>
        <span class="o-tool">${ALIGN_CENTER_ICON}</span>
        <span class="o-tool">${ALIGN_RIGHT_ICON}</span>
        <span class="o-tool">${ALIGN_TOP_ICON}</span>
        <span class="o-tool">${ALIGN_MIDDLE_ICON}</span>
        <span class="o-tool">${ALIGN_BOTTOM_ICON}</span>
        <span class="o-tool">${TEXT_WRAPPING_ICON}</span>
        <span class="o-tool">${FILTER_ICON}</span>
        <span class="o-tool">${SIGMA_ICON}</span>
        <span class="o-tool">${TRIANGLE_DOWN_ICON}</span>


        <i class="fas fa-undo" disabled="disabled"></i> 
