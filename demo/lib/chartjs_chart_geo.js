/*!
 * chartjs-chart-geo v4.3.6 - PATCHED COPY
 * https://github.com/sgratzl/chartjs-chart-geo
 * Copyright (c) 2019-2023 Samuel Gratzl <sam@sgratzl.com>
 * Released under the MIT license
 *
 * Vendored (like lib/chart_js_treemap.js) because the library is consumed as a
 * UMD global (`ChartGeo`) rather than bundled, so it cannot be patched from src/.
 *
 * Local changes on top of the upstream 4.3.6 build, all rendering performance
 * (see `GeoPathRecorder`, `GeoFeature`, `ProjectionScale`, `GeoController`):
 *
 *  - GeoFeature caches a Path2D instead of one offscreen <canvas> per feature.
 *    The feature is streamed through the projection once - building the path and
 *    its bounding box together - instead of three times (path + bounds +
 *    centroid), and the cache is no longer keyed on colors, so hovering and
 *    recoloring no longer re-rasterize anything.
 *  - GeoFeature.inRange hit-tests with ctx.isPointInPath on that path instead of
 *    geoContains, which walked the geometry on every mousemove.
 *  - ProjectionScale.init no longer rebuilds the projection on every update.
 *    Chart.js re-inits scales on each update(); rebuilding discarded
 *    oldChartBounds, which made updateBounds() always report a change and so
 *    dropped every element's cached geometry on every frame.
 *  - ProjectionScale.computeBounds and GeoController.resolveOutline are memoized;
 *    linkScales() re-ran two full passes over the whole map on every update.
 *  - ProjectionScale accepts a `precision` option to relax d3's adaptive
 *    resampling (set by src/.../chartjs_geo_projection_plugin.ts).
 *
 * All of the above is upstreamable as-is; port it to the library's src/ when it
 * is forked, and drop this copy.
 */
(function (global, factory) {
  ...
  class ProjectionScale extends chart_js.Scale {
    ...
    init(options) {
      ...
      super.init(options);
      // chart.js re-inits every scale on every update. Rebuilding the projection
      // here would discard `outlineBounds`/`oldChartBounds`, which in turn makes
      // `updateBounds()` always report a change and drops every element's cached
      // geometry on each frame. Only rebuild when the projection actually changed.
      if (this._projectionOption !== options.projection || !this.projection) {
        this._projectionOption = options.projection;
        ...
        if (
          ...
          typeof this.projection.precision === "function"
        ) {
          // d3 adaptively resamples every segment (default precision sqrt(0.5)).
          // A coarser value cuts a sizeable chunk off each projection stream and
          // is imperceptible on already-simplified geometry.
          this.projection.precision(options.precision);
        }
        this.geoPath.projection(this.projection);
        this.outlineBounds = null;
        this.oldChartBounds = null;
        this._boundsOutline = undefined;
        this._boundsProjection = undefined;
      }
    }
    computeBounds(outline) {
      // chart.js calls linkScales() -> computeBounds() on every update, but the
      // result only depends on the outline geometry and the projection instance.
      if (
        this.outlineBounds &&
        this._boundsOutline === outline &&
        this._boundsProjection === this.projection
      ) {
        return;
      }
      this._boundsOutline = outline;
      this._boundsProjection = this.projection;
      // Tell the next updateBounds() that cached geometry is stale.
    ...
    updateBounds() {
      ...
      const boundsChanged = this._boundsChanged;
  ...
  var interpolateCubehelixDefault = cubehelixLong(
    cubehelix$1(300, 0.5, 0.0),
    cubehelix$1(-240, 0.5, 1.0)
  );
  ...
  SizeScale.id = "size";
  SizeScale.defaults = helpers.merge({}, [
    chart_js.LinearScale.defaults,
    baseDefaults,
    scaleDefaults,
  ]);
  ...
  /**
   * A d3-geo path "context" that records the projected geometry into a Path2D
   * while simultaneously accumulating its bounding box. This lets a feature be
   * streamed through the projection ONCE instead of three times (path + bounds
   * + centroid), and lets the resulting shape be re-filled cheaply on every
   * frame without re-projecting or rasterizing into an offscreen canvas.
   */
  class GeoPathRecorder {
    constructor() {
    ...
    beginPath() {
      // d3 never calls this on a context it was handed, but keep it for parity
    }
    moveTo(x, y) {
    ...
    arc(x, y, r, startAngle, endAngle) {
      this.path.arc(x, y, r, startAngle, endAngle);
      this._add(x - r, y - r);
      this._add(x + r, y + r);
    }
    closePath() {
  ...
  class GeoFeature extends chart_js.Element {
    ...
    updateExtras({ scale, feature, center, pixelRatio, mode }) {
      ...
      const changed =
        ...
        ((_a = this.center) === null || _a === void 0 ? void 0 : _a.longitude) !==
          (center === null || center === void 0 ? void 0 : center.longitude) ||
        ((_b = this.center) === null || _b === void 0 ? void 0 : _b.latitude) !==
          (center === null || center === void 0 ? void 0 : center.latitude) ||
    ...
    inRange(mouseX, mouseY) {
      ...
      if (!r || Number.isNaN(mouseX) || Number.isNaN(mouseY)) {
        return r;
      }
      // Prefer a native hit-test against the already-projected Path2D: it is far
      // cheaper than re-walking the geometry with geoContains on every mousemove,
      // and it works for projections that have no `invert`.
      const cache = this._geometry();
      const ctx = this.projectionScale.chart && this.projectionScale.chart.ctx;
      if (cache.path2d && ctx && typeof ctx.isPointInPath === "function") {
        // isPointInPath takes the point in untransformed canvas space while the
        // path is interpreted in user space, so apply the current transform.
        let px = mouseX;
        let py = mouseY;
        if (typeof ctx.getTransform === "function") {
          const m = ctx.getTransform();
          px = m.a * mouseX + m.c * mouseY + m.e;
          py = m.b * mouseX + m.d * mouseY + m.f;
        }
        return ctx.isPointInPath(cache.path2d, px, py);
      }
    ...
    /**
     * Projects the feature once, caching both its Path2D and its bounding box.
     * The cache is keyed on the projection state only (it is dropped by
     * `updateExtras`), never on colors, so hovering or recoloring is free.
     */
    _geometry() {
      if (this.cache && this.cache.path2d !== undefined) {
        return this.cache;
      }
      let path2d = null;
      let bb;
      if (HAS_PATH2D) {
        const recorder = new GeoPathRecorder();
        this.projectionScale.geoPath.context(recorder)(this.feature);
        this.projectionScale.geoPath.context(null);
        path2d = recorder.path;
        bb = [
          [recorder.x0, recorder.y0],
          [recorder.x1, recorder.y1],
        ];
      } else {
        bb = this.projectionScale.geoPath.bounds(this.feature);
      }
      const grown = growGeoBounds(bb, this.options.borderWidth / 2);
      const bounds = {
        x: grown[0][0],
        x2: grown[1][0],
        y: grown[0][1],
        y2: grown[1][1],
        width: grown[1][0] - grown[0][0],
        height: grown[1][1] - grown[0][1],
      };
      this.cache = { ...(this.cache || {}), path2d, bounds };
      return this.cache;
    }
    getBounds() {
  ...
  class GeoController extends chart_js.DatasetController {
    ...
    resolveOutline() {
      ...
      const outline = ds.outline || SPHERE;
      if (Array.isArray(outline)) {
        // Return a stable object so identity-based caches downstream
        // (computeBounds, GeoFeature.updateExtras) actually hit.
        if (this._outlineSource !== outline) {
          this._outlineSource = outline;
          this._outlineCollection = {
