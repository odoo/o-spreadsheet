const { Spreadsheet, Model, registries, UuidGenerator, helpers, UIPlugin } = o_spreadsheet;

class DebuggerPlugin extends UIPlugin {
  handle(command) {
...
registries.functionRegistry.add("randomString", {
  ...
  compute: (args) => {
    return randomGenerator.uuidv4().substring(0, 8);
  },
  returns: "STRING",
  isExported: "false",
});
