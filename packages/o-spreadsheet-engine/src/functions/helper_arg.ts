...
import { generateMatrix, matrixForEach, matrixMap, transposeMatrix } from "./helper_matrices";
...
export class MimicMatrix {
  ...
  trueFlatten(dimension: "rowFirst" | "colFirst" = "rowFirst"): MimicMatrix {
    const m = dimension === "colFirst" ? this.transpose() : this;
    return new MimicMatrix(1, m.width * m.height, (zone) => {
      const partialHeight = zone.bottom - zone.top + 1;
      const result = [new Array(partialHeight)];

      let passedElements = 0;
      // TO DO: optimize by directly calculating the corresponding zone in the original
      // matrix instead of iterating through all rows and columns
      for (let row = 0; row < m.height; row++) {
        if (zone.top > passedElements + m.width * row) {
          continue;
        }
        if (zone.bottom < passedElements) {
          break;
        }

        const rowZone = {
          left: Math.max(0, zone.top % m.width),
          right: Math.min(m.width - 1, zone.bottom % m.width),
          top: row,
          bottom: row,
        };

        const rowResult = m.getZone(rowZone);
