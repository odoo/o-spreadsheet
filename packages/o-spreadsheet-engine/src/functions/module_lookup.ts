...
export const ROW = {
  ...
  compute: function (cellReference: Arg) {
    ...
    return new MimicMatrix(1, _cellReference.width, (col, row) => ({ value: top + row + 1 }));
  },
  isExported: true,
} satisfies AddFunctionDescription;
...
export const VLOOKUP = {
  ...
  compute: function (
    ...
    if (rowIndex < 0 || rowIndex >= _range.height) {
      return valueNotAvailable(searchKey);
    }
...
export const XLOOKUP = {
  ...
  compute: function (
    ...
    const lookupDirection = _lookupRange.height === 1 ? "col" : "row";
