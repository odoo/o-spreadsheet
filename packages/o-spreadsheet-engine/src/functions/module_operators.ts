...
/**
 * Internal formula for constructing reference ranges dynamically.
 * Technically, this formula was created to be associated with the "#" operator.
 * We chose "#" as an operator because it enables the construction of references
 * in a "dynamic" sense, rather than being tied to a static reference.
 * Thus, formulas like:
 *  =IF(condition, A1, H10)# can be imagined, instead of a simple static reference like A1#.
 **/
export const SPILLED_RANGE = {
  ...
  compute: function (ref: Matrix<{ value: string }> | undefined) {
    ...
    if (spilledRange.invalidXc || spilledRange.invalidSheetName) {
      return new InvalidReferenceError();
    }
    if (originPosition) {
      this.addDependencies?.(originPosition, [spilledRange]);
    }
    // Iterate over all positions in the spilledZone and get the evaluated cell for each
