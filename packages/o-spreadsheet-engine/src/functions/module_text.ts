...
import { matrixToMimicMatrix, toMimicMatrix } from "./helper_arg";
...
export const REGEXEXTRACT = {
  ...
  compute: function (
    ...
    if (_returnMode === 0) {
      ...
      return matrixToMimicMatrix(matches.map((match) => [match[0]]));
    } else {
      if (matches[0].length < 2) {
        return new EvaluationError(_t("No capturing groups found."));
      }
      return matrixToMimicMatrix(matches[0].slice(1).map((s) => [s]));
    }
  },
  isExported: true,
} satisfies AddFunctionDescription;
...
export const SPLIT = {
  ...
  compute: function (
    ...
    return matrixToMimicMatrix([result]).transpose();
  },
  isExported: false,
} satisfies AddFunctionDescription;
...
export const TEXTSPLIT = {
  ...
  compute: function (
    ...
    const _colDelimiters = toMimicMatrix(colDelimiter).flatten("rowFirst", (v) =>
      escapeRegExp(toString(v))
    );
    const _rowDelimiters = toMimicMatrix(rowDelimiter).flatten("rowFirst", (v) =>
      escapeRegExp(toString(v))
    );
    ...
    const cells: Matrix<FunctionResultObject> = [];
    ...
    return matrixToMimicMatrix(cells).transpose();
  },
  isExported: true,
} satisfies AddFunctionDescription;
