...
export function unHighlightBarChartItems(dataSets: ChartDataset<"bar">[]) {
...
export function unHighlightComboChartItems(dataSets: ChartDataset<"bar" | "line">[]) {
  for (const dataset of dataSets) {
...
export function highlightLineChartItem(
  ...
  for (let i = 0; i < dataSets.length; i++) {
    ...
    dataset.backgroundColor = setColorAlpha(
      ...
      LINE_FILL_TRANSPARENCY * (i === index ? 1 : 0.4)
    );
  }
}

export function unHighlightLineChartItems(dataSets: ChartDataset<"line" | "radar">[]) {
  for (const dataset of dataSets) {
    const color = setColorAlpha(dataset.borderColor as Color, 1);
    dataset.borderColor = color;
    dataset.pointBackgroundColor = color;
...
export function highlightPieChartItem(item: LegendItem, dataSets: ChartDataset<"pie">[]) {
  for (const dataset of dataSets) {
    ...
    backgroundColors.forEach((color, i, colors) => {
      colors[i] = setColorAlpha(color, i === item.index ? 1 : 0.4);
    });
  }
}

export function unHighlightPieChartItems(dataSets: ChartDataset<"pie">[]) {
  for (const dataset of dataSets) {
    const backgroundColors = dataset.backgroundColor as Color[] | undefined;
    if (!backgroundColors) {
      return;
