...
import { ApplyRangeChange, CellPosition, UID, UnboundedZone, Zone } from "../../types/misc";
...
export class CoreFormatPlugin extends CorePlugin<FormatPluginState> implements FormatPluginState {
  ...
  handle(cmd: CoreCommand) {
    switch (cmd.type) {
      ...
      case "DUPLICATE_SHEET":
        this.history.update("formats", cmd.sheetIdTo, this.formats[cmd.sheetId]);
        break;
    }
  }
  ...
  private handleAddColRow(cmd: AddColumnsRowsCommand) {
    ...
    const sheetValues = this.formats[sheetId] ?? [];
    for (let idx = 0; idx < sheetValues.length; idx++) {
      ...
      if (value.zone[start] - cmd.quantity === cmd.base && cmd.position === "before") {
        ...
        this.history.update("formats", sheetId, idx, "zone", end, value.zone[end] + cmd.quantity);
      }
    }
  }
  ...
  getCellFormatInRanges(
    ...
    for (const sheetId in zonesBySheet) {
      ...
      for (const zone of zones) {
        for (const { zone: z, format } of this.formats[sheetId] ?? []) {
          const inter = intersection(z, zone);
          if (inter) startingMap.setMany(sheetId, inter, format);
        }
      }
    }
    return startingMap;
