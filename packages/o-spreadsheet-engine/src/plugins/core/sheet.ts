...
import { getDuplicateSheetName, getNextSheetName, isSheetNameEqual } from "../../helpers/sheet";
import { getZoneArea, isBound, isZoneInside, isZoneValid } from "../../helpers/zones";
...
export class SheetPlugin extends CorePlugin<SheetState> implements SheetState {
  ...
  getCellsFromZone(
    ...
    if (isBound(zone) && getZoneArea(zone) < 1000) {
      ...
      const [rowStart, rowAdd] = (options.rowsOrder ?? 1) > 0 ? [zone.top, 1] : [zone.bottom, -1];
    ...
    const rowOrder = options.rowsOrder;
    const colOrder = options.colOrder;
    if (rowOrder) {
      rows.sort((a, b) => (b[0] > a[0] ? rowOrder : -rowOrder));
    }
    for (const [rowIndex, row] of rows) {
      if (!row || !this.indexInRange(rowIndex, zone.top, zone.bottom)) continue;
      const cols: [number, UID | undefined][] = Object.entries(row.cells).map((x) => [
        parseInt(x[0]),
        x[1],
      ]);
      if (colOrder) {
        cols.sort((a, b) => (b[0] > a[0] ? colOrder : -colOrder));
      }
      for (const [colIndex, cellId] of cols) {
        if (!cellId || !this.indexInRange(colIndex, zone.left, zone.right)) continue;
  ...
  private removeRows(sheet: Sheet, rows: HeaderIndex[]) {
    ...
    this.history.update("sheets", sheet.id, "numberOfRows", numberOfRows - rows.length);
  ...
  private addColumns(
    ...
    this.shiftZone(
      sheet.id,
      {
        left: position === "after" ? column + 1 : column,
        right: undefined,
        top: 0,
        bottom: undefined,
      },
