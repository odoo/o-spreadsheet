...
class CompilationParametersBuilder {
  ...
  private matrixCache: Map<string, Matrix<FunctionResultObject>> = new Map();
  ...
  private range(range: Range): MimicMatrix {
    ...
    return new MimicMatrix(width, height, (zone: Zone) => {
      ...
      if (realLeft === realRight && realTop === realBottom) {
        const position = { sheetId: range.sheetId, col: realLeft, row: realTop };
        const result = this.computeCell(position);
        if (!result.position) {
          return [[{ ...result, position }]];
        }
