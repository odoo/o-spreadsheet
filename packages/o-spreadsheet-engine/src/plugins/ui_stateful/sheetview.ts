    ...
    switch (cmd.type) {
      case "SET_VIEWPORT_OFFSET": {
        ...
        return this.chainValidations(
          () => this.viewports.checkScrollingDirection(sheetId, cmd),
          () => this.viewports.checkIfViewportsWillChange(sheetId, cmd)
        )(cmd);
      }
      case "RESIZE_SHEETVIEW":
        return this.chainValidations(
          () => this.viewports.checkValuesAreDifferent(cmd),
  ...
  handle(cmd: Command) {
    ...
    switch (cmd.type) {
      ...
      case "SET_VIEWPORT_OFFSET":
        ...
        break;
      case "SET_ZOOM":
        this.viewports.setZoomLevel(cmd.zoom);
        break;
