...
def has_direct_await(src: str, body_start: int, body_end: int) -> bool:
    ...
    while pos < body_end:
        ...
        if ch == "=":
            ...
            if m:
                brace_pos = m.end() - 1  # the `{` is the last char of the match
...
_TEST_FUNC_RE = re.compile(
    r"\b(test|it|beforeEach|beforeAll|afterEach|afterAll|describe)"
    r"(?:\.(?:each|skip|only|failing|concurrent))?"
    r"\s*(?:\([^)]*\)\s*)?"  # optional .each([...]) table argument
    r"\s*\("
)
...
def find_patches(src: str) -> list[tuple[int, int, str]]:
    ...
    while pos < len(src):
        ...
        # The opening '(' of the argument list is at m.end() - 1
        call_start = m.end() - 1  # position of '('
        ...
        while scan < len(src) and paren_depth > 0:
            ...
            if paren_depth == 1:
                ...
                if m_arrow:
                    ...
                    body_start, body_end, after = extract_brace_body(src, brace_pos)
