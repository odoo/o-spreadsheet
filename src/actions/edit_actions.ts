...
export const mergeCells: ActionSpec = {
  ...
  isActive: (env) => isUnmergeAction(env),
...
function isUnmergeAction(env: SpreadsheetChildEnv): boolean {
  if (cannotMerge(env)) {
    return false;
  }

  const firstZone = env.model.getters.getSelectedZones()[0];
  const sheetId = env.model.getters.getActiveSheetId();
  const mergesInFirstZone = env.model.getters.getMergesInZone(sheetId, firstZone);
  return mergesInFirstZone.length > 0;
}
