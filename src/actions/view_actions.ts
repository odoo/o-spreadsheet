...
export const unhideCols: ActionSpec = {
  ...
  isVisible: (env: SpreadsheetActionEnv) => {
    const hiddenCols = env
      .model()
      .getters.getHiddenColsGroups(env.model().getters.getActiveSheetId())
      .flat();
...
export const unhideRows: ActionSpec = {
  ...
  isVisible: (env: SpreadsheetActionEnv) => {
    const hiddenRows = env
      .model()
      .getters.getHiddenRowsGroups(env.model().getters.getActiveSheetId())
      .flat();
...
export const unFreezePane: ActionSpec = {
  name: _t("Unfreeze"),
  isVisible: (env) => {
    const { xSplit, ySplit } = env
      .model()
      .getters.getPaneDivisions(env.model().getters.getActiveSheetId());
...
export const viewFormulas: ActionSpec = {
  ...
  execute: (env) =>
    env
      .model()
      .dispatch("SET_FORMULA_VISIBILITY", { show: !env.model().getters.shouldShowFormulas() }),
  isReadonlyAllowed: true,
  isEnabledOnLockedSheet: true,
};
