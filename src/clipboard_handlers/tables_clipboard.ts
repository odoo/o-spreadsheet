...
> {
  ...
  protected compact(array2D: ClipboardTableCell[][]): CompactTableHandlerData {
    ...
    const compactBorder = (border: Border): CompactBorderCell => {
    ...
    const styleIndex = (style: ClipboardTableStyle): number => {
      ...
      if (style.border) {
        compactStyle.border = compactBorder(style.border);
      }
  ...
  expand(data: unknown): ClipboardTableCell[][] {
    ...
    const { rows, cols, tables, styleTable, items } = tableData;
    ...
    for (const { r, c, v } of items) {
      expanded[r][c] = {
        ...
        ...(v.styleIdx !== undefined ? { style: expandStyle(styleTable[v.styleIdx]) } : {}),
