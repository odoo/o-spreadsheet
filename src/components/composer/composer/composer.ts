...
export class Composer extends OSComponent {
  ...
  onMousedown(ev: MouseEvent) {
    ...
    if (this.mobilePlugin.isMobile() && !isIOS()) {
      return;
    }
    this.debouncedHover.stopDebounce();
  ...
  autoComplete(proposal: AutoCompleteProposal) {
    if (!proposal?.text || (this.assistant.forcedClosed && this.props.composerStore.canBeToggled)) {
      return;
    }
