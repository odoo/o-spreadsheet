import { EnrichedToken, composerTokenize } from "../../../formulas";
...
export class ComposerStore extends SpreadsheetStore {
  ...
  handle(cmd: Command) {
    switch (cmd.type) {
      ...
      case "START_CHANGE_HIGHLIGHT":
        ...
        this.model.selection.resetAnchor(this, { cell: { col: left, row: top }, zone: cmd.zone });
        break;
      case "ACTIVATE_SHEET":
  ...
  private cycleReferences() {
    ...
    this.model.dispatch("SET_CURRENT_CONTENT", {
  ...
  private onColumnsRemoved(cmd: RemoveColumnsRowsCommand) {
    if (cmd.elements.includes(this.col) && this.mode !== "inactive") {
      this.cancelEdition(); //should do the equivalent of dispatch
      console.log("FAUT RAISE UI");
      //   this.ui.raiseBlockingErrorUI(CELL_DELETED_MESSAGE);
      return;
    }
  ...
  private startComposerRangeSelection() {
    if (this.sheetId === this.getters.getActiveSheetId()) {
      ...
      this.model.selection.resetAnchor(this, { cell: { col: this.col, row: this.row }, zone });
    }
    this.mode = "selecting";
  }
  ...
  private setContent(text: string, selection?: ComposerSelection, raise?: boolean) {
    ...
    if (isNewCurrentContent || this.mode !== "inactive") {
      ...
      if (this.currentTokens.length > 100) {
        if (raise) {
          console.log("FAUT RAISE UI", CELL_DELETED_MESSAGE);
  ...
  private insertText(text: string, start: number) {
    ...
    this.setContent(content, { start: end, end }, true);
    this.updateRangeColor();
  }
  ...
  getHighlights(): Highlight[] {
    ...
    return this.getReferencedRanges().map((range) => {
      const rangeString = this.model.getters.getRangeString(range, editionSheetId);
      return {
        zone: range.zone,
  ...
  getReferencedRanges(): Range[] {
    ...
    return this.currentTokens
      .filter((token) => token.type === "REFERENCE")
      .map((token) => this.model.getters.getRangeFromSheetXC(editionSheetId, token.value));
  }
