...
export const chartShowValuesPlugin: Plugin = {
  ...
  afterDatasetsDraw(chart: any, args, options: ChartShowValuesPluginOptions) {
    ...
    switch (options.type) {
      case "pie":
        drawPieChartValues(chart, options);
        break;
      case "line":
      case "scatter":
      case "combo":
      ...
      case "bar":
        options.horizontal
          ? drawHorizontalBarChartValues(chart, options)
          : drawLineOrBarOrRadarChartValues(chart, options);
        break;
      case "pyramid":
        drawHorizontalBarChartValues(chart, options);
      ...
      case "calendar":
        ...
        break;
      case "bubble":
        drawBubbleChartValues(chart, options);
        break;
      case "funnel":
        drawHorizontalBarChartValues(chart, options);
        break;
...
function drawValues(args: {
  ...
  for (const dataset of chart._metasets) {
    ...
    for (let i = 0; i < dataset._parsed.length; i++) {
      ...
      drawTextWithBackground(valueToDisplay, position.x, position.y, ctx, !!strokeColor);
    }
  }
...
function drawLineOrBarOrRadarChartValues(chart: any, options: ChartShowValuesPluginOptions) {
  drawValues({
    ...
    getValuePosition: ({ chartElement, numberValue: value, textSize, dataset }) => {
      let yPosition = 0;
      if (chart.config.type === "line" || chart.config.type === "radar") {
        yPosition = value < 0 ? chartElement.y + 10 : chartElement.y - 10;
    ...
    getTextColors: ({ chartElement, dataset, numberValue: value, valueIndex }) => ({
      textColor: chartElement.options.backgroundColor,
      strokeColor: options.background(Number(value), dataset, valueIndex) || "#ffffff",
    }),
  });
}
...
function drawCalendarValues(chart: any, options: ChartShowValuesPluginOptions) {
  drawValues({
    ...
    getTextColors: ({ chartElement, dataset, numberValue: value, valueIndex }) => ({
      ...
      textColor: options.background(Number(value), dataset, valueIndex) || "#ffffff",
    }),
  });
}

function drawBubbleChartValues(chart: any, options: ChartShowValuesPluginOptions) {
  drawValues({
    chart,
    options,
    direction: "vertical",
    getNumberValue: (dataset, i) => dataset._parsed[i].y,
    getValuePosition: ({ chartElement, numberValue: value }) => ({
      x: chartElement.x,
      y: chartElement.y,
    }),
...
function drawHorizontalBarChartValues(chart: any, options: ChartShowValuesPluginOptions) {
  drawValues({
    chart,
    options,
...
function drawPieChartValues(chart: any, options: ChartShowValuesPluginOptions) {
  ...
  drawValues({
    ...
    getTextColors: ({ dataset, numberValue: value, valueIndex }) => {
      const background = options.background(Number(value), dataset, valueIndex) || "#ffffff";
