...
export class ZoomableChartStore extends SpreadsheetStore {
  ...
  idConversion: Record<UID, UID> = {};

  handle(cmd: Command) {
    switch (cmd.type) {
      case "DELETE_FIGURE":
        if (cmd.figureId) {
          const chartId = this.idConversion[cmd.figureId];
      ...
      case "UPDATE_CHART":
        ...
        this.idConversion[cmd.figureId] = cmd.figureId;
  ...
  /* Update the trend line configuration based on the current axis limits.
