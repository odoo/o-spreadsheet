...
export class ZoomableChartJsComponent extends ChartJsComponent {
  ...
  get containerStyle() {
    const height = this.sliceable ? `calc(100% - ${SLIDER_HEIGHT}px)` : "100%";
    return `
      height:${height};
    `;
  }
  ...
  get upperBound(): number | undefined {
    const value = this.store.currentAxisLimits[this.props.figureUI.id]?.max;
    if (value === undefined || !this.slider?.scales?.x) {
      return this.slider?.chartArea.right;
    }
  ...
  get lowerBound(): number | undefined {
    const value = this.store.currentAxisLimits[this.props.figureUI.id]?.min;
    if (value === undefined || !this.slider?.scales?.x) {
      return this.slider?.chartArea.left;
    }
    const scale = this.slider.scales.x;
    if (this.hasLinearScale) {
      return scale.getPixelForValue(value);
