...
export class ZoomableChartJsComponent extends ChartJsComponent {
  ...
  get upperBound(): number | undefined {
    return this.computePosition(this.store.currentAxisLimits[this.props.figureUI.id]?.max);
  }

  get lowerBound(): number | undefined {
    return this.computePosition(this.store.currentAxisLimits[this.props.figureUI.id]?.min);
  }
