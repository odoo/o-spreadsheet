...
export const testPlugin: Plugin = {
  ...
  afterDatasetsDraw: function (chart, args, options: TestPluginOptions) {
    ...
    if (eventStartX !== undefined && eventEndX !== undefined) {
      ...
      ctx.rect(xMin, yBottom, xMax - xMin, yTop - yBottom);
      ctx.stroke();
    }
  },
};
...
export class ZoomableChartJsComponent extends Component<Props, SpreadsheetChildEnv> {
  ...
  onPointerDownInMasterChart(ev: PointerEvent) {
    ...
    if (this.eventStartX !== undefined && Math.abs(startX - this.eventStartX) < 5) {
      this.eventStartX = this.eventEndX;
    } else if (this.eventEndX === undefined || Math.abs(startX - this.eventEndX) > 5) {
      this.eventStartX = this.clipXCoordinate(startX);
    }
    this.eventEndX = undefined;
  ...
  onPointerMoveInMasterChart(ev: PointerEvent) {
    if (this.clicked && Math.abs(this.eventStartX! - ev.offsetX) > 5) {
      this.eventEndX = this.clipXCoordinate(ev.offsetX);
      this.slider!.update();
      ev.stopPropagation();
      ev.preventDefault();
    }
  ...
  onPointerMoveInDetailChart(ev: PointerEvent) {
    //TODO: Utiliser les positions initiales des axes au moment où on commence le pan
    if (this.panStartX !== undefined && Math.abs(this.panStartX - ev.offsetX) > 5) {
      ...
      this.updateTrendLineAxis();
      this.chart!.update();
      this.eventStartX = this.getPositionOnChart(newXMin);
      this.eventEndX = this.getPositionOnChart(newXMax);
      this.slider!.update();
    }
  }
