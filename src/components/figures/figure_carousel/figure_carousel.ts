...
import { getChartRuntimeBackground } from "../../../helpers/figures/charts/chart_ui_common";
...
export class CarouselFigure extends Component<SpreadsheetChildEnv> {
  ...
  private get selectedChartBackground(): Color | undefined {
    ...
    const runtime = this.env.model.getters.getChartRuntime(this.selectedCarouselItem.chartId);
    return getChartRuntimeBackground(runtime);
  }
  ...
  openContextMenu(event: MenuMouseEvent) {
    if (event.closedMenuId === "figure-context-menu") {
      return;
    }
  ...
  removeEmptyHeadersFromRange(range: Range): Range {
    if (range.invalidXc || range.invalidSheetName) {
      const id = this.props.figureUI.id;
      throw new Error(
        `Invalid range provided to carousel ${id} data view ${JSON.stringify(this.carousel)}`
      );
    }
