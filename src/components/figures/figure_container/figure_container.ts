...
export class FiguresContainer extends Component<Props, SpreadsheetChildEnv> {
  ...
  setup() {
    ...
    onWillUpdateProps(() => {
      ...
      const draggedFigureId = this.dnd.draggedFigure?.figure.id;
  ...
  private getVisibleFigures(): FigureUI[] {
    ...
    if (
      ...
      const draggedFigure = this.env.model.getters.getFigure(
        sheetId,
        this.dnd.draggedFigure?.figure.id
      );
      if (draggedFigure) {
        visibleFigures.push(this.env.model.getters.getFigureUI(sheetId, draggedFigure));
      }
    }
    ...
    const figureVisibleRects = snapLine.matchedFigIds
      ...
      .map((figureUI) => {
        return {
          ...
          width: figureUI.figure.width,
          height: figureUI.figure.height,
        };
      })
      .filter(isDefined);
