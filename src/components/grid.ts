...
import { Client, Offsets, SpreadsheetEnv, Viewport } from "../types/index";
...
export class Grid extends Component<{ model: Model }, SpreadsheetEnv> {
  ...
  private moveCanvas(ev: Offsets) {
    this.hScrollbar.el.scrollTo(ev.offsetX, 0);
    this.vScrollbar.el.scrollTo(0, ev.offsetY);
  }
  ...
  onMouseDown(ev: MouseEvent) {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      if (isEdgeScrolling) {
        ...
        this.env.modelBusTrigger("tabouret", { offsetX, offsetY });
  ...
  processArrows(ev: KeyboardEvent) {
    ...
    if (ev.shiftKey) {
      ...
      if (newOffsetX !== offsetX || newOffsetY !== offsetY) {
        this.env.modelBusTrigger("tabouret", { offsetX: newOffsetX, offsetY: newOffsetY });
