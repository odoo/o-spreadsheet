...
export class Grid extends Component<Props, SpreadsheetChildEnv> {
  ...
  async copy(cut: boolean, ev: ClipboardEvent) {
    ...
    if (this.env.clipboard) {
       ...
       * navigator.clipboard (case for old browsers),
       * we store the data in the ClipboardEvent
       * instance instead.
       */
    ...
    window.localStorage.setItem("clipboardId", this.env.model.getters.getClipboardId());
    ev.preventDefault();
  }
  ...
  async paste(ev: ClipboardEvent) {
    ...
    if (this.env.clipboard) {
      ...
      osClipboardSpreadsheetContent = clipboardData.getData(ClipboardMIMEType.OSpreadsheet);
      clipboardContent = {
        ...
        [ClipboardMIMEType.OSpreadsheet]: osClipboardSpreadsheetContent,
      };
    }
    ...
    // if (!clipboardId) {
    //   clipboardId = window.localStorage.getItem("clipboardId");
    // }
