...
export class GridOverlay extends Component<Props, SpreadsheetChildEnv> {
  ...
  get icons() {
    ...
    for (const position of this.env.model.getters.getVisibleCellPositions()) {
      ...
      const iconsAtPostition = cellIcons.filter((icon) => icon.component);

      for (const icon of iconsAtPostition) {
        const merge = this.env.model.getters.getMerge(icon.position);
        const zone = merge || positionToZone(icon.position);
        const cellRect = this.env.model.getters.getVisibleRectWithoutHeaders(zone);
        const cell = this.env.model.getters.getCell(icon.position);
