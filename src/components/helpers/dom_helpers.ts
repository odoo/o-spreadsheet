...
export function gridOverlayPosition(zoom = 1): DOMCoordinates {
  // FIXME: since we have multiple grid overlays, this whole `gridOverlayPosition` should be replaced
...
export function zoomCorrectedElementRect(el: Element, zoomLevel: number): Rect {
  ...
  if (zoomedElement) {
    ...
    // FIXME
    // Some browsers, like Safari, mess up the computation of getBoundingClientRect on elements subjected to a zoom CSS property.
