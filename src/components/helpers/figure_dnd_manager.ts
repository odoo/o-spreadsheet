...
import { Figure, Pixel, UID } from "../../types";
...
interface VerticalSnapLine {
  border: VerticalBorderName;
  matchedFig: FigurePosition;
  matchedBorder: VerticalBorderName;
  x: Pixel;
}
...
interface HorizontalSnapLine {
  border: HorizontalBorderName;
  matchedFig: FigurePosition;
  matchedBorder: HorizontalBorderName;
  y: Pixel;
}
...
export abstract class FigureDndManager {
  ...
  constructor(
    ...
    protected readonly initialMouseX: Pixel,
    protected readonly initialMouseY: Pixel
  ) {
            ...
            if (!match || offset < match.offset) {
              match = {
                match: {
                  ...
                  matchedBorder: matchedBorder.border,
                  matchedFig,
...
export class FigureDnDMoveManager extends FigureDndManager {
  drag(mouseX: Pixel, mouseY: Pixel) {
    const x = Math.max(this.initialFigure.x - (this.initialMouseX - mouseX), 0);
    const y = Math.max(this.initialFigure.y - (this.initialMouseY - mouseY), 0);
...
export class FigureDnDResizeManager extends FigureDndManager {
  resize(dirX: -1 | 0 | 1, dirY: -1 | 0 | 1, mouseX: Pixel, mouseY: Pixel) {
    this.currentHorizontalSnapLine = undefined;
    this.currentVerticalSnapLine = undefined;

    const deltaX = dirX * (mouseX - this.initialMouseX);
    const deltaY = dirY * (mouseY - this.initialMouseY);
