...
export function moveAnchorWithinSelection(
  ...
  const maxChecks = getZoneArea(zone);
  for (let i = 0; i < maxChecks; i++) {
    currentCell = getNextCellInZone(currentCell, zone, direction);
    if (!isNavigablePosition(getters, currentCell)) {
      continue;
    }
    selection.updateAnchorCell(currentCell.col, currentCell.row, { scrollIntoView: true });
    return;
  }
}
