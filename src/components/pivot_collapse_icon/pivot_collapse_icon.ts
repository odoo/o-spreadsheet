...
export class PivotCollapseIcon extends Component<Props, SpreadsheetChildEnv> {
  ...
  onClick() {
    ...
    const collapsedDomains = definition.collapsedDomains ? [...definition.collapsedDomains] : [];
    ...
    this.env.model.dispatch("UPDATE_PIVOT", {
      pivotId,
      pivot: { ...definition, collapsedDomains },
    });
    console.log("PivotCollapseIcon.onClick", pivotCell);
  }
  ...
  get isCollapsed() {
    ...
    const definition = this.env.model.getters.getPivotCoreDefinition(pivotId);
    return definition.collapsedDomains?.some((domain) => deepEquals(domain, pivotCell.domain));
  }
}
