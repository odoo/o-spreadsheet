...
export class Popover extends Component<PopoverProps, SpreadsheetChildEnv> {
  ...
  private computePopoverPosition() {
    ...
    el.style["max-height"] = popoverPositionHelper.getMaxHeight(childDims.height) + "px";
    el.style["max-width"] = popoverPositionHelper.getMaxWidth(childDims.width) + "px";
    // Compute the popover dimensions after setting the max-width and max-height
    const popoverDims = {
      width: el.getBoundingClientRect().width,
      height: el.getBoundingClientRect().height,
    };

    let style = popoverPositionHelper.getCss(popoverDims, this.props.verticalOffset);
    for (const property of Object.keys(style)) {
      el.style[property] = style[property];
    }

    const newPosition = popoverPositionHelper.getCurrentPosition(popoverDims);
