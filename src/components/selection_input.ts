...
export class SelectionInput extends Component<Props, SpreadsheetEnv> {
  ...
  get ranges(): SelectionRange[] {
    ...
    const validRanges: SelectionRange[] = [];
    ranges.forEach((range) => {
      const isValidRange = this.getters.isRangeValid(range.xc);
      const newRange = Object.assign({}, range, { isValidRange });
      validRanges.push(newRange);
    });
    return validRanges.length ? validRanges : [];
  }
