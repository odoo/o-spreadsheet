...
export class GeneralDesignEditor extends Component<Props, SpreadsheetChildEnv> {
  ...
  updateChartPositioning(index: number, value: string) {
    if (value === "fixed") {
      this.props.updateChart(this.props.figureId, { fixed_position: true });
    } else if (value === "anchor") {
      this.props.updateChart(this.props.figureId, { fixed_position: false });
    }
  }

  getSelectedPositioning(index: number) {
    return "a";
  }
}
