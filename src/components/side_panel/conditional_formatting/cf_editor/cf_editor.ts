...
import {
  cfOperatorToCriterionType,
  criterionTypeToCfOperator,
} from "../../../../helpers/conditional_format";
...
} from "../../data_validation/data_validation_panel_helper";
...
export class ConditionalFormattingEditor extends Component<Props, SpreadsheetChildEnv> {
  ...
  get cfCriterionMenuItems(): Action[] {
    return getCriterionMenuItems(
      (type) => this.editOperator(criterionTypeToCfOperator(type)),
      availableConditionalFormatOperators
    );
  }

  get selectedCriterionName(): string {
    const selectedType = cfOperatorToCriterionType(this.state.rules.cellIs.operator);
    return dataValidationEvaluatorRegistry.get(selectedType).name;
  }

  get criterionComponent(): ComponentConstructor | undefined {
    const selectedType = cfOperatorToCriterionType(this.state.rules.cellIs.operator);
    return dataValidationPanelCriteriaRegistry.get(selectedType).component;
  }

  get genericCriterion(): GenericCriterion {
    return {
      type: cfOperatorToCriterionType(this.state.rules.cellIs.operator),
