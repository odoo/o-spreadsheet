...
export class ConditionalFormattingEditorStore extends SpreadsheetStore {
  ...
  constructor(get: Get, cf: ConditionalFormat, isNew: boolean) {
    super(get);
  ...
  updateConditionalFormat(newCf: Partial<ConditionalFormat> & { suppressErrors?: boolean }) {
    ...
    if (invalidRanges) {
      ...
      this.cancelledReasons = [CommandResult.InvalidRange];
      return;
    }
    ...
    this.cancelledReasons = result.reasons.filter((r) => r !== CommandResult.NoChanges);
    if (!newCf.suppressErrors) {
      this.state.errors = this.cancelledReasons;
    }
  }
