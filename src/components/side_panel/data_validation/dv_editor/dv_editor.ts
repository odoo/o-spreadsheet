...
export class DataValidationEditor extends Component<Props, SpreadsheetChildEnv> {
  ...
  static props = { id: String, onCloseSidePanel: Function };
  ...
  setup() {
    ...
    const rule = this.env.model.getters.getDataValidationRule(this.editingSheetId, this.props.id);
    if (rule) {
  ...
  onCancel() {
    this.env.replaceSidePanel("DataValidation", `DataValidationEditor_${this.props.id}`);
  }
