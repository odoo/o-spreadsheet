import { _t, isDefined } from "@odoo/o-spreadsheet-engine";
...
export class MoreFormatsStore extends SpreadsheetStore {
  ...
  category: CustomFormatCategory = this.detectFormatCategoryInSelection();
  invalidFormat = false;
  currentFormat = this.getFormatInSelection();
  ...
  handleSelectionEvent() {
    if (this.getters.isGridSelectionActive()) {
      const selectedFormat = this.getFormatInSelection();
      if (selectedFormat !== this.lastFormatInSelection) {
        ...
        this.category = this.detectFormatCategoryInSelection();
      }
    }
  }
  ...
  private detectFormatCategoryInSelection(): CustomFormatCategory {
    const format = this.getFormatInSelection();
    if (!format) {
      return "number";
    }
  ...
  get formatExamples() {
    ...
    if (this.category === "date") {
      ...
      if (numberOfParts === 1) {
        return [{ label: _t("Sample:"), value: formatValue(1234.56, { format, locale }) }];
      }
      const parts = [
  ...
  changeCategory(category: CustomFormatCategory) {
    ...
    this.category = category;
    this.setFormat(this.formatProposals[0]?.format || "");
    return;
  }
