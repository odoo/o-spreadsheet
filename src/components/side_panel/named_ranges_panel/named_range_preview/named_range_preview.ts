...
export class NamedRangePreview extends Component<Props, SpreadsheetChildEnv> {
  ...
  get highlights(): Highlight[] {
    ...
    return [
      {
        range: this.props.namedRange.range,
  ...
  getRangeString() {
    return this.env.model.getters.getRangeString(this.props.namedRange.range);
  }

  deleteNamedRange() {
    this.env.model.dispatch("DELETE_NAMED_RANGE", {
      rangeName: this.props.namedRange.rangeName,
    });
  }
  ...
  updateNamedRangeName(newName: string) {
    interactiveUpdateNamedRange(this.env, {
      ...
      zone: this.props.namedRange.range.zone,
    });
  }

  get selectionInputProps(): SelectionInput["props"] {
    return {
      ranges: [
      ...
      onSelectionConfirmed: () => {
        ...
        if (this.state.currentRange) {
          ...
          interactiveUpdateNamedRange(this.env, {
            ...
            oldRangeName: this.props.namedRange.rangeName,
            newRangeName: this.props.namedRange.rangeName,
            zone: range.zone,
          });
        }
      },
