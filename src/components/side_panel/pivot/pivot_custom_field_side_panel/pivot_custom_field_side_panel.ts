...
export class PivotCustomFieldPanel extends Component<Props, SpreadsheetChildEnv> {
  ...
  private defaultGroupNameRegex = new RegExp(`^${_t("Group")}[0-9]*$`);
  ...
  onUpdateGroupValues(groupIndex: number, values: ValueAndLabel<CellValue>[]) {
    ...
    if (this.doesGroupHaveDefaultName(group)) {
      newGroup.name =
        ...
        getUniquePivotGroupName(_t("Group"), this.state.customField);
    }
  ...
  private doesGroupHaveDefaultName(group: PivotCustomGroup): boolean {
    return (
      group.name === createPivotGroupName(group.values) ||
      this.defaultGroupNameRegex.test(group.name)
    );
  }
