...
export class SpreadsheetPrint extends Component<Props, SpreadsheetChildEnv> {
  ...
  setup() {
    useExternalListener(window, "beforeprint", () => {
      ...
      style.textContent = `@media print { @page { size: ${size}; margin: ${PRINT_MARGIN}px;}}`;
  ...
  get printSelectionOptions(): ValueAndLabel[] {
    ...
    if (selection.length === 1) {
      const sheetId = this.env.model.getters.getActiveSheetId();
      const selectionRange = this.env.model.getters.getRangeFromZone(sheetId, selection[0]);
      const rangeString = this.env.model.getters.getRangeString(selectionRange, sheetId);
  ...
  private getLastUsedHeaders(sheetId: UID): { lastUsedCol: number; lastUsedRow: number } {
    ...
    for (const cell of coreCells) {
      ...
      const spreadZone = this.env.model.getters.getSpreadZone(position);
