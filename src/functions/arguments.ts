 ...
 * Therefore, it becomes more complex. It is the reason why the corresponding argument index depends
 * not only on the position of the value in the function but also on the total number of values passed to the function.
 *
 * The next function has been designed to handle all possible configurations,
 * it has the advantage to not functionally restrict the way the arguments are defined.
...
export function _argTargeting(
  ...
  for (let i = 0; i < functionDescription.args.length; i++) {
    ...
    if (arg.optional || arg.default) {
      if (countValueOptional < nbrValueOptional) {
        valueIndexToArgPosition.set(countValueSupplied, i);
        countValueSupplied++;
      }
      countValueOptional++;
      continue;
    ...
    if (arg.repeating) {
      ...
      for (let j = 0; j < groupsOfRepeatingValues; j++) {
        for (let k = 0; k < functionDescription.nbrArgRepeating; k++) {
          valueIndexToArgPosition.set(countValueSupplied, i + k);
          countValueSupplied++;
        }
      }
  ...
  return (argPosition: number) => {
    return valueIndexToArgPosition.get(argPosition);
  };
}
