import { Arg, Data, isMatrix, Matrix } from "../types";
...
export function assertSquareMatrix(errorStr: string, arg: Matrix<Data>) {
  assert(() => arg.length === arg[0].length, errorStr);
}

export function isNumberMatrix(arg: Matrix<Data>): boolean {
  return arg.every((row) => row.every((val) => typeof val.value === "number"));
}
