type unitTransformations = {
  transform: (number) => number;
  inverseTransform: (number) => number;
};

type unit = unitTransformations & { category: string; factor: number; order?: number };

const transformFromFactor = (factor: number): unitTransformations => ({
