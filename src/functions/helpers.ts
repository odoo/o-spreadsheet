  ...
  const fillArgsBuffer = (i: number, j: number): void => {
    for (let k = 0; k < args.length; k++) {
      ...
      switch (vectorArgsType?.[k]) {
        case "matrix":
          argsBuffer[k] = (arg as Matrix<FunctionResultObject>)[i][j];
          break;
        case "horizontal":
          argsBuffer[k] = (arg as Matrix<FunctionResultObject>)[i][0];
          break;
        case "vertical":
          argsBuffer[k] = (arg as Matrix<FunctionResultObject>)[0][j];
          break;
        case undefined:
          argsBuffer[k] = arg;
          break;
