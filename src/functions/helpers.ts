...
export function applyVectorization(
  ...
  // Reused across every vectorized cell — avoids allocating a new args array per call.
  const argsBuffer: Arg[] = new Array(args.length);

  // Resolve each arg's access pattern once, outside the inner loop. Each getter
  // closes over its own `arg` and knows how to read it for given (col, row).
  ...
  for (let k = 0; k < args.length; k++) {
    ...
    switch (vectorArgsType?.[k]) {
      case "matrix": {
        const m = arg as Matrix<FunctionResultObject>;
        argGetters[k] = (i, j) => m[i][j];
        break;
      }
      case "horizontal": {
        const m = arg as Matrix<FunctionResultObject>;
        argGetters[k] = (i) => m[i][0];
        break;
      }
      case "vertical": {
        const m = arg as Matrix<FunctionResultObject>;
        argGetters[k] = (_i, j) => m[0][j];
        break;
      }
      case undefined:
        argGetters[k] = () => arg;
