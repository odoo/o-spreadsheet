...
export const CELL = {
  ...
  compute: function (info: Maybe<FunctionResultObject>, reference: Matrix<{ value: string }>) {
    // "reference" is defined as meta, so technically we are sure that all value that is not a reference doesn't pass. (throw BAD_EXPR in "compileAST")
    // Also, as the arg definition of "reference" contain "range<...>" and as reference with range are always converted as a matrix (see hasRange in "compileFunctionArgs" and see "range" function in compilation_parameters)
    // --> reference corresponds always to a matrix, even if we pass a single cell reference.
