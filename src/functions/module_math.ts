...
export const SUBTOTAL = {
  ...
  compute: function (
    ...
    for (const ref of refs) {
      ...
      const hiddenRows = acceptHiddenCells ? [] : this.getters.getHiddenRowsGroups(sheetId).flat();
      ...
      for (let row = top; row <= bottom; row++) {
        ...
        if (!acceptHiddenCells && hiddenRows.includes(row)) continue;
    ...
    return subtotalFunctionAggregateByCode[code].bind(this)([evaluatedCellToKeep]);
  },
} satisfies AddFunctionDescription;
