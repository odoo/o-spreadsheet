...
function createScorecardChartRuntime(
  ...
  let baseline = chart.baseline ? getters.getRangeValues(chart.baseline)[0] : undefined;
  baseline = baseline ? String(baseline) : "";
  return {
    title: chart.title,
    keyValue: formattedKeyValue || keyValue,
    baseline: getBaselineText(baseline, keyValue, chart.baselineMode),
