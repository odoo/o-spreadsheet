...
export class CheckboxTable implements Cloneable<CheckboxTable> {
  readonly id: FilterTableId;
  readonly zone: Zone;
  readonly filters: Checkbox[];
  ...
  constructor(zone: Zone) {
    ...
    for (const i of range(zone.left, zone.right + 1)) {
      ...
      this.filters.push(new Checkbox(uuid.uuidv4(), filterZone));
    }
  }
  ...
  getFilterId(col: number): string | undefined {
    console.log("Filter ID : ", this.filters);
    return this.filters.find((filter) => filter.col === col)?.id;
  }

  clone(): CheckboxTable {
    return new CheckboxTable(this.zone);
  }
}
