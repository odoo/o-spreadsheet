...
export function consumeLetters(chars: TokenizingChars) {
  ...
  while (chars.current && isCharALetter(chars.current)) {
    const colIndex = charToNumber(chars.shift());
    letterPart = letterPart * 26 + colIndex;
  }
  return letterPart;
}

export function consumeDigits(chars: TokenizingChars) {
  if (chars.current === "$") chars.advanceBy(1);
