...
export function buildOccurrencesItems(
  ...
  for (const cell of vals) {
    const key = toTrimmedLowerCase(String(cell.value));
  ...
  return uniqueValues(col.nonEmpty)
    ...
    .map(({ value, formattedValue }) =>
      createStatItem(getters, sheetId, value, formattedValue, `=COUNTIF(${range},"${value}")`, {
        value: count.get(toTrimmedLowerCase(String(value))) ?? 0,
      })
    );
}
...
export function buildDateStatSections(
  ...
  const monthItems = Object.entries(MONTHS).map(([month, name]) =>
    createStatItem(
      ...
      "m" + month,
      name.toString(),
      `=SUM((MONTH(${range})=${Number(month) + 1})*(${range}<>""))`
    )
  );
  const dayItems = Object.entries(DAYS).map(([day, name]) =>
    createStatItem(
      ...
      "d" + day,
      name.toString(),
      `=SUM((WEEKDAY(${range})=${Number(day) + 1})*(${range}<>""))`
    )
  );
  return [
