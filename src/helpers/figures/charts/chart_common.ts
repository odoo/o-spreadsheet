...
export function formatChartDatasetValue(
  ...
  return (value: any, axisId: string) => {
    ...
    return formatTickValue({ format, locale }, humanize)(value);
  };
}

export function formatTickValue(localeFormat: LocaleFormat, humanize: boolean = false) {
  return (value: any) => {
    value = Number(value);
    if (isNaN(value)) return value;
    const { locale, format } = localeFormat;
    const formattedValue = humanize
      ? humanizeNumber(
          { value, format: !format && Math.abs(value) >= 1000 ? "#,##" : format },
          locale
        )
      : formatValue(value, {
          locale,
