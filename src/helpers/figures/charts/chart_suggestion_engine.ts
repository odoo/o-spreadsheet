...
function chartsForSingleNumberColumn(col: ColumnAnalysis, getters: Getters): ChartSuggestion[] {
  ...
  if (col.rowCount === 3) {
    const firstCellXC = zoneToXc({ ...col.zone, top: col.zone.top, bottom: col.zone.top });
    const secondCellXC = zoneToXc({ ...col.zone, top: col.zone.top + 1, bottom: col.zone.top + 1 });
    suggestions.push({
      title: _t("Gauge"),
...
function chartsForSinglePercentageColumn(col: ColumnAnalysis, getters: Getters): ChartSuggestion[] {
  ...
  if (col.rowCount === 1) {
    ...
    const firstCellXC = zoneToXc({ ...col.zone, bottom: col.zone.top });
    const secondCellXC = zoneToXc({ ...col.zone, top: col.zone.top + 1, bottom: col.zone.top + 1 });
    suggestions.push({
      title: _t("Gauge"),
      rationale: _t("Natural fit for a 0–100% range."),
...
function chartsForNumberVsNumber(cols: ColumnAnalysis[], getters: Getters): ChartSuggestion[] {
  ...
  if (col1.rowCount === 1) {
    suggestions.push({
      title: _t("KPI Card"),
      rationale: _t("Highlights the most recent value of the second metric."),
      definition: scorecardChart(
        title,
        zoneToXc({ ...col2.zone, top: col2.zone.bottom, bottom: col2.zone.bottom })
      ),
    });
  }
...
function chartsForLabelVsNumber(cols: ColumnAnalysis[], getters: Getters): ChartSuggestion[] {
  ...
  if (labelCol.rowCount === 1) {
    suggestions.push({
      ...
      definition: scorecardChart(
        "",
        zoneToXc({ ...numCol.zone, top: numCol.zone.bottom, bottom: numCol.zone.bottom }),
        { humanize: false, baselineMode: "text", baseline: labelRange }
      ),
    });
  }
...
function patternS(cols: ColumnAnalysis[], getters: Getters): ChartSuggestion[] {
  ...
  if (cols.length === 3 && cols[0].rowCount === 1) {
    const firstCellXC = zoneToXc(cols[0].zone);
    const secondCellXC = zoneToXc(cols[1].zone);
    const lastCellXC = zoneToXc(cols[2].zone);
    suggestions.push({
      title: _t("Gauge"),
      rationale: _t("Shows the position of the last value within the data's min-max range."),
      definition: gaugeChart(title, lastCellXC, `=${firstCellXC}`, `=${secondCellXC}`),
