...
type DS = {
  type: "range";
  dataSets: { dataRange: string; dataSetId: string }[];
...
function barChart(
  ...
  opts: {
    stacked?: boolean;
    horizontal?: boolean;
    legendPosition?: "top" | "none" | "bottom" | "left" | "right";
  } = {}
...
function funnelChart(titleText: string, source: DS): FunnelChartDefinition<string> {
  return {
    type: "funnel",
    title: { text: titleText },
    dataSource: source,
...
function sunburstChart(titleText: string, source: DS): SunburstChartDefinition<string> {
  return {
    type: "sunburst",
    title: { text: titleText },
    dataSource: source,
...
function treemapChart(titleText: string, source: DS): TreeMapChartDefinition<string> {
  return {
    type: "treemap",
    title: { text: titleText },
    dataSource: source,
...
function calendarChart(titleText: string, source: DS): CalendarChartDefinition<string> {
  return {
    type: "calendar",
    title: { text: titleText },
    dataSource: source,
...
function scatterChart(titleText: string, source: DS): ScatterChartDefinition<string> {
  return {
    type: "scatter",
    title: { text: titleText },
    dataSource: source,
...
function scorecardChart(
  ...
  opts: {
    baseline?: string;
    baselineMode?: "difference" | "percentage" | "text" | "progress";
  } = {}
    ...
    if (cells.every((c) => c.type === CellValueType.boolean)) {
      ...
      if (textVals.length > 0) {
        ...
        if (hier) {
          ...
          return ratio < 0.5 && unique <= 20 ? "categorical" : "label";
        }
      }
    }
  }
...
function chartsForDateVsMultipleNumbers(
  ...
  return [
    ...
    {
      title: _t("%s — Combo Chart", title),
      rationale: _t("Bar for the primary metric, line for the secondary."),
      definition: {
        type: "combo",
        title: { text: title },
        dataSource: rangeSource(dataSets.slice(0, 2), hasTitle, labelRange),
        dataSetStyles: {},
        legendPosition: "top",
        humanize: true,
      },
...
function matchPattern(cols: ColumnAnalysis[], getters: Getters): ChartSuggestion[] {
  ...
  if (numberOfColumns >= 3) {
    ...
    if (
      (first === "categorical" || first === "label") &&
      rest.every((t) => t === "number" || t === "percentage")
    ) {
