...
export class ComboChart extends AbstractChart {
  ...
  static getDefinitionFromContextCreation(context: ChartCreationContext): ComboChartDefinition {
    const dataSetsStyles: ComboChartDataSetStyling = {};
    const firstDataSetId = context.dataSource?.dataSets[0]?.id;
    for (const dataSet of context.dataSource?.dataSets || []) {
      dataSetsStyles[dataSet.id] = {
        ...(context.dataSets?.[dataSet.id] || {}),
        type: dataSet.id === firstDataSetId ? "bar" : "line",
      };
    }
    return {
