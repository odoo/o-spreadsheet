...
import { getChartDatasetValues, getChartLabelValues } from "./runtime";
...
export class HeatMap extends AbstractChart {
  ...
  duplicateInDuplicatedSheet(newSheetId: UID): HeatMap {
    ...
    return new HeatMap(definition, newSheetId, this.getters);
  }
...
export function drawHeatMap(structure: HeatMapConfig, canvas: HTMLCanvasElement) {
  ...
  const availableWidth = canvas.width - CHART_PADDING;
