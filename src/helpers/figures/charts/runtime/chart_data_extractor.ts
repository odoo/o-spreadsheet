...
function computeNumericBins(values: number[], format: Format | undefined, locale: Locale) {
  ...
  const getBoundaryValue = (edgeIndex: number) =>
    edgeIndex === binCount ? maxValue : minValue + edgeIndex * binSize;
  return {
    binCount,
    ...
    /** Formatted value at each bin boundary, from the lowest to the highest (binCount + 1 values) */
...
export function getHeatmapChartData(
  ...
  const { dataSetsValues, labels } = computeGroupedValuesAndLabels(
    ...
    (keys) => (columnBins ? range(0, columnBins.binCount) : keys),
    (keys) => (rowBins ? range(0, rowBins.binCount) : keys)
  );
