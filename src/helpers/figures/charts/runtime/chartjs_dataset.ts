...
export function getCalendarChartDatasetAndLabels(
  ...
  if (typeof definition.colorScale === "object") {
    colorMap = new ThreePointsColorMap(
      minValue,
      maxValue,
    ...
    switch (definition.colorScale) {
      case "blues":
        colorMap = new BluesColorMap(minValue, maxValue);
        break;
      case "cividis":
        colorMap = new CividisColorMap(minValue, maxValue);
        break;
      case "greens":
        colorMap = new GreensColorMap(minValue, maxValue);
        break;
      case "oranges":
        colorMap = new OrangesColorMap(minValue, maxValue);
        break;
      case "purples":
        colorMap = new PurplesColorMap(minValue, maxValue);
        break;
      case "rainbow":
        colorMap = new RainbowColorMap(minValue, maxValue);
        break;
      case "reds":
        colorMap = new RedsColorMap(minValue, maxValue);
        break;
      case "viridis":
      ...
      default:
        ...
        colorMap = new GreysColorMap(minValue, maxValue);
    }
  }
  ...
  for (let i = 0; i < matrixValues.length; i++) {
    dataSets.push({
      ...
      backgroundColor: matrixValues[i].map((v) =>
        v !== undefined ? colorMap.getColor(v) : definition.missingValueColor || COLOR_TRANSPARENT
      ),
      barPercentage: 1.0,
