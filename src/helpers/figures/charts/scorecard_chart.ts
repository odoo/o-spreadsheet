...
function getData(formula: string | undefined, getters: Getters, sheetId: UID) {
  let scalar: FunctionResultObject | undefined;
  if (formula) {
    const result = getters.evaluateFormulaResult(sheetId, formula);
    scalar = isMultipleElementMatrix(result) ? result[0][0] : toScalar(result);
  }
  let range: Range | undefined = undefined;
  if (formula) {
    const xc = getFormulaRangeXc(formula);
    if (xc) {
...
export const ScorecardChart: ChartTypeBuilder<"scorecard"> = {
  ...
  getDefinitionFromContextCreation(context, dataSourceBuilder) {
    ...
    return {
      ...
      type: "scorecard",
      keyValue: dataRange ? `=${dataRange}` : context.scorecardKeyValueFormula,
  ...
  getFormulas(getters, sheetId, definition): CompiledFormula[] {
    ...
    if (definition.keyValue && definition.keyValue.startsWith("=")) {
      formulas.push(CompiledFormula.Compile(definition.keyValue, sheetId, getters));
    }
    if (definition.baseline && definition.baseline.startsWith("=")) {
