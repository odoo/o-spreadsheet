...
function checkKeyValue(definition: ScorecardChartDefinition): CommandResult {
  if (definition.keyValueType === "formula" || definition.keyValueType === "literal") {
...
export const ScorecardChart: ChartTypeBuilder<"scorecard"> = {
  ...
  transformDefinition(definition, chartSheetId, { adaptRangeString, adaptFormulaString }) {
    ...
    } else if (definition.keyValueType === "formula" && definition.keyValue) {
      keyValue = adaptFormulaString(chartSheetId, definition.keyValue as string);
    } else {
  ...
  updateRanges(definition, adapterFunctions: RangeAdapterFunctions, sheetId) {
    ...
    let keyValue: Range | string | undefined;
    if (isRange) {
      keyValue = adaptChartRange(definition.keyValue as Range | undefined, adapterFunctions);
    } else if (definition.keyValueType === "formula" && definition.keyValue) {
      keyValue = adapterFunctions.adaptFormulaString(sheetId, definition.keyValue as string);
    } else {
      keyValue = definition.keyValue as string | undefined;
    }
  ...
  getRuntime(getters, definition, _dataExtractors, sheetId): ScorecardChartRuntime {
    ...
    if (definition.keyValue) {
      if (isKeyValueRange) {
        ...
        const result = getters.evaluateFormulaResult(sheetId, definition.keyValue as string);
        ...
        if (scalar !== null && scalar !== undefined) {
          formattedKeyValue =
            definition.humanize ?? true ? humanizeNumber(scalar, locale) : String(scalar.value);
        } else {
          formattedKeyValue = "";
        }
      } else {
