...
import { getFontSizeMatchingWidth, splitTextInTwoLines } from "../../text_helper";
...
export type ScorecardChartConfig = {
  ...
  baselineDescr?:
    | (ScorecardChartElement & { splitted?: false })
    | (Omit<ScorecardChartElement, "text"> & { text: string[]; splitted: true });
...
class ScorecardChartConfigBuilder {
  ...
  computeDesign(): ScorecardChartConfig {
    ...
    const baselineDescrWidth = style.baselineDescr.splitted
      ? Math.max(
    ...
    if (this.baselineDescr) {
      if (style.baselineDescr.splitted) {
        structure.baselineDescr = {
          text: this.splittedBaselineDescr,
  ...
  get splittedBaselineDescr(): string[] {
    return splitTextInTwoLines(this.baselineDescr);
  }
  ...
  private getTextStyles() {
    ...
    const baselineValueMeasure = this.context.measureText(baselineText);
    const remainingWidth = maxLineWidth - baselineValueMeasure.width;
    let baselineDescrFontSize = getFontSizeMatchingWidth(
      remainingWidth,
      baselineValueFontSize,
      (fontSize: number) => {
        this.context.font = getDefaultContextFont(fontSize);
        return this.context.measureText(this.baselineDescr).width;
      }
    );

    let splittedBaseline = false;
    if (baselineDescrFontSize < baselineValueFontSize / 2.5) {
      splittedBaseline = true;
      baselineDescrFontSize = Math.floor(baselineValueFontSize / 2.5);
      for (const str of this.splittedBaselineDescr) {
        const strWidth = getFontSizeMatchingWidth(
          remainingWidth,
          baselineValueFontSize,
          (fontSize: number) => {
            this.context.font = getDefaultContextFont(fontSize);
        ...
        baselineDescrFontSize = Math.min(baselineDescrFontSize, strWidth);
      }
    }
    if (this.runtime.progressBar) {
  ...
  private get getDrawableHeight(): number {
