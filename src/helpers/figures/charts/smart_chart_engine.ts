  ...
  switch (type) {
    case "percentage":
      return {
        ...
        dataSource: { dataSets: [{ dataRange, dataSetId: "1" }] },
        dataSetStyles: {},
        legendPosition: "none",
        dataSetsHaveTitle,
      };
...
function buildTwoColumnChart(columns: ColumnInfo[], getters: Getters): ChartDefinition {
  ...
  if (columns[1].type === "percentage") {
    return {
      ...
      title: {},
      dataSource: {
        dataSets: [{ dataRange: getUnboundRange(getters, columns[1].zone), dataSetId: "1" }],
      },
      dataSetStyles: {},
  ...
  if (columns[0].type === "text" && columns[1].type === "number") {
    ...
    if (uniqueCount !== totalCount) {
      return {
        ...
        title: {},
        dataSource: {
          dataSets: [{ dataRange: getUnboundRange(getters, textColumn.zone), dataSetId: "1" }],
        },
        dataSetStyles: {},
...
function buildScorecard(zone: Zone, getters: Getters): ChartDefinition {
  const style = getters.getCellStyle({
    sheetId: getters.getActiveSheetId(),
    col: zone.left,
    row: zone.top,
  });
...
export function getSmartChartDefinition(zones: Zone[], getters: Getters): ChartDefinition {
  ...
  if (columns.length === 0 || columns.every((col) => col.type === "empty")) {
    ...
    return { ...DEFAULT_BAR_CHART_CONFIG, dataSource: { dataSets } };
  }
