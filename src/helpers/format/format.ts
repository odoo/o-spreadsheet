...
export function formatValue(
  ...
  switch (typeof value) {
    ...
    case "number":
      ...
      return applyNumberInternalFormat(value, formatToApply, locale, formatWidth);
...
function applyNumberInternalFormat(
  ...
  if (internalFormat.type === "date") {
    return repeatCharToFitWidth(applyDateTimeFormat(value, internalFormat), formatWidth);
  }

  const isNegative = value < 0;
  const formatted = applyInternalNumberFormat(Math.abs(value), internalFormat, locale);
  const formattedValue = isNegative ? "-" + formatted : formatted;
...
function repeatCharToFitWidth(formattedValue: string, formatWidth?: FormatWidth): string {
  ...
  function getTimesToRepeat() {
    ...
    const availableWidth = formatWidth.maxWidth - widthTaken;
    const timesToRepeat = Math.floor(availableWidth / charWidth);
    return Math.max(0, timesToRepeat);
  }

  return prefix + repeatedChar.repeat(getTimesToRepeat()) + suffix;
}
