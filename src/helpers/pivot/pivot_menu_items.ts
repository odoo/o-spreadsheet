...
export const groupPivotHeaders: ActionSpec = {
  ...
  isVisible: (env) => {
    ...
    const pivot = env.model.getters.getPivot(matchingHeaders.pivotId);
    return (
      matchingHeaders.values.length > 1 &&
      pivotRegistry.get(pivot.type).canHaveCustomGroup(matchingHeaders.field)
    );
  },
};
