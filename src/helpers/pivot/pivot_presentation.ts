...
export default function (PivotClass: PivotUIConstructor) {
  class PivotPresentationLayer extends PivotClass {
    ...
    private getPreviousRunningTotalValue(
      ...
      const pivotCells = this.getPivotValueCells(measure.id);
      for (const col of pivotCells) {
        for (const cell of col) {
          const cellSecondaryDomain = getDimensionDomain(this, secondaryDimension, cell.domain);
          if (domainToString(cellSecondaryDomain) !== secondaryDomainKey) {
            continue;
          }
          const cellMainDomain = getDimensionDomain(this, mainDimension, cell.domain);
          const cellValue = cellMainDomain.find(
            (node) => node.field === fieldNameWithGranularity
          )?.value;
          if (cellValue === undefined) {
            continue;
          }
          const cellRunningTotalKey = getRunningTotalDomainKey(
            cellMainDomain,
            fieldNameWithGranularity
          );
          if (cellRunningTotalKey !== runningTotalKey) {
            continue;
          }
          if (this.compareRunningTotalValues(cellValue, targetValue, dimension) >= 0) {
            continue;
          }
          if (
            previousValue === undefined ||
            this.compareRunningTotalValues(previousValue, cellValue, dimension) < 0
          ) {
            previousValue = cellValue;
            previousMainDomainKey = domainToString(cellMainDomain);
          }
        }
      }
