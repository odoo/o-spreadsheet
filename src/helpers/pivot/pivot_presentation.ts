...
export function withPivotPresentationLayer(PivotClass: PivotUIConstructor) {
  class PivotPresentationLayer extends PivotClass {
    ...
    getPivotCellValueAndFormat(measureId: string, domain: PivotDomain): FunctionResultObject {
      return this.getMeasureDisplayValue(measureId, domain);
    }
    ...
    private asIndex(
      ...
      return parentRowTotal === 0 || parentColTotal === 0
        ? { value: CellErrorType.DivisionByZero }
        : { value: (value * grandTotal) / (parentColTotal * parentRowTotal), format };
    }
    ...
    private asDifferenceFrom(
      ...
      return comparedValue === "sameValue"
        ? { value: "" }
        : { value: this.measureValueToNumber(rawValue) - comparedValue, format: rawValue.format };
    }
    ...
    private computeRank(
      ...
      for (const col of pivotCells) {
        ...
        for (const groupKey in groupedCell) {
          groupedCell[groupKey] = removeDuplicates(
            groupedCell[groupKey] || [],
            (cell) => cell.value
          )
    ...
    private getComparisonValue(
      ...
      domain: PivotDomain,
      fieldNameWithGranularity: string,
      valueToCompare: CellValue | "(previous)" | "(next)"
    ): number | "sameValue" | undefined {
      const comparedDomain =
        valueToCompare === "(previous)" || valueToCompare === "(next)"
