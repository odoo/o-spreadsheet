...
export class PivotRuntimeDefinition {
  ...
  getDimension(nameWithGranularity: string): PivotDimension {
    const dimension = this.columns
      .concat(this.rows)
      .find((d) => d.nameWithGranularity === nameWithGranularity);
    if (!dimension) {
  ...
  getMeasure(name: string): PivotMeasure {
    ...
    const measure = measures.find((measure) => measure.name === name);
    if (!measure) {
...
function createPivotDimension(fields: PivotFields, dimension: PivotCoreDimension): PivotDimension {
  ...
  return {
    ...
    order: dimension.order || (field?.type === "date" ? "asc" : undefined),
