...
export type TableStyleTemplate = (colorSet: ColorSet) => Omit<TableStyle, "name">;
...
function buildPreset(name: string, template: TableStyleTemplate, colorSet: ColorSet): TableStyle {
  return { ...template(colorSet), name: `${colorSet.name}, ${name}` };
}

export const TABLE_PRESETS: Record<string, TableStyle> = {
  None: { category: "light", templateName: "none", primaryColor: "", name: "none" },
  ...
  TableStyleMedium15: { ...mediumMinimalBordersInBlack, name: "Black, TableStyleMedium15" },
  ...
  TableStyleDark1: { ...darkTemplateInBlack, name: "Black, TableStyleDark1" },
...
export function buildTableStyle(
  ...
  return { ...TABLE_STYLES_TEMPLATES[templateName](colorSet), category: "custom", name };
}
