...
export function intersection(z1: UnboundedZone, z2: Zone): Zone | undefined {
  ...
  return {
    ...
    bottom: Math.min(z1.bottom ?? MAX_ROW, z2.bottom),
    right: Math.min(z1.right ?? MAX_COL, z2.right),
  };
}
 ...
 * reference equality.
 */
export function isEqual(z1: UnboundedZone, z2: Zone): boolean {
  return (
 ...
 */
export function overlap(z1: UnboundedZone, z2: Zone): boolean {
  if (z1.bottom === undefined || z1.bottom < z2.top || z2.bottom < z1.top) {
    return false;
  }
  if (z1.right === undefined || z1.right < z2.left || z2.right < z1.left) {
    return false;
  }
  return true;
}
