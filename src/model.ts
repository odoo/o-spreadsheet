...
export class Model extends owl.core.EventBus implements CommandDispatcher {
  ...
  constructor(data: any = {}, config: Partial<ModelConfig> = {}) {
    ...
    for (let Plugin of basePluginRegistry.getAll()) {
      this._setupBasePlugin(Plugin, workbookData);
    }
  ...
  private _setupBasePlugin(Plugin: typeof BasePlugin, data: WorkbookData) {
    ...
    if (Plugin.modes.includes(this.config.mode)) {
      ...
      this.eventHandlers.push(plugin);
      // @ts-ignore
  ...
  dispatchEvent: EventDispatcher["dispatchEvent"] = (type: string, payload?: any) => {
    for (const h of this.eventHandlers) {
      h.handleEvent(Object.assign({}, payload, { type }));
    }
  };
