...
import { Cell, WorkBookState, NewCell, Zone } from "./state";

export function getCell(state: WorkBookState, col: number, row: number): Cell | null {
  return state.rows[row].cells[col] || null;
}

export function selectedCell(state: WorkBookState): Cell | null {
...
export function formatCell(state: WorkBookState, cell: Cell): string {
  if (cell.value === "") {
    return "";
  }
  if (cell.value === false) {
 ...
 */
export function setValue(state: WorkBookState, xc: string, text: string) {
 ...
 * the cell.
 */
export function deleteCell(state: WorkBookState, xc: string, force: boolean = false) {
...
export function movePosition(state: WorkBookState, deltaX: number, deltaY: number) {
...
export function getCol(state: WorkBookState, x: number): number {
  if (x <= HEADER_WIDTH) {
    return -1;
  }
...
export function getRow(state: WorkBookState, y: number): number {
  if (y <= HEADER_HEIGHT) {
    return -1;
  }
...
export function getColSize(state: WorkBookState, index: number) {
  const { cols } = state;
  return cols[index].size;
}

export function getRowSize(state: WorkBookState, index: number) {
  const { rows } = state;
  return rows[index].size;
}

export function updateScroll(state: WorkBookState, scrollTop: number, scrollLeft: number): boolean {
 ...
 */
export function updateVisibleZone(state: WorkBookState, width?: number, height?: number) {
...
export function deleteSelection(state: WorkBookState) {
...
export function startEditing(state: WorkBookState, str?: string) {
  if (!str) {
...
export function removeHighlights(state: WorkBookState) {
  state.highlights = [];
}

export function cancelEdition(state: WorkBookState) {
...
export function stopEditing(state: WorkBookState) {
  if (state.isEditing) {
 ...
 * */
export function setCurrentContent(state: WorkBookState, content: string) {
 ...
 */
export function selectCell(state: WorkBookState, col: number, row: number, newRange: boolean = false) {
...
export function computeAggregate(state: WorkBookState): string | null {
  let aggregate = 0;
  let n = 0;
 ...
 */
export function activateCell(state: WorkBookState, col: number, row: number) {
  state.activeCol = col;
  state.activeRow = row;
