...
import { Border, BorderCommand, WorkBookState, Style, Zone } from "./state";
...
export function setStyle(state: WorkBookState, style: Style) {
 ...
 */
export function clearFormatting(state: WorkBookState) {
  for (let zone of state.selection.zones) {
    for (let col = zone.left; col <= zone.right; col++) {
      for (let row = zone.top; row <= zone.bottom; row++) {
...
function removeFormatting(state: WorkBookState, col: number, row: number) {
...
export function getStyle(state: WorkBookState): Style {
  const cell = selectedCell(state);
  return cell && cell.style ? state.styles[cell.style] : {};
}

function setStyleToCell(state: WorkBookState, col: number, row: number, style) {
  const cell = getCell(state, col, row);
...
function registerStyle(state: WorkBookState, style) {
...
export function setBorder(state: WorkBookState, command: BorderCommand) {
  for (let zone of state.selection.zones) {
    setBorderToZone(state, zone, command);
  }
}

function setBorderToZone(state: WorkBookState, zone: Zone, command: BorderCommand) {
  if (command === "clear") {
    for (let row = zone.top; row <= zone.bottom; row++) {
      for (let col = zone.left; col <= zone.right; col++) {
...
function clearBorder(state: WorkBookState, col: number, row: number) {
  const cell = getCell(state, col, row);
  if (cell) {
...
function clearSide(state: WorkBookState, col: number, row: number, side: string) {
  const cell = getCell(state, col, row);
  if (cell && cell.border) {
...
function setBorderToCell(state: WorkBookState, col: number, row: number, border: Border) {
  const cell = getCell(state, col, row);
...
function registerBorder(state: WorkBookState, border: Border) {
...
export function setFormat(state: WorkBookState, format: string) {
  for (let zone of state.selection.zones) {
