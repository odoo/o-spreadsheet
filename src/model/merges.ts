...
export function addMerge(state: Workbook, m: string) {
  let id = state.nextId++;
  const [tl, br] = m.split(":");
 ...
 */
export function merge(state: Workbook) {
...
export function unmerge(state: Workbook) {
 ...
 * top left.
 */
export function isMergeDestructive(state: Workbook): boolean {
  const { left, right, top, bottom } = state.selection.zones[state.selection.zones.length - 1];
  for (let row = top; row <= bottom; row++) {
