 ...
 */
export function updateColSize(state: WorkBookState, index: number, delta: number) {
  const { cols } = state;
 ...
 * @param delta Delta add to the size
 */
export function updateRowSize(state: WorkBookState, index: number, delta: number) {
  const { rows } = state;
 ...
 */
export function updateColsSize(state: WorkBookState, base: number, delta: number) {
  const { cols } = state;
  const col = cols[base];
 ...
 */
export function updateRowsSize(state: WorkBookState, base: number, delta: number) {
  const { rows } = state;
  const row = rows[base];
...
export function setColSize(state: WorkBookState, col: number, size: number) {
  updateColSize(state, col, size - state.cols[col].size);
}

export function setRowSize(state: WorkBookState, row: number, size: number) {
