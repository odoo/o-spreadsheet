...
import { Zone, WorkBookState } from "./state";
import { stopEditing, activateCell } from "./core";

/**
 * Add all necessary merge to the current selection to make it valid
 */
function expandZone(state: WorkBookState, zone: Zone): Zone {
...
export function moveSelection(state: WorkBookState, deltaX: number, deltaY: number) {
...
export function selectColumn(state: WorkBookState, col: number, addToCurrent: boolean) {
  stopEditing(state);
...
export function selectRow(state: WorkBookState, row: number, addToCurrent: boolean) {
  stopEditing(state);
...
export function selectAll(state: WorkBookState) {
  stopEditing(state);
 ...
 */
export function updateSelection(state: WorkBookState, col: number, row: number) {
  const anchorCol = state.selection.anchor.col;
  const anchorRow = state.selection.anchor.row;
  const zone: Zone = {
 ...
 */
export function setSelectingRange(state: WorkBookState, isSelecting: boolean) {
  state.isSelectingRange = isSelecting;
}

export function increaseSelectColumn(state: WorkBookState, col: number) {
  const anchorCol = state.selection.anchor.col;
  const zone: Zone = {
    left: Math.min(anchorCol, col),
    top: 0,
...
export function increaseSelectRow(state: WorkBookState, row: number) {
  const anchorRow = state.selection.anchor.row;
  const zone: Zone = {
    left: 0,
    top: Math.min(anchorRow, row),
...
export function zoneIsEntireColumn(state: WorkBookState, zone: Zone) {
  return zone.top === 0 && zone.bottom === state.rows.length - 1;
}

export function zoneIsEntireRow(state: WorkBookState, zone: Zone) {
  return zone.left === 0 && zone.right === state.cols.length - 1;
}

export function getActiveCols(state: WorkBookState): Set<number> {
...
export function getActiveRows(state: WorkBookState): Set<number> {
...
export function startNewComposerSelection(state: WorkBookState): void {
 ...
 */
export function selectionZoneXC(state: WorkBookState): string {
...
export function addHighlights(state: WorkBookState, rangesUsed: { [keys: string]: string }) {
