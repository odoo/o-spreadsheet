...
function internal(
  border: BorderDescrInternal | BorderDescr | undefined | null
): BorderDescrInternal {
  if (!border) {
    return EMPTY_BORDER;
  }
...
function both(border: BorderDescrInternal | BorderDescr | undefined | null): BorderDescrInternal {
  if (!border) {
    return EMPTY_BORDER;
  }
...
function toDescr(
  ...
  if (!border || border.internal === options?.undefinedIf) {
    return undefined;
  }
  if (border.style === "empty") {
    return options?.nullOnEmpty ? null : undefined;
  }
  return { color: border.color, style: border.style };
}

const undefinedIfExternal = { undefinedIf: "external" } as const;
...
/**
 *
 * This plugin manages all things related to cells borders
 */
export class BordersPlugin extends CorePlugin<BordersPluginState> implements BordersPluginState {
  ...
  handle(cmd: CoreCommand) {
    switch (cmd.type) {
      ...
      case "SET_BORDER": {
        ...
        if (cmd.border?.bottom !== undefined || cmd.border === undefined) {
          this.setCellBorder(
            "TOP",
            cmd.sheetId,
            cmd.col,
      ...
      case "SET_BORDERS_ON_TARGET":
        if (!cmd.border) {
          this.setBorders(cmd.sheetId, cmd.target, "clear", undefined);
          break;
        }
        if (cmd.border.left !== undefined) {
          ...
          this.setBorder(
            "LEFT",
            cmd.sheetId,
            cmd.target.map((z) => extendZone(z, "left", -1)),
            deepEquals(cmd.border.left, cmd.border.right)
              ? both(cmd.border.left)
        ...
        if (cmd.border.top !== undefined) {
          ...
          this.setBorder(
            "TOP",
            cmd.sheetId,
            cmd.target.map((z) => extendZone(z, "top", -1)),
            deepEquals(cmd.border.top, cmd.border.bottom)
              ? both(cmd.border.top)
  ...
  private setSheetBorder(
    ...
    this.history.update(
      ...
      sheetId,
      "sheetDefault",
      border === EMPTY_BORDER ? undefined : border
    );
  ...
  private clearBorders(sheetId: UID, zones: Zone[]) {
    this.clearBorder(
      "LEFT",
      sheetId,
      zones.map((zone) => extendZone(zone, "right", 1))
    );
    this.clearBorder(
      "TOP",
      sheetId,
      zones.map((zone) => extendZone(zone, "bottom", 1))
    );
  }

  private clearBorder(borderType: "LEFT" | "TOP", sheetId: UID, zones: Zone[]) {
    zones = recomputeZones(zones);
    const { numberOfCols, numberOfRows } = this.getters.getSheetSize(sheetId);
    const sheetArea = numberOfCols * numberOfRows;
    for (const zone of zones) {
      ...
      const defaultRow = zone.right - zone.left + 1 > numberOfCols / 2;
      if (defaultRow && defaultCol && getZoneArea(zone) > sheetArea / 2) {
        this.clearSheetBorders(borderType, sheetId, zone);
      } else if (defaultCol) {
        this.clearColsBorders(borderType, sheetId, zone);
      } else if (defaultRow) {
        this.clearRowsBorders(borderType, sheetId, zone);
      } else {
        this.clearZoneBorders(borderType, sheetId, zone);
      }
    }
  }

  private clearSheetBorders(borderType: "LEFT" | "TOP", sheetId: UID, zone: Zone) {
    this.clearZoneInternalCellsBorder(borderType, sheetId, zone);
    const sheetZone = this.getters.getSheetZone(sheetId);
    const horizontalZone = this.getters.getRowsZone(sheetId, zone.top, zone.bottom);
    const externalHorizontalZones = recomputeZones([horizontalZone], [zone]);
    let defaults = this.getDefaultBorderInCell(borderType, sheetId, externalHorizontalZones, {
    ...
    const defaultKey = borderType === "LEFT" ? "defaultLeft" : "defaultTop";
    this.history.update(defaultKey, sheetId, "sheetDefault", undefined);
    const rows = Object.keys(this[defaultKey][sheetId]?.rowDefault ?? {});
    for (const rowIdx of rows) {
      const row = parseInt(rowIdx);
      if (zone.top <= row && row <= zone.bottom) {
  ...
  private clearColsBorders(borderType: "LEFT" | "TOP", sheetId: UID, zone: Zone) {
    this.clearZoneInternalCellsBorder(borderType, sheetId, zone);
    const leftoverZones = recomputeZones(
      [this.getters.getColsZone(sheetId, zone.left, zone.right)],
      [zone]
  ...
  private clearRowsBorders(borderType: "LEFT" | "TOP", sheetId: UID, zone: Zone) {
    this.clearZoneInternalCellsBorder(borderType, sheetId, zone);
    const leftoverZones = recomputeZones(
      [this.getters.getRowsZone(sheetId, zone.top, zone.bottom)],
      [zone]
    ...
    if (borderType === "TOP") {
      const colOverlap = Object.keys(this[defaultKey][sheetId]?.colDefault ?? {});
      for (const colIndex of colOverlap) {
        const col = parseInt(colIndex);
        const colDefault = this[defaultKey][sheetId]?.colDefault?.[col];
        if (!colDefault) {
          continue;
        }
        if (colDefault.internal !== "internal") {
          this.setCellBorder(borderType, sheetId, col, zone.top, colDefault);
        }
        if (colDefault.internal !== "external") {
          this.setCellBorder(borderType, sheetId, col, zone.bottom, colDefault);
        }
      }
    }
  ...
  private clearZoneBorders(borderType: "LEFT" | "TOP", sheetId: UID, zone: Zone) {
    this.clearZoneInternalCellsBorder(borderType, sheetId, zone);
    const defaultKey = borderType === "LEFT" ? "defaultLeft" : "defaultTop";
    const borderKey = borderType === "LEFT" ? "bordersLeft" : "bordersTop";
    ...
    const rowOverlap = Object.keys(this[defaultKey][sheetId]?.rowDefault ?? {})
      .map((k) => parseInt(k, 10))
      .filter((header) => zone.top <= header && header <= zone.bottom);
    for (const row of rowOverlap) {
      for (let col = zone.left; col <= zone.right; col++) {
        this.history.update(borderKey, sheetId, col, row, EMPTY_BORDER);
      }
    }

    const colOverlap = Object.keys(this[defaultKey][sheetId]?.colDefault ?? {})
      .map((k) => parseInt(k, 10))
      .filter((header) => zone.left <= header && header <= zone.right);

    for (const col of colOverlap) {
      for (let row = zone.top; row <= zone.bottom; row++) {
        this.history.update(borderKey, sheetId, col, row, EMPTY_BORDER);
      }
    }

    if (this[defaultKey][sheetId]?.sheetDefault) {
      for (let col = zone.left; col <= zone.right; col++) {
        for (let row = zone.top; row <= zone.bottom; row++) {
          this.history.update(borderKey, sheetId, col, row, EMPTY_BORDER);
        }
  ...
  private clearZoneInternalCellsBorder(borderType: "LEFT" | "TOP", sheetId: UID, zone: Zone) {
    if (borderType === "LEFT") {
      this.clearPartialCellsBorder("LEFT", sheetId, leftCol(zone), "internal");
      this.clearCellsBorder("LEFT", sheetId, extendZone(extendZone(zone, "left", -1), "right", -1));
      this.clearPartialCellsBorder(
        "LEFT",
        sheetId,
        { left: zone.right, right: zone.right, top: zone.top, bottom: zone.bottom },
        "external"
      );
    }
    if (borderType === "TOP") {
      this.clearPartialCellsBorder("TOP", sheetId, topRow(zone), "internal");
      this.clearCellsBorder("TOP", sheetId, extendZone(extendZone(zone, "top", -1), "bottom", -1));
      this.clearPartialCellsBorder(
        "TOP",
        sheetId,
        { left: zone.left, right: zone.right, top: zone.bottom, bottom: zone.bottom },
        "external"
      );
    }
  }

  private clearCellsBorder(borderType: "LEFT" | "TOP", sheetId: UID, zone: Zone) {
    const borderKey = borderType === "LEFT" ? "bordersLeft" : "bordersTop";
    for (let col = zone.left; col <= zone.right; col++) {
      for (let row = zone.top; row <= zone.bottom; row++) {
  ...
  private clearPartialCellsBorder(
    borderType: "LEFT" | "TOP",
    sheetId: UID,
    zone: Zone,
  ...
  private ensureColumnBorderContinuity(
    ...
    for (let row: HeaderIndex = 0; row <= this.getters.getNumberRows(sheetId) + 1; row++) {
  ...
  private ensureRowBorderContinuity(sheetId: UID, topRow: HeaderIndex, bottomRow: HeaderIndex) {
    for (let col: HeaderIndex = 0; col < this.getters.getNumberCols(sheetId) + 1; col++) {
      for (const borderType of ["LEFT", "TOP"] as const) {
   ...
   */
  private shiftBordersVertically(sheetId: UID, start: HeaderIndex, quantity: number) {
    for (const defaultKey of ["defaultTop", "defaultLeft"] as const) {
      const positionValue = Object.entries(this[defaultKey][sheetId]?.rowDefault ?? []);
      if (quantity > 0) {
        positionValue.reverse();
      }
      for (const [rowIndex, value] of positionValue) {
        const row = parseInt(rowIndex);
        if (row < start) {
          continue;
        }
   ...
   * @param delta how much borders will be moved (negative if moved to the left)
   */
  private shiftBordersHorizontally(sheetId: UID, start: HeaderIndex, quantity: number) {
    for (const defaultKey of ["defaultTop", "defaultLeft"] as const) {
      const positionValue = Object.entries(this[defaultKey][sheetId]?.colDefault ?? []);
      if (quantity > 0) {
        positionValue.reverse();
      }
      for (const [colIndex, value] of positionValue) {
        const col = parseInt(colIndex);
        if (col < start) {
          continue;
        }
  ...
  private mergeBorders(sheetId: UID, zone: Zone) {
    ...
    // Clear inside
    this.clearCellsBorder("LEFT", sheetId, extendZone(zone, "left", -1));
    this.clearCellsBorder("TOP", sheetId, extendZone(zone, "top", -1));
  }
  ...
  private getCellSpecificBorder(
    sheetId: UID,
    col: HeaderIndex,
    row: HeaderIndex
  ): BorderOrNull | undefined {
    const left = toDescr(this.bordersLeft[sheetId]?.[col]?.[row], {
      nullOnEmpty: true,
      undefinedIf: "external",
    });
    const right = toDescr(this.bordersLeft[sheetId]?.[col + 1]?.[row], {
      nullOnEmpty: true,
      undefinedIf: "internal",
    });
    const top = toDescr(this.bordersTop[sheetId]?.[col]?.[row], {
      nullOnEmpty: true,
      undefinedIf: "external",
    });
    const bottom = toDescr(this.bordersTop[sheetId]?.[col]?.[row + 1], {
      nullOnEmpty: true,
      undefinedIf: "internal",
    });
  ...
  private getDefaultBorderInCell(
    ...
    for (const position of zones.flatMap((zone) => cellPositions(sheetId, zone))) {
      const cellBorder = this[borderKey][sheetId]?.[position.col]?.[position.row];
      if (cellBorder) {
        continue;
      }
      ...
      defaults.push([position, EMPTY_BORDER]);
    }
    return defaults;
  }
  ...
  getCellBorder({ sheetId, col, row }: CellPosition): Border | null {
    const left = toDescr(this.getBorderValue("LEFT", sheetId, col, row), undefinedIfExternal);
    const right = toDescr(this.getBorderValue("LEFT", sheetId, col + 1, row), undefinedIfInternal);
    const top = toDescr(this.getBorderValue("TOP", sheetId, col, row), undefinedIfExternal);
    const bottom = toDescr(this.getBorderValue("TOP", sheetId, col, row + 1), undefinedIfInternal);
  ...
  getBordersColors(sheetId: UID): Color[] {
    ...
    const tops = Object.values(this.bordersTop[sheetId] ?? {})
      ...
      .map((b) => b.color);
    colors = colors.concat(tops);

    const lefts = Object.values(this.bordersLeft[sheetId] ?? {})
      .filter(isDefined)
      .flatMap((r) => Object.values(r).filter(isDefined))
      .map((b) => b.color);
    colors = colors.concat(lefts);
  ...
  getBorderClipboardData(sheetId: UID, zone: Zone) {
    ...
    if (defaultTop.rowDefault && defaultTop.rowDefault[0]?.internal === "external") {
      defaultTop.rowDefault[0] = undefined;
    }
    ...
    if (defaultLeft.colDefault && defaultLeft.colDefault[0]?.internal === "external") {
      defaultLeft.colDefault[0] = undefined;
    }
  ...
  private setOrBoth(
    ...
    const borderDict = borderType === "TOP" ? "defaultTop" : "defaultLeft";
    const dimArray = dim === "COL" ? "colDefault" : "rowDefault";
    const currentBorder = this?.[borderDict]?.[sheetId]?.[dimArray]?.[index];
    if (
      currentBorder &&
      currentBorder.internal !== border.internal &&
      border.style === currentBorder.style &&
      border.color === currentBorder.color
    ) {
      this.history.update(borderDict, sheetId, dimArray, index, both(border));
    } else {
      this.history.update(borderDict, sheetId, dimArray, index, border);
    }
  }
  ...
  import(data: WorkbookData) {
    ...
    if (Object.keys(data.borders || {}).length) {
      for (const sheet of data.sheets) {
        ...
        // Default Borders
        if (sheet.defaultBorder) {
          const { sheetDefault, colDefault, rowDefault } = sheet.defaultBorder;
          if (sheetDefault !== undefined) {
            const border = data.borders[sheetDefault];
            this.history.update(
              "defaultLeft",
              sheetId,
              "sheetDefault",
              deepEquals(border.left, border.right)
                ? both(border.left)
                : border.left
                ? internal(border.left)
                : external(border.right)
            );
            this.history.update(
              "defaultTop",
              sheetId,
              "sheetDefault",
              deepEquals(border.top, border.bottom)
                ? both(border.top)
                : border.top
                ? internal(border.top)
                : external(border.bottom)
            );
          }
          for (const [colIndex, borderId] of Object.entries(colDefault ?? {})) {
            if (!borderId) {
              continue;
            }
            const col = parseInt(colIndex);
            ...
            if (border.left !== undefined) {
              this.setOrBoth("LEFT", sheetId, "COL", col, internal(border.left));
            }
          ...
          for (const [rowIndex, borderId] of Object.entries(rowDefault ?? {})) {
            ...
            if (border.bottom !== undefined) {
              this.setOrBoth("TOP", sheetId, "ROW", row + 1, external(border.bottom));
            }
          }
        }
  ...
  _export(data: WorkbookData, options: { fullBorder: boolean } = { fullBorder: false }) {
    ...
    for (const sheet of data.sheets) {
      ...
      for (const [col, value] of Object.entries(this.defaultTop?.[sheet.id]?.colDefault ?? {})) {
        const descr = toDescr(value, nullOnEmpty);
        if (value?.internal !== "external") {
          defaults.colDefault[col] ??= {};
          defaults.colDefault[col]["top"] = descr;
        }
        if (value?.internal !== "internal") {
          defaults.colDefault[col] ??= {};
          defaults.colDefault[col]["bottom"] = descr;
        }
      }

      for (const [col, value] of Object.entries(this.defaultLeft?.[sheet.id]?.colDefault ?? {})) {
        const descr = toDescr(value, nullOnEmpty);
        if (value?.internal !== "external") {
          defaults.colDefault[col] ??= {};
          defaults.colDefault[col]["left"] = descr;
        }
        if (value?.internal !== "internal") {
          const nextCol = parseInt(col) - 1;
          defaults.colDefault[nextCol] ??= {};
          defaults.colDefault[nextCol]["right"] = descr;
        }
      }

      for (const [row, value] of Object.entries(this.defaultTop?.[sheet.id]?.rowDefault ?? {})) {
        const descr = toDescr(value, nullOnEmpty);
        if (value?.internal !== "external") {
          defaults.rowDefault[row] ??= {};
          defaults.rowDefault[row]["top"] = descr;
        }
        if (value?.internal !== "internal") {
          const nextRow = parseInt(row) - 1;
          defaults.rowDefault[nextRow] ??= {};
          defaults.rowDefault[nextRow]["bottom"] = descr;
        }
      }

      for (const [row, value] of Object.entries(this.defaultLeft?.[sheet.id]?.rowDefault ?? {})) {
        const descr = toDescr(value, nullOnEmpty);
        if (value?.internal !== "external") {
          defaults.rowDefault[row] ??= {};
          defaults.rowDefault[row]["left"] = descr;
        }
        if (value?.internal !== "internal") {
          defaults.rowDefault[row] ??= {};
          defaults.rowDefault[row]["right"] = descr;
        }
      }
