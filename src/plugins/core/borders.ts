...
export class BordersPlugin extends CorePlugin<BordersPluginState> implements BordersPluginState {
  ...
  handle(cmd: CoreCommand) {
    switch (cmd.type) {
      ...
      case "REMOVE_COLUMNS_ROWS":
        const elements = [...cmd.elements].sort();
  ...
  getBordersColors(sheetId: UID): Color[] {
    ...
    return new Array(...new Set(colors)).filter((c) => c);
  }
  ...
  getBorderClipboardData(sheetId: UID, zone: Zone) {
    ...
    if (
      defaultTop.rowDefault &&
      defaultTop.rowDefault[defaultTop.rowDefault.length]?.internal === "internal"
    ) {
      defaultTop.rowDefault[defaultTop.rowDefault.length] = undefined;
    }
    ...
    if (
      defaultLeft.colDefault &&
      defaultLeft.colDefault[defaultLeft.colDefault.length]?.internal === "internal"
    ) {
      defaultLeft.colDefault[defaultLeft.colDefault.length] = undefined;
    }
  ...
  _export(data: WorkbookData, options: { fullBorder: boolean } = { fullBorder: false }) {
    ...
    for (const sheet of data.sheets) {
      ...
      for (const [row, value] of Object.entries(this.defaultTop?.[sheet.id]?.rowDefault ?? {})) {
        ...
        if (value?.internal !== "internal") {
          const nextCol = parseInt(row) - 1;
          defaults.rowDefault[nextCol] ??= {};
          defaults.rowDefault[nextCol]["bottom"] = descr;
        }
      }
