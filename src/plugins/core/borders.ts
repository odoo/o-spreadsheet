...
export class BordersPlugin extends CorePlugin<BordersPluginState> implements BordersPluginState {
  ...
  handle(cmd: CoreCommand) {
    switch (cmd.type) {
      ...
      case "SET_ZONE_BORDERS":
        ...
        break;
      case "SET_ZONE_BORDERDATA":
        const borderTarget = cmd.target.map((zone) => this.getters.expandZone(cmd.sheetId, zone));
        this.addBorders(cmd.sheetId, borderTarget, cmd.border);
        break;
  ...
  getZoneExternalBorders(sheetId: UID, zone: Zone): ZoneBorderData {
    ...
    for (const border of this.borders[sheetId] ?? []) {
      ...
      for (let col = inter.left; col <= inter.right; col++) {
        for (let row = inter.top; row <= inter.bottom; row++) {
          ...
          cell.bottom = (row === bzone.bottom ? bborder.bottom : bborder.horizontal) ?? cell.bottom;
  ...
  private addBorder(
    ...
    for (const existingBorder of this.borders[sheetId] ?? []) {
      ...
      if (plannedBorder) {
        ...
        if (!force) {
          border = { ...this.computeBorderFromZone(inter, existingBorder).style, ...border };
        }
  ...
  private minimizeZone(zone: Zone, border: ZoneBorderData): Zone {
