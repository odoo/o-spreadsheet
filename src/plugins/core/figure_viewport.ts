...
export class FigureViewportPlugin extends CorePlugin<State> implements State {
  ...
  import(data: WorkbookData) {
    for (const sheet of data.sheets) {
      const figureViewports = (sheet.figures || []).filter((figure) => figure.tag === "viewport");
      for (const figureViewport of figureViewports) {
        this.history.update("figureViewports", sheet.id, figureViewport.id, figureViewport.data);
      }
    }
  }
  ...
  export(data: WorkbookData) {
    for (const sheet of data.sheets) {
      const figureViewports = sheet.figures.filter((figure) => figure.tag === "viewport");
      for (const figureViewport of figureViewports) {
        figureViewport.data = this.figureViewports[sheet.id]?.[figureViewport.id];
      }
    }
  }
}
