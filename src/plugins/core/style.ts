...
export class StylePlugin extends CorePlugin<StylePluginState> implements StylePluginState {
  ...
  handle(cmd: CoreCommand) {
    switch (cmd.type) {
      ...
      case "SET_FORMATTING":
        if ("style" in cmd) {
          if (cmd.style) this.setStyles(cmd.sheetId, cmd.target, cmd.style);
      ...
      case "UPDATE_CELL":
        if ("style" in cmd) {
          if (cmd.style) this.setStyles(cmd.sheetId, [positionToZone(cmd)], cmd.style);
  ...
  private onMerge(sheetId: UID, zone: Zone) {
    this.setStyle(
      sheetId,
      zone,
      this.getCellStyle({ sheetId, col: zone.left, row: zone.top }),
      true
    );
  }

  private setStyles(sheetId: UID, zones: Zone[], style: Style | undefined, force = false) {
    for (const zone of zones) {
      this.setStyle(sheetId, zone, style, force);
    }
  }

  private setStyle(sheetId: UID, zone: Zone, style: Style | undefined, force = false) {
    const styles: ZoneStyle[] = [];
    let editingZone: Zone[] = [zone];
    editingZone.push(...this.getters.getMergesZoneWithTopLeftInZone(sheetId, zone));
    for (const existingStyle of this.styles[sheetId] ?? []) {
      ...
      let newStyle = force ? style : { ...existingStyle.style, ...style };
  ...
  private clearStyle(sheetId: UID, zones: Zone[]) {
    this.setStyles(sheetId, zones, undefined, true);
  }
  ...
  import(data: WorkbookData) {
    if (Object.keys(data.styles || {}).length) {
      for (const sheet of data.sheets) {
        for (const [zone, styleId] of iterateItemIdsZone(sheet.id, sheet.styles)) {
