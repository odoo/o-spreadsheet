...
export class ViewportPlugin extends UIPlugin {
  static getters = ["getActiveViewport", "getViewport", "getViewportDimension", "getGridDimension"];
  ...
  handle(cmd: Command) {
    switch (cmd.type) {
      ...
      case "REDO":
        this.cleanViewports();
        this.resetViewports();
        this._scroll(this.getters.getActiveSheetId());
        break;
      case "RESIZE_VIEWPORT":
        this.cleanViewports();
  ...
  getViewport(sheetId: UID): Viewport {
  ...
  private _scroll(sheetId: UID) {
    ...
    this.ui.modelBus.trigger("set-scrollbar-values", { offsetX, offsetY });
  }
