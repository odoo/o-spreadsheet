...
import { isZoneValid, zoneToXc } from "../../../helpers/zones";
    ...
    if (subWidth < 1 || subHeight < 1) {
      throw new ReferenceError(
        _t(
          ...
          { rangeName: zoneToXc(range.zone), nCols, nRows }
        )
      );
    }

    const top = range.zone.top + zone.top;
    const left = range.zone.left + zone.left;
