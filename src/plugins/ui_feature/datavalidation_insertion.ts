  ...
  allowDispatch(command: Command): CommandResult | CommandResult[] {
    switch (command.type) {
      case "ADD_DATA_VALIDATION_RULES": {
        ...
        for (const sheetId in command.rulesBySheetId) {
          results.push(
            this.canDispatch("ADD_DATA_VALIDATION_RULE", {
              sheetId,
              rule: command.rulesBySheetId[sheetId].rule,
              ranges: command.rulesBySheetId[sheetId].ranges,
            })
          );
        }
  ...
  handle(cmd: Command) {
    switch (cmd.type) {
      case "ADD_DATA_VALIDATION_RULE":
        ...
        break;
      case "ADD_DATA_VALIDATION_RULES":
        for (const sheetId in cmd.rulesBySheetId) {
          this.dispatch("ADD_DATA_VALIDATION_RULE", {
            sheetId,
            ranges: cmd.rulesBySheetId[sheetId].ranges,
            rule: cmd.rulesBySheetId[sheetId].rule,
          });
        }
        break;
    }
