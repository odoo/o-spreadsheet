...
export class ClipboardPlugin extends UIPlugin {
  ...
  private getSerializedGridData(): string {
    const zones = this.getters.getSelectedZones();
    const clipboardData = this.getClipboardData(zones);
    let copiedData: MinimalClipboardData = {};
    for (const handler of this.selectClipboardHandlers(clipboardData)) {
      const data = handler.copy(clipboardData);
      copiedData = { ...copiedData, ...data };
    }
    copiedData.version = CURRENT_VERSION;
    copiedData.clipboardId = this.clipboardId;
    ...
    return (
      this.copiedData.cells
        .map((cells) => {
          return cells
            .map((c) =>
              this.getters.shouldShowFormulas() && c?.tokens && c.tokens.length > 0
                ? c?.content || ""
  ...
  private getHTMLContent(): string | undefined {
    ...
    let htmlTable = `<table border="1" style="border-collapse:collapse" data-clipboard-id="${this.clipboardId}">`;
