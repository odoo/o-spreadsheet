...
export class SheetViewPlugin extends UIPlugin {
  ...
  getVisibleFigures(): FigureUI[] {
    ...
    for (const figure of figures) {
      ...
      const { x, y } = figureUI;
      if (x + figure.width < offsetCorrectionX || x > width) {
        continue;
      }
      if (y + figure.height < offsetCorrectionY || y > height) {
        continue;
      }
  ...
  private absoluteToViewport(position: PixelPosition): DOMCoordinates {
    const { scrollX, scrollY } = this.getActiveSheetScrollInfo();
    const { x: offsetCorrectionX, y: offsetCorrectionY } = this.getMainViewportCoordinates();
    return {
      x: position.x < offsetCorrectionX ? position.x : position.x - scrollX,
      y: position.y < offsetCorrectionY ? position.y : position.y - scrollY,
    };
  }

  private viewportToAbsolute(coord: DOMCoordinates): PixelPosition {
    const { scrollX, scrollY } = this.getActiveSheetScrollInfo();
    const { x: offsetCorrectionX, y: offsetCorrectionY } = this.getMainViewportCoordinates();
    return {
      x: coord.x < offsetCorrectionX ? coord.x : coord.x + scrollX,
      y: coord.y < offsetCorrectionY ? coord.y : coord.y + scrollY,
    };
  }

  getFigureUI(sheetId: string, figure: Figure): FigureUI {
    const absolute_x =
      figure.offset.x + this.getters.getColDimensions(sheetId, figure.anchor.col)["start"];
    const absolute_y =
      figure.offset.y + this.getters.getRowDimensions(sheetId, figure.anchor.row)["start"];
    const { x, y } = this.absoluteToViewport({ x: absolute_x, y: absolute_y });
    return { x, y, figure };
  }

  getAnchorOffset(sheetId: string, figureUI: FigureUI): AnchorOffset {
    const position = this.viewportToAbsolute(figureUI);
    if (figureUI.figure.fixed_position) {
      return { anchor: { col: 0, row: 0 }, offset: { x: position.x, y: position.y } };
    }
    const anchor_col = this.getColIndex(figureUI.x);
    const offset_x = position.x - this.getters.getColDimensions(sheetId, anchor_col)["start"];
    const anchor_row = this.getRowIndex(figureUI.y);
    const offset_y = position.y - this.getters.getRowDimensions(sheetId, anchor_row)["start"];
