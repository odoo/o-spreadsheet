...
export class SheetViewPlugin extends UIPlugin {
  ...
  private getColIndexLeftOfMainViewport(x: Pixel): HeaderIndex {
    ...
    let colStart = this.getters.getColRowOffset("COL", col, 0);
  ...
  private getRowIndexTopOfMainViewport(y: Pixel): HeaderIndex {
    ...
    let rowStart = this.getters.getColRowOffset("ROW", row, 0);

    for (; y < rowStart; row--) {
      rowStart -= this.getters.getColSize(sheetId, row - 1);
    }
    return row;
  }
  ...
  getPositionAnchorOffset(position: PixelPosition): AnchorOffset {
    ...
    return {
      ...
      row,
      offset: {
        x: x - colX + this.gridOffsetX,
        y: y - rowY + this.gridOffsetY,
      },
    };
  }
