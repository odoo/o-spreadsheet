...
autofillModifiersRegistry
  .add("ALPHANUMERIC_INCREMENT_MODIFIER", {
    apply: (rule: AlphanumericIncrementModifier, originCell: Cell) => {
  ...
  .add("INCREMENT_MODIFIER", {
    apply: (rule: IncrementModifier, originCell: Cell, getters: Getters) => {
      rule.current += rule.increment;
  ...
  .add("DATE_INCREMENT_MODIFIER", {
    apply: (rule: DateIncrementModifier, originCell: Cell, getters: Getters) => {
  ...
  .add("COPY_MODIFIER", {
    apply: (rule: CopyModifier, originCell: Cell, getters: Getters) => {
  ...
  .add("FORMULA_MODIFIER", {
    apply: (rule: FormulaModifier, originCell: Cell, getters: Getters, direction: DIRECTION) => {
      rule.current += rule.increment;
      let x = 0;
      let y = 0;
      switch (direction) {
