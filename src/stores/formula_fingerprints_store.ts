import { ColorGenerator, RangeImpl, reorderZone, setColorAlpha } from "../helpers";
...
/**
 * A string that represents a reference vector.
 */
...
export class FormulaFingerprintStore extends SpreadsheetStore {
  ...
  private computeFingerprints() {
    ...
    const fingerprintColors = this.assignColors(allFingerprints);
    for (const [position, fingerprint] of fingerprints.entries()) {
      const color = fingerprintColors[fingerprint];
      this.map.set(position, color);
  ...
  private colorSpreadZone(position: CellPosition, fingerprintColor: Color) {
    ...
    for (let row = spreadZone.top; row <= spreadZone.bottom; row++) {
      for (let col = spreadZone.left; col <= spreadZone.right; col++) {
        ...
        this.map.set(spreadPosition, fingerprintColor);
      }
    }
  }

  private assignColors(fingerprints: Set<Fingerprint>): Record<Fingerprint, Color> {
    const fingerprintColors = {
      [DATA_FINGERPRINT]: "#D9D9D9",
    };
    fingerprints.delete(DATA_FINGERPRINT);
    const colors = new ColorGenerator(fingerprints.size);
    for (const fingerprint of fingerprints) {
      fingerprintColors[fingerprint] = setColorAlpha(colors.next(), 0.8);
    }
    return fingerprintColors;
  }
