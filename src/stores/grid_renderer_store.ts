...
export class GridRenderer extends DisposableStore {
  ...
  protected getters: CustomGetters;
  ...
  drawLayer(
    ...
    switch (layer) {
      case "Background":
        ...
        break;
      case "Headers":
        if (!renderingContext.hideHeaders) {
  ...
  private drawBackground(renderingContext: GridRenderingContext, boxes: Box[]) {
    ...
    const areGridLinesVisible =
      !this.getters.isDashboard() && this.getters.getGridLinesVisibility(renderingContext.sheetId);
  ...
  private getGridBoxes(ctx: GridRenderingContext, zone: Zone): Box[] {
    ...
    for (const row of visibleRows) {
      for (const col of visibleCols) {
        ...
        boxes.push(this.createZoneBox(ctx, sheetId, positionToZone(position), viewport));
      }
    }
    for (const merge of this.getters.getMerges(sheetId)) {
      ...
      if (overlap(merge, viewport)) {
        const box = this.createZoneBox(ctx, sheetId, merge, viewport);
