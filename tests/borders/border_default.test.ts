import { Border, Model, UID, Zone } from "@odoo/o-spreadsheet-engine";
...
describe("Default Borders", () => {
  ...
  describe.each(TEST_BORDERS)("Border : %s", (border) => {
    ...
    test("Can set border on row", () => {
      ...
      expect(getCellBorder(model, "A1")).toEqual(border);
      expect(getCellBorder(model, "D2")).toEqual(border);
    ...
    test("Can set border on col", () => {
      ...
      expect(getCellBorder(model, "A1")).toEqual(border);
      expect(getCellBorder(model, "B5")).toEqual(border);
      expect(getCellBorder(model, "D5")).toBeNull();
    });
  });
  ...
  describe("Sheet Manipulation: Delete Cell Left", () => {
    ...
    test("Default Sheet", () => {
      ...
      expect(getCellBorder(model, "A2")).toEqual(TOP_BORDER);
      expect(getCellBorder(model, "B2")).toEqual(TOP_BORDER);
      expect(getCellBorder(model, "Y2")).toBeNull();
    });
  });
  ...
  ] as [Zone, Border][][])("Clipboard : copy partial sheet (border)", (...commands) => {
  ...
  test.each([
  ...
  ] as [Zone, Border][][])("Clipboard : cut partial sheet (border)", (...commands) => {
    const handlers = clipboardHandlersRegistries.cellHandlers
      .getAll()
      .map((handler) => new handler(model.getters, model.dispatch));
    ...
    for (let col = 1; col < 4; col++) {
      gridState.push([]);
      for (let row = 1; row < 4; row++) {
        gridState[col - 1][row - 1] = getCellBorder(model, toXC(col, row));
      }
    }
    ...
    for (let col = 1; col < 4; col++) {
      for (let row = 1; row < 4; row++) {
        expect(getCellBorder(model, toXC(col, row))).toBeNull();
      }
    }
    expect(getCellBorder(model, toXC(5, 5))).toEqual(gridState[0][0]);
    expect(getCellBorder(model, toXC(5, 6))).toEqual(gridState[0][1]);
    expect(getCellBorder(model, toXC(5, 7))).toEqual(gridState[0][2]);

    expect(getCellBorder(model, toXC(6, 5))).toEqual(gridState[1][0]);
    expect(getCellBorder(model, toXC(6, 6))).toEqual(gridState[1][1]);
    expect(getCellBorder(model, toXC(6, 7))).toEqual(gridState[1][2]);

    expect(getCellBorder(model, toXC(7, 5))).toEqual(gridState[2][0]);
    expect(getCellBorder(model, toXC(7, 6))).toEqual(gridState[2][1]);
    expect(getCellBorder(model, toXC(7, 7))).toEqual(gridState[2][2]);
  });
  ...
  ] as [Zone, Border][][])("Clipboard : copy whole sheet (border)", (...commands) => {
    const handlers = clipboardHandlersRegistries.cellHandlers
      .getAll()
      .map((handler) => new handler(model.getters, model.dispatch));
