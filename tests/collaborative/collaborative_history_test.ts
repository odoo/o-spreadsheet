...
import "../canvas.mock";
import { addColumns, createSheet, redo, setCellContent, undo } from "../commands_helpers";
...
describe("Collaborative local history", () => {
  ...
  test("trying to reproduce", () => {
    alice.dispatch("ADD_COLUMNS", {
    ...
    expect([alice, bob, charly]).toHaveSynchronizedValue((user) => getCell(alice, "N2"), undefined);
  });
