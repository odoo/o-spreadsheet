...
describe("Collaborative global history", () => {
  ...
  test("Undo two commands from differents users", () => {
    ...
    redo(bob);
    expect(all).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "hello");
    expect(all).toHaveSynchronizedValue((user) => getCellContent(user, "A2"), "hello");
...
describe.skip("Collaborative local history", () => {
  let network: MockNetwork;
  let alice: Model;
  let bob: Model;
  let charly: Model;
  ...
  test("Undo is propagated to other clients", () => {
    ...
    undo(alice);
    expect(spy).toHaveBeenCalledTimes(2); // [UPDATE_CELL - MOVE_POSITION]
