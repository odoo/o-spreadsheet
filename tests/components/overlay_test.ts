...
function selectColumn(letter: string, extra: any = {}) {
  ...
  const x = model.getters.getCols()[index].start + 1;
...
async function dblClickColumn(letter: string) {
  const index = lettersToNumber(letter);
  const x = model.getters.getCols()[index].end;
  triggerMouseEvent(".o-overlay .o-col-resizer", "mousemove", x, 10);
  await nextTick();
...
function selectRow(index: number, extra: any = {}) {
  const y = model.getters.getRows()[index].start + 1;
...
async function dblClickRow(index: number) {
  const y = model.getters.getRows()[index].end;
  triggerMouseEvent(".o-overlay .o-row-resizer", "mousemove", 10, y);
  await nextTick();
...
describe("Resizer component", () => {
  ...
  test("Can resize a column", async () => {
    ...
    expect(model.getters.getCols()[1].size).toBe(model.getters.getCols()[0].size + 50);
    expect(model.getters.getCols()[2].size).toBe(model.getters.getCols()[0].size);
  });

  test("Can resize a row", async () => {
    await resizeRow(2, 50);
    expect(model.getters.getRows()[1].size).toBe(model.getters.getRows()[0].size + 50);
    expect(model.getters.getRows()[2].size).toBe(model.getters.getRows()[0].size);
  });
  ...
  test("Can resize multiples columns", async () => {
    ...
    expect(model.getters.getCols()[1].size).toBe(model.getters.getCols()[0].size);
    expect(model.getters.getCols()[2].size).toBe(model.getters.getCols()[0].size + 50);
    expect(model.getters.getCols()[3].size).toBe(model.getters.getCols()[0].size + 50);
    expect(model.getters.getCols()[4].size).toBe(model.getters.getCols()[0].size);
    expect(model.getters.getCols()[4].start).toBe(model.getters.getCols()[0].size * 4 + 100);
  });
  ...
  test("Can resize multiples rows", async () => {
    ...
    expect(model.getters.getRows()[1].size).toBe(model.getters.getRows()[0].size);
    expect(model.getters.getRows()[2].size).toBe(model.getters.getRows()[0].size + 50);
    expect(model.getters.getRows()[3].size).toBe(model.getters.getRows()[0].size + 50);
    expect(model.getters.getRows()[4].size).toBe(model.getters.getRows()[0].size);
    expect(model.getters.getRows()[4].start).toBe(model.getters.getRows()[0].size * 4 + 100);
  });
  ...
  test("Double click: Modify the size of a row", async () => {
    model.dispatch("SET_VALUE", { xc: "B2", text: "b2" });
    await dblClickRow(1);
    expect(model.getters.getRows()[1].size).toBe(21);
  });
  ...
  test("Select ABC E, dblclick E then resize all", async () => {
    ...
    expect(model.getters.getCols()[0].size).toBe(1006);
    expect(model.getters.getCols()[1].size).toBe(1006);
    expect(model.getters.getCols()[2].size).toBe(1006);
    expect(model.getters.getCols()[3].size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getCols()[4].size).toBe(1006);
  });
  ...
  test("Select ABC E, dblclick F then resize only F", async () => {
    ...
    expect(model.getters.getCols()[0].size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getCols()[1].size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getCols()[2].size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getCols()[3].size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getCols()[4].size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getCols()[5].size).toBe(1006);
  });
  ...
  test("Select 123 5, dblclick 5 then resize all", async () => {
    ...
    selectRow(4, { ctrlKey: true });
    await dblClickRow(4);
    expect(model.getters.getRows()[0].size).toBe(21);
    expect(model.getters.getRows()[1].size).toBe(21);
    expect(model.getters.getRows()[2].size).toBe(21);
    expect(model.getters.getRows()[3].size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRows()[4].size).toBe(21);
  });
  ...
  test("Select 123 5, dblclick 6 then resize only 6", async () => {
    ...
    selectRow(4, { ctrlKey: true });
    await dblClickRow(5);
    expect(model.getters.getRows()[0].size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRows()[1].size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRows()[2].size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRows()[3].size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRows()[4].size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRows()[5].size).toBe(21);
  });

  test("Select A, drag to C then ABC selected", async () => {
    let x = model.getters.getCols()[0].start + 1;
    triggerMouseEvent(".o-overlay .o-col-resizer", "mousedown", x, 10);
    x = model.getters.getCols()[2].start + 1;
    triggerMouseEvent(window, "mousemove", x, 10, { buttons: 1 });
    expect(model.getters.getActiveCols()).toEqual(new Set([0, 1, 2]));
  });

  test("Select 1, drag to 3 then 123 selected", async () => {
    let y = model.getters.getRows()[0].start + 1;
    triggerMouseEvent(".o-overlay .o-row-resizer", "mousedown", 10, y);
    y = model.getters.getRows()[2].start + 1;
    triggerMouseEvent(window, "mousemove", 10, y, { buttons: 1 });
    expect(model.getters.getActiveRows()).toEqual(new Set([0, 1, 2]));
  });

  test("right click after last column does not open context menu", async () => {
    const nCols = model.getters.getCols().length;
    const x = model.getters.getCols()[nCols - 1].end + 1;
  ...
  test("right click after last row does not open context menu", async () => {
    const nRows = model.getters.getRows().length;
    const y = model.getters.getRows()[nRows - 1].end + 1;
