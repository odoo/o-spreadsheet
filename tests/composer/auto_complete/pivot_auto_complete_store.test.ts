...
describe("spreadsheet pivot auto complete", () => {
  ...
  test("PIVOT.VALUE autocomplete date day_of_week field for group value", async () => {
    ...
    updatePivot(model, "1", {
      columns: [],
      rows: [{ name: "Created on", granularity: "day_of_week" }],
      measures: [{ name: "Expected Revenue", aggregator: "sum" }],
    });
    const { store: composer } = makeStoreWithModel(model, ComposerStore);
  ...
  test("PIVOT.VALUE autocomplete date hour_number field for group value", async () => {
    ...
    updatePivot(model, "1", {
      columns: [],
      rows: [{ name: "Created on", granularity: "hour_number" }],
      measures: [{ name: "Expected Revenue", aggregator: "sum" }],
    });
    const { store: composer } = makeStoreWithModel(model, ComposerStore);
  ...
  test("PIVOT.VALUE autocomplete date minute_number field for group value", async () => {
    ...
    updatePivot(model, "1", {
      columns: [],
      rows: [{ name: "Created on", granularity: "minute_number" }],
      measures: [{ name: "Expected Revenue", aggregator: "sum" }],
    });
    const { store: composer } = makeStoreWithModel(model, ComposerStore);
  ...
  test("PIVOT.VALUE autocomplete date second_number field for group value", async () => {
    ...
    updatePivot(model, "1", {
      columns: [],
      rows: [{ name: "Created on", granularity: "second_number" }],
      measures: [{ name: "Expected Revenue", aggregator: "sum" }],
    });
    const { store: composer } = makeStoreWithModel(model, ComposerStore);
