...
describe("Composer hover integration test", () => {
  ...
  test("Can hover functions that require the cell position as context in a standalone composer", async () => {
    ...
    await hoverComposerContent("ROW");
    expect(".o-speech-bubble").toHaveText('"(row)"');
    await hoverComposerContent("COLUMN");
    expect(".o-speech-bubble").toHaveText('"(column)"');
    await hoverComposerContent("=");
    expect(".o-speech-bubble").toHaveText('"(row)"');
  });
});
