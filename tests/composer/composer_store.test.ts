...
<<<<<<< master
  ...
  test("click on a pivot value cell insert the formula", () => {
    ...
    selectCell(model, "B4");
    expect(store.currentContent).toBe('=PIVOT.VALUE(1,"__count:sum","Name","Bob")');

    // click on total value
    selectCell(model, "B5");
    expect(store.currentContent).toBe('=PIVOT.VALUE(1,"__count:sum")');
  });
  ...
  test("click on a pivot value inserts the formula in the right locale", () => {
    ...
    addPivot(model, "D1:D4", {
      columns: [],
      rows: [{ fieldName: "Name" }],
      measures: [{ id: "count", fieldName: "__count", aggregator: "sum" }],
    });
    setCellContent(model, "A1", "=PIVOT(1)");
    const { store } = makeStoreWithModel(model, CellComposerStore);
    store.startEdition("=");
    selectCell(model, "B3");
    expect(store.currentContent).toBe('=PIVOT.VALUE(1;"count";"Name";"Alice")');
  });
  ...
  test("select an empty pivot value inserts the pivot function", () => {
    ...
    selectCell(model, "B4");
    expect(store.currentContent).toBe('=PIVOT.VALUE(1,"Price:sum","Name","Bob")');
  });
...
=======
>>>>>>> c604fc920cd345242b1df763749dbb768be4f3c8
});
