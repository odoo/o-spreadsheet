...
describe("Edit criterion in side panel", () => {
  describe("Value in list", () => {
    beforeEach(async () => {
      ...
      env.getStore(SidePanelStore).open("DataValidationEditor", { ruleId: "id", sheetId });
      await nextTick();
    });
    ...
    test("Side panel is correctly pre-filled for composer criterion", async () => {
      ...
      env.getStore(SidePanelStore).open("DataValidationEditor", { ruleId: "dv1", sheetId });
      await nextTick();
    ...
    test("color should follow updated value", async () => {
      ...
      // change "ok" to "bye"
      const inputs = fixture.querySelectorAll<HTMLInputElement>(".o-dv-list-values .o-input");
      await setInputValueAndTrigger(inputs[0], "bye");
    ...
    test("Can store values and color with leading and trailing spaces", async () => {
      const inputs = fixture.querySelectorAll<HTMLInputElement>(".o-dv-list-values .o-input");
      await setInputValueAndTrigger(inputs[0], "  hello  ");
      await click(fixture.querySelector(".o-round-color-picker-button")!);
      await click(fixture, ".o-color-picker-line-item[data-color='#CFE2F3'");
      await click(fixture, ".o-dv-save");
