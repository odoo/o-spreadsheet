import { ClipboardPasteTarget, Format, Model, UID, Zone } from "../../src";
...
describe("Default Plugin: Format", () => {
  ...
  describe.each(TEST_FORMATS)("Format : %s", (format) => {
    test("Can set format on sheet", () => {
      setFormat(model, [model.getters.getSheetZone(sheetId)], format);
    ...
    test("Can set format on row", () => {
      setFormat(model, [model.getters.getRowsZone(sheetId, 0, 2)], format);
      expect(model.getters.getCells(sheetId).length).toBe(0);
      expect(getCellFormat(model, "A1")).toEqual(format);
    ...
    test("Can set format on col", () => {
      setFormat(model, [model.getters.getColsZone(sheetId, 0, 2)], format);
      expect(model.getters.getCells(sheetId).length).toBe(0);
      expect(getCellFormat(model, "A1")).toEqual(format);
  ...
  describe.each([
    ...
    test("Row after sheet", () => {
      ...
      setFormat(model, [model.getters.getSheetZone(sheetId)], sheetFormat);
      setFormat(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowFormat);
    ...
    test("Row after col", () => {
      ...
      const rowFormat = format2;
      setFormat(model, [model.getters.getColsZone(sheetId, 1, 1)], colFormat);
      setFormat(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowFormat);
    ...
    test("Row after row", () => {
      ...
      setFormat(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowFormat);
      setFormat(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowFormat2);
    ...
    test("Row after cell", () => {
      ...
      setFormat(model, target("B2"), cellFormat);
      setFormat(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowFormat);
    ...
    test("Col after col", () => {
      ...
      setFormat(model, [model.getters.getColsZone(sheetId, 1, 1)], colFormat);
      setFormat(model, [model.getters.getColsZone(sheetId, 1, 1)], colFormat2);
    ...
    test("Sheet after sheet", () => {
      ...
      setFormat(model, [model.getters.getSheetZone(sheetId)], sheetFormat);
      setFormat(model, [model.getters.getSheetZone(sheetId)], sheetFormat2);
...
describe("Default Plugin: setRowsFormat preserves cells outside the zone", () => {
  // A zone spanning >50% of columns triggers setRowsFormat (stored as rowDefault).
  // Cells in the same rows but OUTSIDE the zone's column range must have their
  // old effective format preserved as an explicit cell format so that the new rowDefault
  // (which applies to the whole row) does not visually affect them.
  ...
  test("cells outside the zone column range keep their col-default format after setRowsFormat", () => {
    ...
    expect(getCellFormat(model, "F3")).toBe(PERCENT_FORMAT);

    // Col I (col 8) is outside the zone; rows 0-2 must keep the colDefault, not PERCENT_FORMAT
