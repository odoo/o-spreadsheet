...
describe("Default Plugin: Style", () => {
  ...
  describe.each(TEST_STYLES)("Style : %s", (style) => {
    test("Can set style on sheet", () => {
      setStyle(model, [model.getters.getSheetZone(sheetId)], style);
    ...
    test("Can set style on row", () => {
      setStyle(model, [model.getters.getRowsZone(sheetId, 0, 2)], style);
    ...
    test("Can set style on col", () => {
      setStyle(model, [model.getters.getColsZone(sheetId, 0, 2)], style);
  ...
  describe.each([
    ...
    test("Row after sheet", () => {
      ...
      setStyle(model, [model.getters.getSheetZone(sheetId)], sheetStyle);
      setStyle(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowStyle);
    ...
    test("Row after col", () => {
      ...
      setStyle(model, [model.getters.getColsZone(sheetId, 1, 1)], colStyle);
      setStyle(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowStyle);
    ...
    test("Row after row", () => {
      ...
      setStyle(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowStyle);
      setStyle(model, [model.getters.getRowsZone(sheetId, 1, 1)], rowStyle2);
    ...
    test("Row after cell", () => {
      ...
      setStyle(model, [toZone("B2")], cellStyle);
    ...
    test("Col after col", () => {
      ...
      setStyle(model, [model.getters.getColsZone(sheetId, 1, 1)], colStyle);
      setStyle(model, [model.getters.getColsZone(sheetId, 1, 1)], colStyle2);
    ...
    test("Sheet after sheet", () => {
      ...
      setStyle(model, [model.getters.getSheetZone(sheetId)], sheetStyle);
      setStyle(model, [model.getters.getSheetZone(sheetId)], sheetStyle2);
  ...
  describe("Sheet Manipulation: Add Column", () => {
    ...
    test("Default Col", () => {
      setStyle(model, [model.getters.getColsZone(sheetId, 1, 1)], MULTI_STYLE);
    ...
    test("Default Sheet", () => {
      setStyle(model, [model.getters.getSheetZone(sheetId)], MULTI_STYLE);
  ...
  describe("Merge: Merge into unmerge propagate style", () => {
    test("Default Row", () => {
      setStyle(model, [model.getters.getRowsZone(sheetId, 1, 1)], MULTI_STYLE);
      expect(getCellStyle(model, "C2")).toEqual(MULTI_STYLE);
      expect(getCellStyle(model, "C3")).toEqual({});
      expect(getCellStyle(model, "C4")).toEqual({});

      merge(model, "C2:C4");
      expect(getCellStyle(model, "C2")).toEqual(MULTI_STYLE);
      expect(getCellStyle(model, "C3")).toEqual(MULTI_STYLE);
      expect(getCellStyle(model, "C4")).toEqual(MULTI_STYLE);
      expect(model.getters.getCells(sheetId).length).toBe(2);
    ...
    test("Default Col", () => {
      setStyle(model, [model.getters.getColsZone(sheetId, 1, 1)], MULTI_STYLE);
      expect(getCellStyle(model, "B2")).toEqual(MULTI_STYLE);
      expect(getCellStyle(model, "C2")).toEqual({});
      expect(getCellStyle(model, "D2")).toEqual({});
...
describe("Default Plugin: setRowsStyle preserves cells outside the zone", () => {
  // A zone spanning >50% of columns triggers setRowsStyle (stored as rowDefault).
  // Cells in the same rows but OUTSIDE the zone's column range must have their
  // old effective style preserved as an explicit cell style so that the new rowDefault
  // does not visually affect them.
  ...
  test("cells outside the zone column range keep their col-default style after setRowsStyle", () => {
    ...
    setStyle(model, [{ left: 0, right: 5, top: 0, bottom: 2 }], FCOLOR_STYLE2);

    // Cells inside the zone get the new style
    expect(getCellStyle(model, "A1")).toEqual(FCOLOR_STYLE2);
    expect(getCellStyle(model, "F3")).toEqual(FCOLOR_STYLE2);

    // Col I (col 8) is outside the zone; rows 0-2 must keep the colDefault, not FCOLOR_STYLE2
