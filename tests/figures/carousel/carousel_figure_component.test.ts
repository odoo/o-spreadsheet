...
describe("Carousel figure component", () => {
  ...
  test("display chart menu", async () => {
    ...
    await click(fixture, ".o-carousel-tab:nth-child(2)");
    expect(".o-chart-menu-item").toHaveCount(1); // ellipsis, no full screen
  });
  ...
  test("Can delete the sheet the data view is on", async () => {
    const sheetId = model.getters.getActiveSheetId();
    createSheet(model, { sheetId: "sheet2" });
    setGrid(model, { A1: "Hello", C3: "World" }, "sheet2");
    createCarouselWithDataView(model, toRangeData("sheet2", "A1:C3"), "carouselId", sheetId);

    await mountSpreadsheet({ model });
    expect(".o-figure .o-standalone-viewport").toHaveCount(1);

    deleteSheet(model, "sheet2");
    await nextTick();
    expect(".o-figure .o-standalone-viewport").toHaveCount(0);
  });
});
