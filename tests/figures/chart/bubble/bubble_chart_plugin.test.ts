...
import { FIRST_CHART_COLOR } from "../../../../src/helpers/color";
...
describe("bubble chart", () => {
  test("create bubble chart from creation context", () => {
    const definition = BubbleChart.getDefinitionFromContextCreation(
      GENERAL_CHART_CREATION_CONTEXT,
  ...
  test("bubble chart runtime uses dedicated ranges and color modes", () => {
    ...
    let dataset = runtime.chartJsConfig.data?.datasets?.[0];
    expect(dataset.data).toHaveLength(4);
    const xValues = dataset.data.map((point: Point) => point.x);
    const yValues = dataset.data.map((point: Point) => point.y);
    ...
    dataset = runtime.chartJsConfig.data?.datasets?.[0];
