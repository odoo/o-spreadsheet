...
describe("datasource tests", function () {
  ...
  test("ranges in definition change automatically", () => {
    ...
    expect(chart.labelRanges?.[0]).toStrictEqual("Sheet1!C2:C4");
  });
  ...
  test("remove labels from existing chart", () => {
    ...
    updateChart(model, "1", { labelRanges: [] });
    expect(
      (model.getters.getChartDefinition("1") as LineChartDefinition).labelRanges?.length
    ).toBeFalsy();
  });
  ...
  test("Chart is copied on sheet duplication", () => {
    ...
    expect(newChart.labelRanges[0]?.sheetId).toEqual(secondSheetId);
    expect(zoneToXc(newChart.labelRanges[0]!.zone)).toEqual("A2:A4");
  ...
  test("Chart on columns deletion", () => {
    ...
    expect(def.labelRanges?.length).toBeFalsy();
  });
});
...
describe("Chart without labels", () => {
  ...
  test("The legend is displayed even when there is only one dataSet or no label", () => {
    ...
    createChart(model, { ...defaultChart, labelRanges: ["B1:B2"] }, "44");
  ...
  test("The legend is not displayed if there is no label for pie charts", () => {
    ...
    createChart(model, { ...pieChart, labelRanges: ["B1:B2"] }, "44");
  ...
  test("Labels are not generated if there is no label", () => {
    ...
    createChart(model, { ...defaultChart, type: "bar", labelRanges: ["B1:B2"] }, "44");
...
describe("Chart design configuration", () => {
  ...
  test("value without matching index in the label set", () => {
    ...
    createChart(
      model,
      { type: "bar", labelRanges: ["A2:A3"], dataSets: [{ dataRange: "B1:B15" }] },
      "1"
    );
    const data = getChartConfiguration(model, "1").data;
    expect(data.labels).toEqual([""]);
  ...
  test("label without matching index in the data set", () => {
    ...
    createChart(
      model,
      { type: "bar", labelRanges: ["A2:A15"], dataSets: [{ dataRange: "B1:B3" }] },
      "1"
    );
    const data = getChartConfiguration(model, "1").data;
    expect(data.labels).toEqual(["P1"]);
  ...
  test("no data points at all", () => {
    ...
    createChart(
      model,
      { type: "bar", labelRanges: ["A2:A3"], dataSets: [{ dataRange: "B1:B3" }] },
      "1"
    );
    const data = getChartConfiguration(model, "1").data;
    expect(data.labels).toEqual([]);
...
describe("Chart evaluation", () => {
  ...
  test("undo/redo invalidates the chart runtime", () => {
    ...
    updateChart(model, chartId, { labelRanges: ["A1:A2"] });
...
describe("Pie chart invalid values", () => {
  ...
  test("Non-number values are removed from the pie chart", () => {
    ...
    createChart(
      model,
      { dataSets: [{ dataRange: "F5:F8" }], labelRanges: ["G5:G8"], type: "pie" },
      "1"
    );
...
describe("Chart labels truncation", () => {
  test.each(["bar", "line", "combo", "radar"] as const)(
    ...
    (type) => {
      ...
      createChart(
        model,
        { type, dataSets: [{ dataRange: "B1:B2" }], labelRanges: ["A1:A2"] },
        "chartId"
      );
  ...
  test.each(["bar", "line", "combo"] as const)(
    ...
    (type) => {
      ...
      createChart(
        model,
        { type, dataSets: [{ dataRange: "B1:B3" }], labelRanges: ["A1:A3"] },
        "chartId"
      );
