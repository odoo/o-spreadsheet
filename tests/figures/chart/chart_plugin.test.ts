...
test("moving average trending line", () => {
  ...
  createChart(
    model,
    {
      type: "line",
      dataSets: [
        { dataRange: "C1:C5", trend: { display: true, type: "movingAverage", window: 3 } },
      ],
      labelRange: "B1:B5",
      labelsAsText: false,
      dataSetsHaveTitle: false,
  ...
  // @ts-ignore
  expect(runtime.dataSetsValues[1].data).toEqual([null, null, 2, 3, 4]);
  updateChart(model, "1", {
    dataSets: [{ dataRange: "C1:C5", trend: { display: true, type: "movingAverage", window: 2 } }],
  });
  runtime = model.getters.getChartRuntime("1") as LineChartRuntime;
  // @ts-ignore
  expect(runtime.dataSetsValues[1].data).toEqual([null, 1.5, 2.5, 3.5, 4.5]);
});
