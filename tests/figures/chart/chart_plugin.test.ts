...
describe("datasource tests", function () {
  ...
  test("ranges in definition change automatically", () => {
    ...
    expect(chart.labelRange?.[0]).toStrictEqual("Sheet1!C2:C4");
  });
  ...
  test("remove labels from existing chart", () => {
    ...
    updateChart(model, "1", { labelRange: [] });
    expect(
  ...
  test("Chart is copied on sheet duplication", () => {
    ...
    const labelRange = newChart.labelRange?.[0];

    expect(labelRange?.sheetId).toEqual(secondSheetId);
    expect(zoneToXc(labelRange!.zone)).toEqual("A2:A4");
