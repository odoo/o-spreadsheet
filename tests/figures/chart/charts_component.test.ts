...
describe("charts", () => {
  ...
  test.each(TEST_CHART_TYPES)(
    ...
    async (chartType) => {
      await mountSpreadsheet();
      createTestChart(chartType);
      updateChart(model, chartId, { annotationText: "This is an annotation text" });
      await nextTick();
      expect(fixture.querySelector(".o-figure")).not.toBeNull();
      expect(".o-chart-menu-item[data-id='chartInfo']").toHaveCount(1);
    }
  );

  test("charts do not have an info button if there is no annotation text or link", async () => {
    await mountSpreadsheet();
    createTestChart("calendar");
    await nextTick();
    expect(fixture.querySelector(".o-figure")).not.toBeNull();
    expect(fixture.querySelector(".o-chart-menu-item[data-id='chartInfo']")).toBeNull();
  });
  ...
  test.each(TEST_CHART_TYPES)("%s have an info button in readonly mode", async (chartType) => {
    ...
    await nextTick();
    expect(fixture.querySelector(".o-figure")).not.toBeNull();
    expect(fixture.querySelector(".o-chart-menu-item")).toHaveAttribute("data-id", "chartInfo");
  });
  ...
  describe("annotation text and/or link are displayed when clicking on info button", () => {
    test.each(["normal", "readonly", "dashboard"] as const)(
      ...
      async (mode) => {
        ...
        await nextTick();
        await click(fixture.querySelector(".o-chart-menu-item[data-id='chartInfo']")!);
    ...
    test.each(["normal", "readonly", "dashboard"] as const)(
      "basicChart: only link is displayed in %s mode",
      async (mode) => {
        await mountSpreadsheet();
        createTestChart("basicChart");
        updateChart(model, chartId, { annotationLink: "https://www.odoo.com" });
        model.updateMode(mode);
        await nextTick();
        await click(fixture.querySelector(".o-chart-menu-item[data-id='chartInfo']")!);
    ...
    test.each(["normal", "readonly", "dashboard"] as const)(
      "basicChart: only text is displayed in %s mode",
      async (mode) => {
        await mountSpreadsheet();
        createTestChart("basicChart");
        updateChart(model, chartId, { annotationText: "This is an annotation text" });
  ...
  test("withHttps helper adds https if the url does not start with https", async () => {
    await mountSpreadsheet();
    createTestChart("basicChart");
    updateChart(model, chartId, { annotationLink: "javascript:alert('hello world')" });
    // https://javascript:alert is not valid and adding https prevent javascript injection
    await nextTick();
    await click(fixture.querySelector(".o-chart-menu-item")!);
    expect(fixture.querySelector(".o-button-link")).toHaveAttribute(
      "href",
    ...
    switch (chartType) {
      case "combo":
        ...
        expect((dataSeries.querySelector(" .o-selection input") as HTMLInputElement).value).toBe(
          TEST_CHART_DATA.basicChart.dataSource.dataSets[0].dataRange
        );
  ...
  test("can edit chart data series color", async () => {
    ...
    let colorMenu = fixture.querySelectorAll(".o-round-color-picker-button")[1];
    ...
    await editSelectComponent(".data-series-selector", "serie_2");

    colorMenu = fixture.querySelectorAll(".o-round-color-picker-button")[1];
  ...
  test("can edit pie chart slices color", async () => {
    ...
    const colorMenu = fixture.querySelectorAll(".o-round-color-picker-button")[1];
  ...
  test("can edit annotation text and link", async () => {
    ...
    if (annotationText) {
      ...
      annotationText.dispatchEvent(new InputEvent("input", { data: "coucou", bubbles: true }));
      annotationText.blur();
      await nextTick();
    }
    const linkInput = fixture.querySelector(
      "[data-test-id='annotation-link'] input"
    ) as HTMLInputElement;
    await setInputValueAndTrigger(linkInput, "https://www.google.com");
  ...
  test("can undo annotation text change", async () => {
    createChart(
      model,
      {
        ...toChartDataSource({
    ...
    expect(model.getters.getChartDefinition(chartId).annotationText).toBe("coucou");
    undo(model);
    expect(model.getters.getChartDefinition(chartId).annotationText).toBe(
      "This is an annotation text"
    );
  });
  ...
  test("can open design panel of chart with duplicated dataset", async () => {
    createChart(
      model,
      {
        ...toChartDataSource({ dataSets: [{ dataRange: "C1:C4" }, { dataRange: "C1:C4" }] }),
        type: "line",
      },
      chartId
    );
  ...
  test("Can remove the last data series", async () => {
    ...
    expect(dataSource).toMatchObject({
      dataSets: [
        ...
        { dataRange: "C1:C4", dataSetId: dataSetIds[1] },
      ],
    });
    expect(dataSetStyles).toMatchObject({
      [dataSetIds[0]]: { yAxisId: "y" },
      [dataSetIds[1]]: {},
    });
    ...
    expect(updatedDataSetStyles).toMatchObject({
      [dataSetIds[0]]: {
        yAxisId: "y",
        backgroundColor: "#4EA7F2", // The color is added to keep colors consistent
      },
    });
  });
    ...
    expect(model.getters.getChartDefinition(chartId)).toMatchObject(
      toChartDataSource({
        dataSets: [
          ...
          { dataRange: "C1:C4", dataSetId: dataSetIds[1] },
          { dataRange: "D1:D4", dataSetId: dataSetIds[2] },
        ],
      })
    );
    ...
    expect(model.getters.getChartDefinition(chartId)).toMatchObject(
      toChartDataSource({
        dataSets: [
          ...
          { dataRange: "2:2", dataSetId: expect.any(String) },
        ],
      })
    );
  });
  ...
  test("Can add multiple ranges all in once with fullColumn range", async () => {
    ...
    expect(definition).toMatchObject(
      toChartDataSource({
        dataSets: [
          { dataRange: "A:A", dataSetId: dataSetIds[0] },
          { dataRange: "B:B", dataSetId: dataSetIds[1] },
        ],
      })
    );
  });
    ...
    expect(definition).toMatchObject(
      toChartDataSource({
        dataSets: [
          ...
          { dataRange: "C2:C4", yAxisId: "y1", dataSetId: dataSetIds[1] },
        ],
      })
    );
  });
  ...
  test("Can add multiple series in transposed dataset and keep the current orientation", async () => {
    ...
    expect(definition).toMatchObject(
      toChartDataSource({
        dataSets: [
          { dataRange: "B2:C2", dataSetId: dataSetIds[0] },
          { dataRange: "B3:C3", dataSetId: dataSetIds[1] },
          { dataRange: "B4:C4", dataSetId: dataSetIds[2] },
        ],
      })
    );
    await simulateClick(".o-data-series .o-add-selection");
    ...
    expect(definition).toMatchObject(
      toChartDataSource({
        dataSets: [
          ...
          { dataRange: "B3:C3", dataSetId: dataSetIds[1] },
          { dataRange: "B4:C4", dataSetId: dataSetIds[2] },
          { dataRange: "D2:E2", dataSetId: dataSetIds[3] },
          { dataRange: "D3:E3", dataSetId: dataSetIds[4] },
          { dataRange: "D4:E4", dataSetId: dataSetIds[5] },
        ],
      })
    );
  });
  ...
  test("Deleting a dataset and undo restores the exact same definition", async () => {
    createChart(
      model,
      {
        type: "combo",
        ...toChartDataSource({
          dataSets: [
            { dataRange: "A1", type: "line" },
            { dataRange: "B1", type: "bar" },
          ],
        }),
      },
      chartId
    ...
    await mountChartSidePanel();
    await openChartConfigSidePanel(model, env, chartId);
    const before = deepCopy(model.getters.getChartDefinition(chartId));
    await simulateClick(fixture.querySelectorAll(".o-data-series .o-remove-selection")[1]);
    undo(model);
    const after = model.getters.getChartDefinition(chartId);
    expect(after).toEqual(before);
  });

  test("Deleting a dataset then switching chart type do not restore the deleted dataset", async () => {
    createChart(
      model,
      {
        type: "combo",
    ...
    // Switch to bar (saves context with 2 datasets)
    await changeChartType("bar");
    await simulateClick(fixture.querySelectorAll(".o-data-series .o-remove-selection")[1]);

    // Switch to column -> the deleted dataset should not come back
    await changeChartType("column");
    const def = model.getters.getChartDefinition(chartId);
  ...
  describe("aggregate", () => {
    ...
    test("Chart datasets from old chart type are discarded as soon as a dataset is changed in the new type", async () => {
      ...
      await changeChartType("pie");
      updateChart(model, chartId, { ...toChartDataSource({ dataSets: [{ dataRange: "C1" }] }) });
      await nextTick();
    ...
    test("Label range is not wrongly restored when switching from a chart with a label range to a scorecard then to another chart", async () => {
      createChart(
        ...
        {
          type: "line",
          ...toChartDataSource({ dataSets: [{ dataRange: "A1" }], labelRanges: ["B3"] }),
        },
        chartId
      );
      await mountChartSidePanel(chartId);
      ...
      await changeChartType("line");
      expect(model.getters.getChartDefinition(chartId)).toMatchObject(
        toChartDataSource({ dataSets: [{ dataRange: "A1" }], labelRanges: undefined })
      );
    });
  });
      ...
      async (type: "bar" | "line" | "scatter" | "combo") => {
        ...
        for (const trendType of ["exponential", "logarithmic", "linear", "trailingMovingAverage"]) {
          ...
          if (trendType === "linear") {
            ...
            expect(Object.values(definition.dataSetStyles)[0].trend?.type).toEqual(trendType);
          }
        }
      }
    );
    ...
    test("Trend line is not in the choice of data series to edit", async () => {
      ...
      createChart(
        model,
        {
          type: "line",
          ...toChartDataSource({
            dataSets: [{ dataRange: "E1:E4", trend }],
            dataSetsHaveTitle: false,
          }),
        },
        chartId
...
describe("Change chart type", () => {
  ...
  test("When changing chart type from hierarchical to bar, all label ranges are preserved", async () => {
    ...
    expect(getChartDataSource(model, chartId)?.labelRanges).toEqual(["A1:A4", "B1:B4"]);
  });
  ...
  test("Changing an empty bar chart to scorecard does not crash and leaves keyValue undefined", async () => {
    createChart(model, { type: "bar", ...toChartDataSource({ dataSets: [] }) }, chartId);
    await mountChartSidePanel(chartId);
...
describe("groupByParentCategories – side panel", () => {
  ...
  test.each(["bar", "line", "scatter", "combo", "pyramid", "waterfall"] as const)(
    ...
    async (type) => {
      ...
      await mountChartSidePanel(chartId);
      expect("input[name='groupByParentCategories']").toHaveCount(0);
    }
  );
  ...
  test.each(["bar", "line", "scatter", "combo", "pyramid", "waterfall"] as const)(
    ...
    async (type) => {
      ...
      expect("input[name='groupByParentCategories']").toHaveCount(1);
    }
  );

  test("toggling the checkbox toggles groupByParentCategories", async () => {
    createChart(
      model,
      {
        type: "bar",
    ...
    await simulateClick("input[name='groupByParentCategories']");
  ...
  test("checkbox state reflects current definition value", async () => {
    ...
    const checkbox = fixture.querySelector(
      "input[name='groupByParentCategories']"
    ) as HTMLInputElement;
    expect(checkbox.checked).toBe(true);
  });
});
