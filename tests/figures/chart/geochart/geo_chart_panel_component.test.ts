...
describe("Geo chart side panel", () => {
  ...
  describe("Design panel", () => {
    test("Geo design panel is correctly initialized", async () => {
      ...
      expect(".o-color-scale select").toHaveValue("inferno");
    });
    ...
    test("Can edit the color scale", async () => {
      createGeoChart(model, { colorScale: "inferno" });
      await openChartDesignSidePanel(model, env, fixture, chartId);

      await setInputValueAndTrigger(".o-color-scale select", "magma");
      expect(getGeoChartDefinition(chartId)?.colorScale).toEqual("magma");
      expect(".o-color-scale select").toHaveValue("magma");
    });
    ...
    test("Can edit the projection type", async () => {
      createGeoChart(model, { projection: "mercator" });
      await openChartDesignSidePanel(model, env, fixture, chartId);

      await setInputValueAndTrigger(".o-projection select", "naturalEarth1");
      expect(getGeoChartDefinition(chartId)?.projection).toEqual("naturalEarth1");
      expect(".o-projection select").toHaveValue("naturalEarth1");
    });
