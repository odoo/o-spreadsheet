...
describe("heatmap chart", () => {
  ...
  test("heatmap chart aggregates duplicate row/column pairs by summing values", () => {
    ...
    expect(datasets[0].values).toEqual([11, 2, undefined]); // Apple/Mon = 1 + 10
    expect(datasets[1].values).toEqual([3, undefined, 4]);
  });
  ...
  test("without a data range, counts occurrences of each (row, column) pair", () => {
    ...
    expect(datasets[1].values).toEqual([1, undefined]); // Banana x Tue never occurs
  });
  ...
  test("showValues displays nothing for a missing (row, column) combination", () => {
    ...
    expect(showValuesPlugin.callback(undefined, { _dataset: datasets[0] }, 1)).toBe(undefined);
  });
