...
describe("heatmap chart", () => {
  ...
  test("tooltip is hidden for a missing (row, column) combination", () => {
    // prettier-ignore
    const model = createModelFromGrid({
      A2: "Apple",  B2: "Mon", C2: "1",
      A3: "Banana", B3: "Tue", C3: "2",
    ...
    expect(tooltip.filter({ dataset: datasets[0], dataIndex: 0 })).toBe(false);
  });
  ...
  test("showValues displays nothing for a missing (row, column) combination", () => {
    ...
    expect(showValuesPlugin.callback(undefined, { _dataset: datasets[0] }, 0)).toBe("");
  });
