...
describe("Insert chart menu item", () => {
  ...
  test("Chart is inserted at correct position", () => {
    ...
    expect(figureUI).toStrictEqual({
      figure: expect.any(Object),
      x: (width - figureUI.figure.width) / 2,
      y: (height - figureUI.figure.height) / 2,
    });
  });
  ...
  test("Chart is inserted at correct position on a scrolled viewport", () => {
    ...
    expect(figureUI).toStrictEqual({
      figure: expect.any(Object),
      x: 2 * DEFAULT_CELL_WIDTH + (width - figureUI.figure.width) / 2,
      y: 4 * DEFAULT_CELL_HEIGHT + (height - figureUI.figure.height) / 2,
    });
  });
  ...
  test("Chart is inserted at correct position on a scrolled viewport with columns frozen", () => {
    ...
    expect(figureUI).toStrictEqual({
      figure: expect.any(Object),
      x: (width - figureUI.figure.width) / 2, // figure is inside left frozen pane
      y: 4 * DEFAULT_CELL_HEIGHT + (height - figureUI.figure.height) / 2,
    });
  });
