...
describe("Insert chart menu item", () => {
  ...
  test("Chart of single column without title", () => {
    ...
    payload.definition.labelRange = "B2:B5";
  ...
  test("Chart of single column with title", () => {
    ...
    payload.definition.labelRange = "B1:B5";
    payload.definition.isDoughnut = true;
    payload.definition.legendPosition = "none";
  ...
  test("Doughnut chart when numeric column sum < 100", () => {
    setSelection(model, ["C2:C4"]);
    insertChart();
    const payload = { ...defaultPiePayload };
    payload.definition.dataSets = [{ dataRange: "C2:C4" }];
  ...
  test("Pie chart when numeric column sum = 100", () => {
  ...
  test("Bar chart when numeric column sum > 100", () => {
    setCellContent(model, "B6", "200");

    setSelection(model, ["B2:B6"]);
    insertChart();
    const payload = { ...defaultPayload };
    payload.definition.dataSets = [{ dataRange: "B2:B6" }];
    payload.definition.labelRange = "B2:B6";
    payload.definition.stacked = false;
    expect(dispatchSpy).toHaveBeenCalledWith("CREATE_CHART", payload);
  });

  test("Pie chart for one text column with <=6 unique values", () => {
  ...
  test("Bar chart for one text column with >6 unique values", () => {
    ["A", "B", "C", "D", "E", "F", "G", "A"].forEach((val, i) =>
      setCellContent(model, `K${i + 1}`, val)
    );
    setSelection(model, ["K1:K8"]);
    insertChart();

    const payload = { ...defaultPayload };
    payload.definition.dataSets = [{ dataRange: "K1:K8" }];
    payload.definition.labelRange = "K1:K8";
    payload.definition.stacked = false;
    payload.definition.aggregated = true;
    expect(dispatchSpy).toHaveBeenCalledWith("CREATE_CHART", payload);
  });

  test("Two numeric columns create scatter chart", () => {
    setSelection(model, ["C2:C4", "D2:D4"]);
    insertChart();
    const payload = { ...defaultPayload };
    payload.definition.type = "scatter";
    (payload.definition.dataSets = [{ dataRange: "D2:D4" }]),
      (payload.definition.labelRange = "C2:C4");
    payload.definition.labelsAsText = false;
    expect(dispatchSpy).toHaveBeenCalledWith("CREATE_CHART", payload);
  });

  test("Date + number column creates line chart", () => {
    setCellContent(model, "K1", "10/10/2022");
    setCellContent(model, "K2", "10/11/2022");
    setCellContent(model, "K3", "10/12/2022");

    setSelection(model, ["K1:K3", "B1:B4"]);
    insertChart();
    const payload = {
      ...defaultPayload,
      definition: {
  ...
  test("Text + number with <= 6 unique labels creates pie", () => {
  ...
  test("Text + number with >6 non-unique labels creates treemap", () => {
  ...
  test("Text + multiple numeric columns with cyclic labels creates radar chart", () => {
