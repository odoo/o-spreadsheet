...
import { createSunburstChart, setFormat } from "../../../test_helpers/commands_helpers";
...
describe("Sunburst chart chart", () => {
  ...
  test("Sunburst ghost items do not have a background/border", () => {
    ...
    expect(datasets[1].backgroundColor(toChartJSCtx(rootData))).toBeSameColorAs("rgba(0,0,0,0)");
    expect(datasets[1].borderColor(toChartJSCtx(rootData))).toBeSameColorAs("rgba(0,0,0,0)");

    const subData: SunburstChartRawData = { value: 10, label, groups: ["Group1", "SubGroup1"] };
    expect(datasets[0].backgroundColor(toChartJSCtx(subData))).toBeSameColorAs("rgba(0,0,0,0)");
    expect(datasets[0].borderColor(toChartJSCtx(subData))).toBeSameColorAs("rgba(0,0,0,0)");
  });
  ...
  test("Sunburst chart tooltip", () => {
    ...
    expect(tooltip?.callbacks?.label?.(toChartJSCtx(data))).toBe("Group1: 10.0$");
    ...
    expect(tooltip?.callbacks?.label?.(toChartJSCtx(groupData))).toBe("Q1 / May / W2: 10.0$");
  });
