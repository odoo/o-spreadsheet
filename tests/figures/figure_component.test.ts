...
describe("figures", () => {
  ...
  test("can select multiple figures", async () => {
    ...
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig2", "fig1"]);
    await simulateClick(".o-figure[data-id=fig3]", undefined, undefined, { shiftKey: true });
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig3", "fig2", "fig1"]);
    await simulateClick(".o-figure[data-id=fig3]", undefined, undefined, { shiftKey: true });
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig2", "fig1"]);
    await simulateClick(".o-figure[data-id=fig2]");
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig2"]);
  });
