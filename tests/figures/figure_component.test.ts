...
describe("figures", () => {
  ...
  test("can select multiple figures", async () => {
    ...
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig1"]);
    await simulateClick(".o-figure[data-id=fig2]", undefined, undefined, { shiftKey: true });
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig1", "fig2"]);
    await simulateClick(".o-figure[data-id=fig3]");
    await nextTick();
    expect(model.getters.getSelectedFiguresIds()).toEqual(["fig3"]);
  });
  ...
  test("Can move a figure with keyboard", async () => {
    ...
    await simulateClick(".o-figure");
    await nextTick();
    const selectedFigure = model.getters.getSelectedFiguresIds();
  ...
  test("Can move multiple figures with keyboard", async () => {
    ...
    const selectedFigure = model.getters.getSelectedFiguresIds();
    expect(selectedFigure).toEqual(["fig1", "fig2"]);
    expectXY("fig1", 1, 1);
    expectXY("fig2", 2, 2);
    //down
    await keyDown({ key: "ArrowDown" });
  ...
  test("Selected figure isn't removed by scroll", async () => {
    ...
    fixture.querySelector(".o-scrollbar.vertical")!.dispatchEvent(new Event("scroll"));
    expect(model.getters.getSelectedFiguresIds()).toEqual(["someuuid"]);
  });
  ...
  describe.each(["image", "basicChart", "scorecard", "gauge"])(
    ...
    (type: string) => {
      ...
      test(`Copied figure ${type} are selected`, async () => {
        ...
        await simulateClick(".o-figure-menu-item");
        await simulateClick(".o-menu div[data-name='copy']");
        expect(model.getters.getSelectedFiguresIds()).toEqual([figureId]);
        paste(model, "A1");
        expect(getFigureIds(model, sheetId)).toHaveLength(2);
        const chartIds = getFigureIds(model, sheetId);
        expect(model.getters.getSelectedFiguresIds()).not.toEqual([figureId]);
        expect(model.getters.getSelectedFiguresIds()).toEqual([chartIds[1]]);
      });
