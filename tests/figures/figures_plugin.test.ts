...
describe("figure plugin", () => {
  ...
  test("can select multiple figures", () => {
    ...
    expect(model.getters.getSelectedFiguresIds()).toEqual(["bis", "someuuid"]);
    model.dispatch("SELECT_FIGURE", { figureId: "repetita", selectMultiple: true });
    expect(model.getters.getSelectedFiguresIds()).toEqual(["repetita", "bis", "someuuid"]);
    model.dispatch("UNSELECT_FIGURE", { figureId: "bis" });
    expect(model.getters.getSelectedFiguresIds()).toEqual(["repetita", "someuuid"]);
    model.dispatch("SELECT_FIGURE", { figureId: "bis" });
    expect(model.getters.getSelectedFiguresIds()).toEqual(["bis"]);
  });
  ...
  test("can delete a figure and it will reselect it", () => {
    const model = new Model();
    const sheetId = model.getters.getActiveSheetId();
    createFigure(model, {
      sheetId,
