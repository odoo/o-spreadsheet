...
describe("INDIRECT formula", () => {
  ...
  test("Using address string as reference (absolute RC notation)", () => {
    const grid = evaluateGrid({
      A1: "1",
      A2: "A2",
      A3: "R2C1",
      A4: '=INDIRECT("R1C1", false)',
      A5: "=INDIRECT(A3, false)",
    });
    expect(grid.A4).toBe(1);
    expect(grid.A5).toBe("A2");
  });

  test("Using address string as reference (relative RC notation)", () => {
    const grid = evaluateGrid({
      A1: "1",
      C2: "C2",
      A3: "R[-3]C[2]",
      A4: '=INDIRECT("R[-3]C[0]", false)',
      A5: "=INDIRECT(A3, false)",
    });
    expect(grid.A4).toBe(1);
    expect(grid.A5).toBe("C2");
  });
