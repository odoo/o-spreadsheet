...
describe("vectorization", () => {
  ...
  beforeEach(() => {
    ...
    addToRegistry(functionRegistry, "FUNCTION.THAT.SPREADS", {
      ...
      computeArray: function (zone, arg1: Arg) {
  ...
  test("vectorization display #N/A errors when vectors havent the same size", () => {
    ...
    expect(getEvaluatedCell(model, "F1").message).toBe("Array arguments are of different size.");
    ...
    expect(getEvaluatedCell(model, "D4").message).toBe("Array arguments are of different size.");
  });
