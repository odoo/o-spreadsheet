...
describe("Grid component in dashboard mode", () => {
  ...
  test("Filter icon is correctly rendered", async () => {
    ...
    expect(model.getters.getCellIconRect(iconA, "withHeaders")).toMatchObject({ y, x: leftA });
    const iconB = getCellIcons(model, "C2")[0];
    expect(model.getters.getCellIconRect(iconB, "withHeaders")).toMatchObject({ y, x: leftB });
  });
