...
describe("Grid component", () => {
  ...
  test("Double clicking on an icon does not open the composer", async () => {
    ...
    const rect = model.getters.getCellIconRect(icon, "withoutHeaders");
  ...
  describe("keybindings", () => {
    ...
    test("Filter icon is correctly rendered", () => {
      ...
      expect(model.getters.getCellIconRect(iconA, "withoutHeaders")).toMatchObject({ y, x: leftA });
      const iconB = getCellIcons(model, "C2")[0];
      expect(model.getters.getCellIconRect(iconB, "withoutHeaders")).toMatchObject({ y, x: leftB });
    });
