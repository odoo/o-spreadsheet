...
describe("move selected element(s)", () => {
  ...
  describe("move selected column(s)", () => {
    ...
    test("Can select a column but not move it in readonly", async () => {
      ...
      expect(new Set([...model.getters.getActiveCols()])).toEqual(new Set([0]));
...
describe("Can select de-select headers", () => {
  ...
  test("Can select already selected column remove it from selection", async () => {
    ...
    await selectColumn("D", { shiftKey: true });
    await selectColumn("E", { ctrlKey: true });
    expect(new Set([...model.getters.getActiveCols()])).toEqual(new Set([0, 1, 2, 3, 4]));
    await selectColumn("C", { ctrlKey: true });
    expect(new Set([...model.getters.getActiveCols()])).toEqual(new Set([0, 1, 3, 4]));
  });
  ...
  test("Can select already selected row remove it from selection", async () => {
    ...
    await selectRow(3, { shiftKey: true });
    await selectRow(4, { ctrlKey: true });
    expect(new Set([...model.getters.getActiveRows()])).toEqual(new Set([0, 1, 2, 3, 4]));
    await selectRow(2, { ctrlKey: true });
    expect(new Set([...model.getters.getActiveRows()])).toEqual(new Set([0, 1, 3, 4]));
  });
});
