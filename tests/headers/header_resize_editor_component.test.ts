import { signal, type Signal } from "@odoo/owl";
import { Model } from "../../src";
import {
  HeaderResizeEditor,
  HeaderResizeEditorTarget,
} from "../../src/components/header_resize_editor/header_resize_editor";
...
describe("Header resize editor", () => {
  ...
  let target: Signal<HeaderResizeEditorTarget | null>;
  ...
  async function mountHeaderResizeEditor(
    editorTarget: HeaderResizeEditorTarget | null = { dimension: "COL", index: 0 }
  ): Promise<void> {
    ({ fixture } = await mountComponentWithPortalTarget(HeaderResizeEditor, {
      model,
      props: { target, gridRect: { x: 0, y: 0, width: 0, height: 0 } },
    }));
    if (editorTarget) {
      target.set(editorTarget);
      await nextTick();
    }
  ...
  function getSizeInput(): HTMLInputElement {
    ...
    if (!input) {
      throw new Error("Header resize input was not rendered");
    }
    return input;
  }
  ...
  test("sets a custom width for selected columns", async () => {
    ...
    await mountHeaderResizeEditor({ dimension: "COL", index: 0 });

    const input = getSizeInput();
  ...
  test("sets a custom height for selected rows", async () => {
    ...
    await mountHeaderResizeEditor({ dimension: "ROW", index: 1 });
  ...
  test("applying an empty input restores the default width", async () => {
    ...
    await mountHeaderResizeEditor({ dimension: "COL", index: 1 });
  ...
  test.each([MIN_COL_WIDTH - 1, MAX_HEADER_SIZE + 1])(
    ...
    async (size) => {
      ...
      expect(target()).toEqual({ dimension: "COL", index: 1 });
    }
  );
  ...
  test("renders and focuses the editor only while a target is set", async () => {
    await mountHeaderResizeEditor(null);
    expect(SELECTORS.sizeInput).toHaveCount(0);

    target.set({ dimension: "COL", index: 0 });
    await nextTick();
    expect(document.activeElement).toBe(getSizeInput());
  });
});
