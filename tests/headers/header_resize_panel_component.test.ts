...
import { MIN_COL_WIDTH } from "../../src/constants";
...
describe("header resize side panel component", () => {
  ...
  test.each([
    ...
    [(MIN_COL_WIDTH - 1).toString(), `Size must be between ${MIN_COL_WIDTH} and 2000 pixels.`],
    ["2001", `Size must be between ${MIN_COL_WIDTH} and 2000 pixels.`],
  ...
  test("Focusing the size input selects exact size mode", async () => {
    ...
    await simulateClick(SELECTORS.fitToDataRadio);
    expect(document.querySelector<HTMLInputElement>(SELECTORS.fitToDataRadio)!.checked).toBe(true);
    await simulateClick(SELECTORS.sizeInput);
    expect(document.querySelector<HTMLInputElement>(SELECTORS.exactSizeRadio)!.checked).toBe(true);
  });
});
