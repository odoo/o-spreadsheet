...
describe("formatting values (with formatters)", () => {
  test("can set a format to a cell", () => {
    ...
    model.setValue("A1", "3");
    expect(model.formatCell(model.state.cells.A1)).toBe("3");
    model.selectCell(0, 0);
    model.setFormat("0.00%");
    expect(model.state.cells.A1.format).toBe("0.00%");
  ...
  test("can set a format to an empty cell", () => {
    ...
    model.setFormat("0.00%");
    expect(model.state.cells.A1.format).toBe("0.00%");
    expect(model.formatCell(model.state.cells.A1)).toBe("");
  ...
  test("can clear a format in a non empty cell", () => {
    ...
    model.selectCell(0, 0);
    model.setFormat("0.00%");
    expect(model.state.cells.A1.format).toBeDefined();
    expect(model.formatCell(model.state.cells.A1)).toBe("300.00%");
    model.setFormat("");
    expect(model.formatCell(model.state.cells.A1)).toBe("3");
    expect(model.state.cells.A1.format).not.toBeDefined();
