...
describe("Named range plugin", () => {
  ...
  test("Renaming a named range changes the formulas with this ranges", () => {
    ...
    expect(getCellRawContent(model, "A1")).toEqual("=FirstRange+HelloThere");
    expect(getCellRawContent(model, "A2")).toEqual("=HelloThere*2");
    expect(getCellRawContent(model, "A3")).toEqual("SecondRange");
    expect(getCellRawContent(model, "A4")).toEqual('="SecondRange"');
    ...
    expect(getCellRawContent(model, "A5")).toEqual("=SUM(25)"); // SUM functions should not be changed
    expect(getCellRawContent(model, "A6")).toEqual("=SUMMIT+10");
  });
  ...
  test("Named ranges are case insensitive", () => {
    ...
    expect(model.getters.getNamedRange("FIRSTRANGE")).toMatchObject({ name: "FirstRange" });
  ...
  test("Can undo/redo named ranges commands", () => {
    ...
    updateNamedRange(model, "MyRange", "RenamedRange", "C3:D4");
    expect(model.getters.getNamedRanges()[0].name).toBe("RenamedRange");
  ...
  test("Named range in compiled formulas are correctly transformed", () => {
    createNamedRange(model, "MyRange", "A1:A3");
    setCellContent(model, "B1", "=SUM(MyRange+1)");

    let cell = getCell(model, "B1") as FormulaCell;
    expect(cell.compiledFormula).toMatchObject({
      symbols: ["SUM", "MyRange"],
      normalizedFormula: "=SUM(MyRange+|N|)",
      tokens: [
        { type: "OPERATOR", value: "=" },
        { type: "SYMBOL", value: "SUM" },
        { type: "LEFT_PAREN", value: "(" },
        { type: "SYMBOL", value: "MyRange" },
        { type: "OPERATOR", value: "+" },
        { type: "NUMBER", value: "__NO_REAL_VALUE__" },
        { type: "RIGHT_PAREN", value: ")" },
      ],
    });

    updateNamedRange(model, "MyRange", "RenamedRange", "A1:A3");
    cell = getCell(model, "B1") as FormulaCell;
    expect(cell.compiledFormula).toMatchObject({
      symbols: ["SUM", "RenamedRange"],
      normalizedFormula: "=SUM(RenamedRange+|N|)",
      tokens: [
        { type: "OPERATOR", value: "=" },
        { type: "SYMBOL", value: "SUM" },
        { type: "LEFT_PAREN", value: "(" },
        { type: "SYMBOL", value: "RenamedRange" },
        { type: "OPERATOR", value: "+" },
        { type: "NUMBER", value: "__NO_REAL_VALUE__" },
        { type: "RIGHT_PAREN", value: ")" },
      ],
