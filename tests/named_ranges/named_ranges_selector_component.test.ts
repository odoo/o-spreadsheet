...
describe("Named ranges topbar selector", () => {
  ...
  test("Entering spaces with replace them by underscores", async () => {
    ...
    expect(model.getters.getNamedRanges()[0]).toMatchObject({ name: "Named_with_spaces" });
  });
  ...
  test("Writing in the input filter the menu items shown", async () => {
    ...
    await simulateClick(".o-menu-item");
    expect(model.getters.getSelectedZone()).toEqual(toZone("C1:D4"));
    expect(".o-named-range-selector input").toHaveValue("AnotherRange");
    expect(model.getters.getNamedRanges()).toHaveLength(2);
  });

  test("Blurring the input will reset it", async () => {
    createNamedRange(model, "MyRange", "A1");
    await mountRangeSelector();
    const input = fixture.querySelector<HTMLInputElement>(".o-named-range-selector input")!;
    expect(input).toHaveValue("MyRange");
    await nextTick();

    input.focus();
    await setInputValueAndTrigger(input, "RandomName", "onlyInput");
    expect(input).toHaveValue("RandomName");
