...
describe("Pivot custom field panel", () => {
  test("Can create a custom field", async () => {
    ...
    expect(model.getters.getPivotCoreDefinition(pivotId).customFields).toEqual({
      Opportunity2: {
        parentField: "Opportunity",
        name: "Opportunity2",
        groups: [{ name: "my opportunity", values: ["my opportunity"] }],
      },
    });
  });
  ...
  test("Group with default name change name when adding values", async () => {
    await openPivotCustomFieldSidePanel(TEST_CUSTOM_FIELD);

    await click(fixture, ".o-add-group");
    expect('[data-id="Group"] .os-input').toHaveValue("Group");

    await click(fixture, '[data-id="Group"]  .o-tag-input');
    await selectAutoCompleteOption("my opportunity");
    expect('[data-id="my opportunity"] .os-input').toHaveValue("my opportunity");

    await click(fixture.querySelectorAll('[data-id="my opportunity"]  .o-tag')[0], ".o-delete");
    expect('[data-id="Group"] .os-input').toHaveValue("Group");

    // User defined name is not changed
    await setInputValueAndTrigger(".o-custom-group  input.os-input", "ThisIsMyGroup");
    await click(fixture, '[data-id="ThisIsMyGroup"]  .o-tag-input');
    await selectAutoCompleteOption("test opportunity");
    expect('[data-id="ThisIsMyGroup"] .os-input').toHaveValue("ThisIsMyGroup");
  });
