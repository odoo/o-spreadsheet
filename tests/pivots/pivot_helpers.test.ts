...
test("isSortedColumnValid", () => {
  ...
  // Total column
  const sortedColumn: PivotSortedColumn = { measure: "Price:sum", order: "asc", domain: [] };
  expect(isSortedColumnValid(sortedColumn, model.getters.getPivot(pivotId))).toBe(true);

  // Valid column
  sortedColumn.domain = [{ field: "Date:year", value: 2020, type: "char" }];
  expect(isSortedColumnValid(sortedColumn, model.getters.getPivot(pivotId))).toBe(true);

  // Invalid column value
  sortedColumn.domain = [{ field: "Customer", value: "Random Person", type: "char" }];
  expect(isSortedColumnValid(sortedColumn, model.getters.getPivot(pivotId))).toBe(false);

  // Invalid column field
  sortedColumn.domain = [{ field: "Random Field", value: "Alice", type: "char" }];
  expect(isSortedColumnValid(sortedColumn, model.getters.getPivot(pivotId))).toBe(false);

  // Invalid column granularity
  sortedColumn.domain = [{ field: "Date:quarter", value: 2020, type: "char" }];
  expect(isSortedColumnValid(sortedColumn, model.getters.getPivot(pivotId))).toBe(false);

  // Row dimension as sorted column
  sortedColumn.domain = [{ field: "Customer", value: "Alice", type: "char" }];
  expect(isSortedColumnValid(sortedColumn, model.getters.getPivot(pivotId))).toBe(false);
