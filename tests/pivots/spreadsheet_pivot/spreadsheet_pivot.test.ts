...
describe("Spreadsheet Pivot", () => {
  ...
  test("Pivot with day_of_week", () => {
    ...
    addPivot(model, "A1:B3", {
      columns: [],
      rows: [{ name: "Date", granularity: "day_of_week" }],
  ...
  test("Pivot with hour_number", () => {
    ...
    addPivot(model, "A1:B3", {
      columns: [],
      rows: [{ name: "Date", granularity: "hour_number" }],
      measures: [{ name: "Price", aggregator: "sum" }],
    });
    expect(getEvaluatedGrid(model, "C1:C5")).toEqual([
      ["(#1) Pivot"],
  ...
  test("Pivot with minute_number", () => {
    ...
    addPivot(model, "A1:B3", {
      columns: [],
      rows: [{ name: "Date", granularity: "minute_number" }],
      measures: [{ name: "Price", aggregator: "sum" }],
    });
    expect(getEvaluatedGrid(model, "C1:C5")).toEqual([
      ["(#1) Pivot"],
  ...
  test("Pivot with second_number", () => {
    ...
    addPivot(model, "A1:B3", {
      columns: [],
      rows: [{ name: "Date", granularity: "second_number" }],
      measures: [{ name: "Price", aggregator: "sum" }],
    });
    expect(getEvaluatedGrid(model, "C1:C5")).toEqual([
      ["(#1) Pivot"],
