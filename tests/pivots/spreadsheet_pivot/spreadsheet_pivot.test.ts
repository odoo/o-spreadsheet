...
describe("Spreadsheet Pivot", () => {
  ...
  test("PIVOT row headers are indented relative to the groupBy depth.", () => {
    ...
    expect(cellsWithMargins).toMatchObject([
      { value: "Pivot", format: undefined, iconMargin: 0 },
  ...
  test("Can split a tabular form into pivot formulas", () => {
    const model = createModelWithPivot("A1:I22");
    updatePivot(model, "1", {
      columns: [],
      rows: [
    ...
    setCellContent(model, "A25", "=PIVOT(1)");
    const oldGrid = removeFalsyAttributes(getGrid(model));

    const sheetId = model.getters.getActiveSheetId();
    model.dispatch("SPLIT_PIVOT_FORMULA", { sheetId, col: 0, row: 24, pivotId: "1" });

    delete oldGrid["A25"]; // Pivot title is removed in split
    expect(removeFalsyAttributes(getGrid(model))).toMatchObject(oldGrid);
  });
  ...
  test("Tabular form ignores collapsed headers", () => {
    ...
    updatePivot(model, "1", {
      collapsedDomains: { COL: [], ROW: [[{ type: "char", field: "Stage", value: "New" }]] },
    });
    expect(getGrid(model)).toEqual(oldGrid);
  });
});
