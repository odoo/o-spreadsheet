...
describe("data", () => {
  test("give default col size if not specified", () => {
    ...
    expect(model.getters.getColInfo(sheet, 0)!.size).toEqual(DEFAULT_CELL_WIDTH);
    expect(model.getters.getColInfo(sheet, 1)!.size).toEqual(DEFAULT_CELL_WIDTH);
  });
});
...
describe("Import", () => {
  test("Import sheet with rows/cols size defined.", () => {
    ...
    const sheet = model.getters.getActiveSheetId();
    expect(model.getters.getColInfo(sheet, 0)!.size).toBe(42);
    expect(model.getters.getColInfo(sheet, 1)!.size).toBe(DEFAULT_CELL_WIDTH);
    expect(model.getters.getRowInfo(sheet, 0)!.size).toBe(DEFAULT_CELL_HEIGHT);
    expect(model.getters.getRowInfo(sheet, 1)!.size).toBe(13);
  });
