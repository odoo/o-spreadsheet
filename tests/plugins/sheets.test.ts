...
describe("sheets", () => {
  ...
  test("Cols and Rows are correctly duplicated", () => {
    ...
    const newSheet = model.getters.getSheetIds()[1];
    activateSheet(model, newSheet);
    expect(model.getters.getColInfo(model.getters.getActiveSheetId(), 0)!.size).not.toBe(1);
    expect(model.getters.getRowInfo(model.getters.getActiveSheetId(), 0)!.size).not.toBe(1);
  });
