...
describe("Grid renderer animations", () => {
  test("Animation frames are requested at the start of an animation, and stopped at the end of the animation", () => {
    ...
    animationFrameCallback(gridRenderer.animationDuration);
  ...
  test("Animations are correctly updated on scroll", () => {
    ...
    drawGrid();
    animationFrameCallback(0);
    animationFrameCallback(gridRenderer.animationDuration / 2);
...
describe("Individual animation tests", () => {
  ...
  test("Can animate an icon fading in", () => {
    addIconCF(model, "B3", ["3", "7"], "arrows");
    setGrid(model, { A1: "1", A2: "=MUNIT(A1)" });
    drawGrid();
    expect(getBoxFromXc("B3").icons?.left).toEqual(undefined);

    setCellContent(model, "A1", "2");
    drawGrid();
    expect(getBoxFromXc("B3").icons?.left).toMatchObject({ opacity: 0 });

    animationFrameCallback(0);
    expect(getBoxFromXc("B3").icons?.left).toMatchObject({ opacity: 0 });

    animationFrameCallback(gridRenderer.animationDuration / 2);
    expect(getBoxFromXc("B3").icons?.left).toMatchObject({ opacity: 0.5 });

    animationFrameCallback(gridRenderer.animationDuration);
    expect(getBoxFromXc("B3").icons?.left?.opacity).toBe(undefined);
  });

  test("Can animate an icon fading out", () => {
    addIconCF(model, "B3", ["3", "7"], "arrows");
    setGrid(model, { A1: "2", A2: "=MUNIT(A1)" });
    drawGrid();
    expect(getBoxFromXc("B3").icons?.left).toBeTruthy();

    setCellContent(model, "A1", "1");
    drawGrid();
    expect(getBoxFromXc("B3").icons?.left).toMatchObject({ opacity: 1 });
    ...
    animationFrameCallback(gridRenderer.animationDuration);
    expect(getBoxFromXc("B3").icons?.left).toBe(undefined);
  });

  test("Can animate an icon change", () => {
    addIconCF(model, "B3", ["3", "7"], "arrows");
