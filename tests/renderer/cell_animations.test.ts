...
import { cellAnimationRegistry, EASING_FN } from "../../src/registries/cell_animation_registry";
...
describe("Grid renderer animations", () => {
  test("Animation frames are requested at the start of an animation, and stopped at the end of the animation", async () => {
  ...
  test("Animations are not run on updated cells", async () => {
    drawGrid();
    await setCellContent(model, "A2", "58");
    drawGrid();
    expect(spyRequestAnimationFrame).not.toHaveBeenCalled();
  });

  test("When changing sheet, animations are cancelled and no new animations are run for the new sheet", async () => {
    const spyCancelAnimationFrame = jest.spyOn(window, "cancelAnimationFrame");
    await createSheet(model, { sheetId: "sh2" });
    await setCellContent(model, "A1", "22", "sh2");
    await setCellContent(model, "A2", "=A1");
    drawGrid();
    await setCellContent(model, "A1", "2");
    drawGrid();
  ...
  test("Animations are not run the on copy/paste zone ", async () => {
    ...
    await addCfRule(model, "A1:A3", rule, "11");
    await setCellContent(model, "A3", "2");
    await setCellContent(model, "D8", "=SUM(B1:B3)");
    drawGrid();
  ...
  test("Animations are not canceled on unrelated model update", async () => {
    await setCellContent(model, "A2", "=A1");
    drawGrid();
    await setCellContent(model, "A1", "2");
    drawGrid();
    ...
    await renameSheet(model, model.getters.getActiveSheetId(), "newName");
    await setCellContent(model, "B12", "2");
    drawGrid();
    expect(gridRenderer["animations"].size).toEqual(1);
  });

  test("Animations are correctly updated on scroll", async () => {
    await setGrid(model, { A1: "hi", B2: "=A1" });
    drawGrid();
  ...
  test("Animations are run on undo/redo", async () => {
  ...
  test("Animations are cancelled on add/remove columns", async () => {
    const spyCancelAnimationFrame = jest.spyOn(window, "cancelAnimationFrame");
  ...
  test("Animations are replaced if the cell content changes again", async () => {
    await setCellContent(model, "A2", "=A1");
    drawGrid();
    ...
    await setCellContent(model, "A1", "world");
    drawGrid();
  ...
  test("Star format does not trigger animations on resize", async () => {
...
describe("Individual animation tests", () => {
  test("Can animate a text fading in", async () => {
    await setCellContent(model, "A2", "=MUNIT(A1)");
    drawGrid();
  ...
  test("Can animate a text fading out", async () => {
    await setGrid(model, { A1: "2", A2: "=MUNIT(A1)" });
    drawGrid();
  ...
  test("Text fading out has the correct clipRect", async () => {
    await resizeColumns(model, ["A"], 10);
    await setGrid(model, {
      A1: "TRUE",
      A2: '=IF(A1, "very long string that is clipped", "")',
      B2: "a",
    });
    drawGrid();
  ...
  test("Can animate a text changing", async () => {
    await setCellContent(model, "A2", "=A1");
    await setCellContent(model, "A1", "oldText");
    drawGrid();

    await setCellContent(model, "A1", "newText");
    drawGrid();
  ...
  test("Text change animation has a correct clipRect", async () => {
    await resizeColumns(model, ["B"], 10);
    await setGrid(model, { B2: "=A1" });
    drawGrid();
    const longText = "Very long text that is clipped";
    await setCellContent(model, "A1", longText);
    drawGrid();
    animationFrameCallback(0);
  ...
  test("Text change animation boxes have the correct empty icons", async () => {
    await setGrid(model, { A1: "2", A2: "=A1" });
    await addIconCF(model, "A2", ["3", "7"], "arrows");
    drawGrid();
    await setCellContent(model, "A1", "1");
    drawGrid();
    animationFrameCallback(0);
  ...
  test("Can animate a background color change", async () => {
    await addEqualCf(model, "A1", { fillColor: "#0000FF" }, "15");
    await setCellContent(model, "A1", "=A2");
    drawGrid();

    await setCellContent(model, "A2", "15");
    drawGrid();
  ...
  test("Can animate both a text fading out and a background color change at the same time", async () => {
    await addEqualCf(model, "B3", { fillColor: "#0000FF" }, "1");
    await setGrid(model, { A1: "2", A2: "=MUNIT(A1)" });
    await setFormatting(model, "B3", { textColor: "#FF00FF" });
  ...
  test("Can animate a text color change", async () => {
    await addEqualCf(model, "A1", { textColor: "#0000FF" }, "15");
    await setCellContent(model, "A1", "=A2");
    drawGrid();
  ...
  test("Can animate a border fading in", async () => {
    await setGrid(model, { A1: "1", A2: "=MUNIT(A1)" });
    await createDynamicTable(model, "A2", { styleId: "TableStyleLight1" });
    drawGrid();
  ...
  test("Can animate a border fading out", async () => {
    await setGrid(model, { A1: "2", A2: "=MUNIT(A1)" });
    await createDynamicTable(model, "A2", { styleId: "TableStyleLight1" });
    drawGrid();
    expect(getBoxFromXc("A3").border).toEqual({
  ...
  test("Can animate a border changing color", async () => {
    await setGrid(model, { A2: "=MUNIT(2)" });
    await createDynamicTable(model, "A2", { styleId: "TableStyleLight1" });
    drawGrid();
    ...
    await updateTableConfig(model, "A2", { styleId: "TableStyleLight2" });
    drawGrid();
    expect(getBoxFromXc("B2").border).toEqual({
      top: { ...DEFAULT_BORDER_DESC, color: "#000000" },
      bottom: { ...DEFAULT_BORDER_DESC, color: "#000000" },
  ...
  test("Can animate an icon svg change", async () => {
    await addIconCF(model, "B3", ["3", "7"], "arrows");
    await setGrid(model, { A1: "2", B3: "=A1" });
    drawGrid();
  ...
  test("Icon appearing without text change trigger a text sliding animation containing the icon", async () => {
    await setGrid(model, { B3: "8" });
    await addIconCF(model, "B3", ["3", "7"], "arrows");
    await undo(model);
    drawGrid();
  ...
  test("Both icon and text appearing at once triggers a fade in animation", async () => {
    await setGrid(model, { A1: '=""', B3: "=A1" });
    await addIconCF(model, "B3", ["3", "7"], "arrows");
    drawGrid();
    expect(getBoxFromXc("B3").icons.left?.svg).toEqual(undefined);
  ...
  test("Icon disappearing with text staying triggers a text sliding animation containing the icon", async () => {
    await setGrid(model, { A1: "9", B3: "=A1" });
    await addIconCF(model, "B3", ["3", "7"], "arrows");
    drawGrid();
  ...
  test("Icon and text both disappearing at once triggers a fade out animation", async () => {
    await setGrid(model, { A1: "9", B3: "=A1" });
    await addIconCF(model, "B3", ["3", "7"], "arrows");
    drawGrid();
    expect(getBoxFromXc("B3").icons.left?.svg).toEqual(ICONS.arrowGood.svg);
  ...
  test("Can animate a data bar change", async () => {
    await addDataBarCF(model, "B1:B2", "#00FF00");
    await setGrid(model, { A1: "5", B1: "=A1", B2: "10" });
    drawGrid();
