...
describe("selection input plugin", () => {
  test("empty input should focus the first range", async () => {
    const { store } = await makeStore(SelectionInputStore);
  ...
  test("input with inital ranges should not be focused", async () => {
    const { store, container } = await makeStore(SelectionInputStore, ["D4"]);
    expect(store.selectionInputs.length).toBe(1);
  ...
  test("multiple initial values have different ids", async () => {
    const { store } = await makeStore(SelectionInputStore, ["D4", "D5"]);
  ...
  test("focus input which is already focused", async () => {
    const { store } = await makeStore(SelectionInputStore);
    store.focusById(idOfRange(store, 0));
    expect(store.selectionInputs[0].isFocused).toBe(true);
    store.focusById(idOfRange(store, 0));
  ...
  test("focused input should not change when selecting a zone for composer", async () => {
    const { store, model, container } = await makeStore(SelectionInputStore);
  ...
  test("cannot add empty range when maximum ranges reached", async () => {
    const { store } = await makeStore(SelectionInputStore, [], true);
    expect(store.selectionInputs).toHaveLength(1);
    store.addEmptyRange();
    expect(store.selectionInputs).toHaveLength(1);
  });

  test("Cannot add multiple ranges to a 'singleRange' input", async () => {
    const { store } = await makeStore(SelectionInputStore, [], true);
    expect(store.selectionInputs).toHaveLength(1);
  ...
  test("cannot add new range when maximum ranges reached", async () => {
    const { store } = await makeStore(SelectionInputStore, [], true);
    expect(store.selectionInputs).toHaveLength(1);
  ...
  test("add an empty range with initial value", async () => {
    const { store } = await makeStore(SelectionInputStore, ["A5"]);
    expect(store.selectionInputs.length).toBe(1);
    store.addEmptyRange();
    expect(store.selectionInputs.length).toBe(2);
    expect(store.selectionInputs[1].isFocused).toBe(true);
  ...
  test("last range is focused when one is removed", async () => {
    const { store } = await makeStore(SelectionInputStore);
    store.addEmptyRange();
    store.addEmptyRange();
    store.addEmptyRange();
  ...
  test("ranges are unfocused", async () => {
    const { store } = await makeStore(SelectionInputStore);
    expect(store.selectionInputs[0].isFocused).toBe(true);
    store.unfocus();
    expect(store.selectionInputs[0].isFocused).toBeFalsy();
  ...
  test("same range is updated while expanding", async () => {
  ...
  test("focus does not change values", async () => {
    const { store } = await makeStore(SelectionInputStore, ["A1", "B2"]);
  ...
  test("manually changing the input updates highlighted zone", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
  ...
  test("selection input updates handle full column ranges", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
  ...
  test("selection input updates handle full row ranges", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
  ...
  test("manually changing the input with existing range", async () => {
    const { store, container } = await makeStore(SelectionInputStore, ["A8"]);
  ...
  test("setting multiple ranges in one input", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
  ...
  test("writing an invalid range does not crash", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
  ...
  test("writing an invalid range with valid ones keeps the invalid one", async () => {
    const { store, container } = await makeStore(SelectionInputStore, ["B2", "E3"]);
    store.focusById(idOfRange(store, 0));
  ...
  test("writing an empty range removes the highlight", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
  ...
  test("disable input removes highlighted zones", async () => {
    const { store, container } = await makeStore(SelectionInputStore);
    store.changeRange(idOfRange(store, 0), "C2");
    expect(highlightedZones(container)).toStrictEqual(["C2"]);
    store.unfocus();
  ...
  test("initial ranges are not highlighted", async () => {
    const { store } = await makeStore(SelectionInputStore, ["C2", "D4"]);
    expect(store.selectionInputs.length).toBe(2);
    expect(store.highlights.length).toBe(0);
  });

  test("ranges are split for value", async () => {
    const { store } = await makeStore(SelectionInputStore);
    store.changeRange(idOfRange(store, 0), "C2, D5");
    expect(store.selectionInputValues).toStrictEqual(["C2", "D5"]);
  });

  test("multiple ranges are combined", async () => {
    const { store } = await makeStore(SelectionInputStore);
    store.addEmptyRange();
    store.changeRange(idOfRange(store, 0), "C2");
  ...
  test("trailing commas and spaces are removed", async () => {
    const { store } = await makeStore(SelectionInputStore);
    store.changeRange(idOfRange(store, 0), "C2, ");
    expect(store.selectionInputValues).toStrictEqual(["C2"]);
  });

  test("leading commas and spaces are removed", async () => {
    const { store } = await makeStore(SelectionInputStore);
  ...
  test("highlights are updated when focus switched from one input to another", async () => {
    const { store: store1, container } = await makeStore(SelectionInputStore, ["D4"]);
  ...
  test("color is kept between focuses", async () => {
    const { store } = await makeStore(SelectionInputStore, ["D4"]);
    store.focusById(idOfRange(store, 0));
    const color = store.selectionInputs[0].color;
    expect(color).toBeTruthy();
    store.unfocus();
  ...
  test("focus and change range with unbounded ranges", async () => {
    const { store } = await makeStore(SelectionInputStore, ["A:A"]);
    store.focusById(idOfRange(store, 0));
  ...
  test("consistent range colors upon refocusing multiple times", async () => {
    const { store } = await makeStore(SelectionInputStore, ["B1:B5", "C1:C5"]);
    store.focusById(idOfRange(store, 0));
  ...
  test("no duplicate range ids when creating and deleting ranges frequently", async () => {
    const { store } = await makeStore(SelectionInputStore, ["B1:B5", "C1:C5"]);
  ...
  test("Can add a empty range to a second input without pre-focusing", async () => {
    const { store: store1, container } = await makeStore(SelectionInputStore, [], true);
  ...
  test("Can change the range of a second input without pre-focusing", async () => {
    const { store: store1, container } = await makeStore(SelectionInputStore, [], true);
    const store2 = container.instantiate(SelectionInputStore, []);
    store1.focusById(idOfRange(store1, 0));
