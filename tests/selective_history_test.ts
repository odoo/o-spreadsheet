...
// editor.history.HEAD_LAYER.print();

class MiniEditor {
  private revisionIds: Set<UID> = new Set();
  private state = "";
  private undoTransformation;
  private redoTransformation;
  ...
  execAfter(insertAfter: UID | null, instruction: [UID, string, number]) {
    ...
    this.history.insertExternal(commandId, command, insertAfter || "initial");
  }
  ...
  add(commandId: UID, text: string, position: number) {
    ...
    this.history.add(commandId, command);
  }
...
describe("Undo/Redo manager", () => {
  ...
  describe("External instruction insertion", () => {
    ...
    test("undo/redo with external inbetween", () => {
      const editor = new MiniEditor();
      editor.add("1", "A", 0);
      editor.undo("1");
      editor.execAfter(null, ["2", "B", 0]);
    ...
    test("undo/redo with external inbetween 2", () => {
      const editor = new MiniEditor();
      editor.add("1", "A", 0);
      editor.undo("1");
    ...
    test("undo/redo with external inbetween 3", () => {
      const editor = new MiniEditor();
      editor.add("1", "A", 0);
      editor.add("2", "C", 1);
      editor.undo("2");
    ...
    test("cannot insert after inexisting instruction", () => {
      const editor = new MiniEditor();
      editor.add("1", "A", 0);
