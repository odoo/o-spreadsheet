...
describe("Filter menu component", () => {
  ...
  describe("Filter Tests", () => {
    ...
    test("Clear all work on the displayed values", async () => {
      ...
      await simulateClick(".o-filter-menu-confirm");
      expect(model.getters.getFilterHiddenValues({ sheetId, col: 0, row: 0 })).toEqual(["1"]);
    });
    ...
    test("Select all work on the displayed values", async () => {
      ...
      await simulateClick(".o-filter-menu-confirm");
      expect(model.getters.getFilterHiddenValues({ sheetId, col: 0, row: 0 })).toEqual(["", "2"]);
    });
    ...
    test("Clear all works on all values, not only the first 50", async () => {
      ...
      const values: { value: string; isChecked: boolean }[] = [];
      for (let i = 1; i < 60; i++) {
        values.push({ value: `${i}`, isChecked: false });
      }
      expect(getFilterMenuValues()).toEqual(values);
    });
