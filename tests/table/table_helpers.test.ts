...
describe("Table cell style", () => {
  ...
  describe("Pivot specific table style", () => {
    ...
    const firstSubSubHeaderRow: Style = { fillColor: "#119", textColor: "#119" };
    const secondSubSubHeaderRow: Style = { fillColor: "#e99", textColor: "#e99" };
    ...
    test("Can display a style on the measure row", () => {
      tableMetaData.measureHeaderIndex = 1;
    ...
    test("Can display a style on sub-sub-header rows", () => {
      tableMetaData.firstSubSubHeaderRows = new Set([1, 3]);
      tableMetaData.secondSubSubHeaderRows = new Set([2, 4]);
      tableStyle.firstSubSubHeaderRow = { style: firstSubSubHeaderRow };
      tableStyle.secondSubSubHeaderRow = { style: secondSubSubHeaderRow };

      const computedStyle = getComputedTableStyle(tableConfig, tableStyle, tableMetaData);
      expect(computedStyle.styles[0][0]).toEqual({});
      expect(computedStyle.styles[0][1]).toMatchObject(firstSubSubHeaderRow);
      expect(computedStyle.styles[0][2]).toMatchObject(secondSubSubHeaderRow);
      expect(computedStyle.styles[0][3]).toMatchObject(firstSubSubHeaderRow);
      expect(computedStyle.styles[0][4]).toMatchObject(secondSubSubHeaderRow);
    });
  });
});
...
describe("Table cell borders", () => {
  ...
  describe("Pivot specific table border", () => {
    ...
    test("Can display a border on sub-sub-header rows", () => {
      ...
      tableMetaData.firstSubSubHeaderRows = new Set([1, 3]);
      tableMetaData.secondSubSubHeaderRows = new Set([2, 4]);
      tableStyle.firstSubSubHeaderRow = { border: { top: border } };
      tableStyle.secondSubSubHeaderRow = { border: { top: border2 } };
