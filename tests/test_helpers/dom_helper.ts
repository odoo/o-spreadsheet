...
export async function simulateClick(
  ...
  if (mode === "doubleClick") {
    ...
    triggerMouseEvent(selector, "pointerup", x, y, extra);
    triggerMouseEvent(selector, "click", x, y, extra);
    triggerMouseEvent(selector, "dblclick", x, y, extra);
  }
  await nextTick();
}
...
export async function clickCell(
  ...
  x += option.offsetX || 0;
  y += option.offsetY || 0;
  const clickMode = option.doubleClick ? "doubleClick" : "click";
  await simulateClick(".o-grid-overlay", Math.ceil(x), Math.ceil(y), extra, clickMode);
}
