...
import { adaptFormulaToExcel } from "../src/xlsx/content_helpers";
...
describe("Test XLSX export", () => {
  ...
  describe("Generic sheets (style, hidden, size, cf)", () => {
    test("Simple model data with default style", async () => {
      ...
      expect(exportedXLSX).toMatchSnapshot();
    });
    test("Merges", async () => {
  ...
  describe("Charts", () => {
    test.skip("simple Chart", () => {
      console.log("first, we should support charts; then, we might do something :)");
    });
  });
});
