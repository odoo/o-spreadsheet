...
describe("Import xlsx data", () => {
  ...
  test("Can import array formula correctly and spill ranges values skipped", () => {
    ...
    expect(getWorkbookCell(0, 29, sheet)).toEqual("=RANDARRAY(2,2)");
    expect(getWorkbookCell(0, 30, sheet)).toEqual(undefined);
    expect(getWorkbookCell(1, 29, sheet)).toEqual(undefined);
    expect(getWorkbookCell(1, 30, sheet)).toEqual(undefined);
  });
  ...
  test.each([
    ...
    const expectedFormat: string | undefined = format;
  ...
  test("Can import CF with formulas", () => {
    ...
    expect(cf.rule.type).toEqual("CellIsRule");
    expect((cf.rule as CellIsRule).operator).toEqual("isBetween");
  ...
  test("Can convert table formula inside a table", () => {
    ...
    expect(testSheet.cells["H4"]).toEqual("=SUM(E3:E6)");
    expect(testSheet.cells["H5"]).toEqual("=SUM(E3:E6)");
  ...
  test.each([
    [
      "line chart",
      [{ trend: { type: "polynomial", order: 2, display: true, color: "#4472C4" } }, {}],
    ],
    ["bar chart", [{ trend: { type: "exponential", display: true, color: "#ED7D31" } }, {}]],
    ["scatter chart", [{ trend: { type: "logarithmic", display: true, color: "#A5A5A5" } }]],
    [
      "scatter chart with textual labels",
      [{ trend: { type: "polynomial", order: 1, display: true, color: "#FFC000" } }],
    ],
    [
      "combo chart",
      [
        { trend: { type: "trailingMovingAverage", window: 3, display: true, color: "#70AD47" } },
        {},
      ],
    ],
  ...
  test.each([
    ...
    ["scatter chart with textual labels", "left"],
    ["combo chart", "none"],
  ...
  test("Can import named ranges", () => {
    expect(convertedData.namedRanges).toEqual([
      { rangeName: "My_Named_Range", rangeString: "jestMiscTest!$A$8:$B$10" },
      { rangeName: "Table_Col_Named_Range", rangeString: "jestMiscTest!A8:A12" }, // Converted from table formula `Table10[Col1]`
      { rangeName: "Table_Named_Range", rangeString: "jestMiscTest!A8:B12" }, // Converted from table formula `Table10`
    ]);
  });
});
